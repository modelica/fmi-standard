...
This attribute is ignored for Model Exchange and Co-Simulation (in case of <<multipleInterfaceTypes,mixed-interface FMUs>>).
Output <<Clock,`Clocks`>> must not have this attribute.
Smaller values signal higher priorities.
Input <<Clock,`Clocks`>> that have to be activated at the same time instant are ordered according to this attribute.
...
The value of <<clocks>> is a list of value references of <<Clock,Clocks>> this variable depends on.
