...
If a `TerminalMemberVariable` or `TerminalStreamMemberVariable` is an array, "matching" includes "matching based on structured naming convention" besides pure name-equality if the following conditions are fulfilled: 
...
_[Example: Assume that an (FMI 3.0) FMU has a one-dimensional array variable `x` of length 3 as member variable of a terminal and is connected to another (FMI 2.0 or FMI 3.0) FMU supporting structured naming convention, which has scalar variables `x[1]`, `x[2]`, `x[3]` as member variables of a terminal.
Then `x[1]`, `x[2]`, `x[3]` are interpreted as a "naming convention array", which "matches" the array variable `x` of the first FMU]_ 
