...
NOTE: For example `2009-12-08T14:33:22Z`, `2025-06-12T19:14:14.839539+02:00`, etc.
...
For Co-Simulation and Model Exchange, this attribute must be ignored.
+
NOTE: This applies when the attribute is present in <<multipleInterfaceTypes,mixed interface FMUs>>.
...
This attribute defines the time interval latexmath:[T_{\mathit{interval}}] between consecutive <<Clock>> activations.
This value must be greater than 0.0.
This attribute is required for <<constant-clock,constant>> <<periodic-clock,periodic Clocks>>, and is optional for <<fixed-clock,fixed>> and <<tunable-clock,tunable>> <<periodic-clock,periodic Clocks>>.
It must not be used otherwise.

|`shiftDecimal`
|
[[shiftDecimal,`shiftDecimal`]]
This attribute defines the delay until the first Clock activation relative to latexmath:[t_{\mathit{start}}] (<<startTime>>).
...
|`intervalCounter`
|[[intervalCounter,`intervalCounter`]]This attribute defines (together with <<resolution>>) the interval between consecutive Clock activations:
...
|`shiftCounter`
|[[shiftCounter,`shiftCounter`]]This value defines (together with <<resolution>>) the delay until the first Clock activation relative to latexmath:[t_{\mathit{start}}] (<<startTime>>):
...
Multiple variables may reference the same variable via `derivative`.

A variable with attribute `derivative` that is not listed as a <<ContinuousStateDerivative>> is not used for integration.

[NOTE]
====
Variables with attribute `derivative` that are not listed as <<ContinuousStateDerivative>> include, for example:

* An <<input>> or <<output>> that is the derivative of another <<input>> or <<output>>, respectively, where the `derivative` attribute signals this relationship to the importer.
* When mapping a second-order system to first-order <<state,states>>, a velocity <<state>> that has `derivative` referencing the position <<state>> to preserve the higher-order structure.
====
...
If `true`, the <<state>> may be reinitialized by the FMU in <<EventMode>>. +
If `false`, the <<state>> will not be reinitialized.
...
Attribute <<dependencies>> for <<output>> <<Clock,`Clocks`>> lists all known variables (including input clocks) that contribute to cause an activation of that <<outputClock,output Clock>>.
...
The referred variables must have the `derivative` attribute.
...
NOTE: The association of a clocked variable with its Clocks must not be listed here as a dependency; this association is defined by the <<clocks>> attribute in `<fmiModelDescription><ModelVariables>`.
...
The terminal name must be unique among siblings; a child terminal may have the same name as its parent.
Terminal names are independent from the names of <<ModelVariables>>.
...
The naming of <<ModelVariables>> is independent from the terminal names and member variable names, regardless of whether the flat or structured naming convention is used.
...
These header files are not included in the `binaries` directory, since they are already utilized to build the executable of the importer.
