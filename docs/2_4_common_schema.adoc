...
This flag is ignored in `ScheduledExecution`.
...
For Co-Simulation the value of the independent variable is <<lastSuccessfulTime>> return by the last call to <<fmi3DoStep>> or the value of argument <<intermediateUpdateTime>> of <<intermediateUpdate,fmi3CallbackIntermediateUpdate>>.
