...
Variable connection check::
+
_When two variables v1 and v2 are connected together, and on at least one of the variables no `<BaseUnit>` element is defined, then the connection equation "v2 = v1" holds (if v1 is an <<output>> of an FMU and v2 is an <<input>> of another FMU, with `fmi3Get{VariableType}` the value of v1 is inquired and used as value for v2 by calling `fmi3Set{VariableType}`)._
+
_When two variables v1 and v2 are connected together, and for both of them `<BaseUnit>` elements are defined, then they must have identical exponents of their `<BaseUnit>`._
