...
|
[[initial,`initial`]]
Enumeration that defines how the variable is initialized, i.e. if a <<fmi3SetXXX>> is allowed and how the FMU internally treats this value in *Instantiated* and *Initialization Mode*.

For a variable with <<causality>> = <<input>>, the attribute <<initial>> must not be provided, because calling <<fmi3SetXXX>> is not restricted in *Initialization Mode* and the FMU cannot change this value internally.
