...
Additional optional information about the graphical representation and the grouping of FMU variables into terminals is stored in the optional text file `appearance/terminalsAndIcons.xml` in XML format as specified by the XML schema file `fmi3TerminalsAndIcons.xsd`.
...
For Co-Simulation the value of the independent variable is <<lastSuccessfulTime>> return by the last call to <<fmi3DoStep>> or the value of argument <<intermediateUpdateTime>> of <<fmi3CallbackIntermediateUpdate>>.
...
_[Some importers might require a variable to be dependent on a single clock for technical reasons. They could reject FMUs violating this restriction.]_
|====
...
Both features are optional and can be defined in the separate XML file `appearance/terminalsAndIcons.xml`. +
...
|`fmiVersion`
|This attribute declares the FMI version defining the XSD of the `appearance/terminalsAndIcons.xml` file.
...
If used in previous FMI versions, this attribute points to the FMI version defining this `appearance/terminalsAndIcons.xml` file (e.g. XSD, file location).
...
The optional image file of the FMU icon is placed at the path `appearance/icon.png` in the ZIP archive of the FMU.
The terminals should not be visible in the image.
Optionally an SVG file with path `appearance/icon.svg` can be provided if also the PNG file is present.
...
The base URI that this relative URI is resolved against is the URI of the `appearance/terminalsAndIcons.xml` file in the FMU ZIP archive.
...
_[Note that this specification is functionally equivalent to looking up image sources from the `appearance` folder of the FMU ZIP archive after dot removal from the path as per section 5.2.4 of RFC 3986.]_
