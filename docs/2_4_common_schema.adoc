...
This attribute is required if <<intervalVariability,intervalVariability == `constant`>> and attribute <<intervalCounter>> is not used.
...
This attribute is allowed if <<supportsFraction,`supportsFraction = true`>> and is required if <<intervalVariability,intervalVariability == `constant`>> and attribute <<intervalDecimal>> is not used.
...
This attribute is allowed if <<supportsFraction,`supportsFraction = true`>>.
...
Scheduled Execution: The value of the variable only changes when a model partition the variable belongs to is active.
