...
This attribute is required if <<supportsFraction,`supportsFraction = false`>> and Clock type is <<constant-clock,`constant`>>, <<fixed-clock,`fixed`>> or <<tunable-clock,`tunable`>> <<periodic-clock,periodic Clocks>>.
...
This attribute is required if <<supportsFraction,`supportsFraction = true`>> and Clock type is <<constant-clock,`constant`>>, <<fixed-clock,`fixed`>> or <<tunable-clock,`tunable`>> <<periodic-clock,periodic Clocks>>.
It must not be used otherwise.
