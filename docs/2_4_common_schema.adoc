...
The attributes of variables are shown in <<figure-schema-fmi3AbstractVariable>>.

.fmi3AbstractVariable element (base for fmi3Enumeration and fmi3TypedVariable).
[[figure-schema-fmi3AbstractVariable]]
image::images/schema/fmi3AbstractVariable.png[width=80%, align="center"]

.fmi3TypedVariable element (based on fmi3AbstractVariable).
[[figure-schema-fmi3TypedVariable]]
image::images/schema/fmi3TypedVariable.png[width=80%, align="center"]

.fmi3InitializableVariable element (based on fmi3TypedVariable, base for fmi3Float32, fmi3Float64, fmi3Int8, fmi3UInt8, fmi3Int16, fmi3UInt16, fmi3Int32, fmi3UInt32, fmi3Int64, fmi3UInt64, fmi3Boolean, fmi3String and fmi3Binary).
[[figure-schema-fmi3InitializableVariable]]
image::images/schema/fmi3InitializableVariable.png[width=80%, align="center"]

.fmi3Clock element (based on fmi3TypedVariable).
[[figure-schema-fmi3Clock]]
image::images/schema/fmi3Clock.png[width=80%, align="center"]
...
The following constraints apply: only clocked variables (i.e. variables with the attribute <<clocks>>) may have the <<previous>> attribute; only variables with <<variability,`variability == discrete`>> may have a <<previous>> value; two variables must not have the same <<previous>> value reference; a variable must not refer to itself in the <<previous>> attribute; and the variable referenced by the <<previous>> attribute must have the same numeric type, <<variability,`variability == discrete`>>, and must not have <<causality,`causality == input`>>.
...
<<Clock>> variables must not have <<initial>> attributes.
...
Attribute <<dependencies>> for <<output>> <<Clock,`Clocks`>> lists all known variables (including input clocks) that contribute to trigger a clock tick for that output clock.
