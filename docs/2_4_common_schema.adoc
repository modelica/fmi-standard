...
If a `TerminalMemberVariable` or `TerminalStream memberVariable` is an array, "matching" includes "matching based on structured naming convention" besides pure name-equality und the following conditions:

* The FMU uses the structured naming convention.
* The "nanming convention array" member variables end with the array indices in squared brackets.
* The syntax is `B-name [ arrayIndices ]`, whereas `B-name` the "naming convention array" name (see the precise syntax in the structured naming convention).
* The "naming convention array" is 0 or 1 based.
* All elements of the "naming convention array" must be present as scalar variabls in consecutive order
* A ScalarVariable with the "naming convention array" name must not exist. I.e. if ScalarVariable `[1]` exists then ScalarVariable `x` must not exist).

_[Example: Assume that an (FMI3) FMU has a one-dimensional array variable `x` of lenght 3 as member variable of a terminal, and is connected to another (FMI2 or FMI3) FMU using structured naming convention that has scalar variables `x[1]`, `x[2]`, `x[3]` as member variables of a terminal.
