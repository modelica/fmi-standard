...
_[The purpose of this attribute is to define when a variable can be set and when it makes sense to inquire its potentially changed value._
_For example, <<discrete>> variables change their values only at event instants (ME) or at communication points (CS and SE) and it is therefore only necessary to inquire them with <<get-and-set-variable-values,`fmi3Get{VariableType}`>> them at these points in time.]_
...
[[discrete,`discrete`]]
`= discrete`: +
The value of the variable may change only in <<EventMode>>. +
Co-Simulation without <<EventMode>> and Scheduled Execution: The value may change only at communication points.
During <<IntermediateUpdateMode>>, <<discrete>> variables are not allowed to change.
The <<variability>> of <<clocked-variable,`clocked variables`>> must have <<discrete>>.
...
Variables with <<variability>> `continuous` may change in <<InitializationMode>> and super state <<Initialized>>.
...
_[The importer decides when to use the <<start>> value of a variable with <<causality>> = <<input>>._
_For example, if the <<input>> variable is an iteration variable of an algebraic loop in <<InitializationMode>>, then the <<start>> value can be used as start value for the iteration.]_

If <<get-and-set-variable-values,`fmi3Set{VariableType}`>> is not called on a variable with <<causality>> = <<input>>, then the FMU must keep the <<start>> value as value of this <<input>>.
