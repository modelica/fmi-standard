...
.Clock attribute details.
[[table-clock-attributes]]
[cols="1,5",options="header"]
|====
|Attribute
|Description
...
An <<outputClock,`output clock`>> can be referenced by more than one <<triggeredBy>> attribute of <<inputClock, `input clocks`>>.
...
If a <<periodic>> <<clock>> is constant periodic, the `interval` attribute is `constant`.
If the required attribute `interval` is set to `true`, then the FMU and the simulation algorithm have to respect the predefined interval and offset.
If the attribute `interval` is set to `fixed`, `calculated` or `tunable`, another interval or offset can be used, derived from the current simulation setup.
