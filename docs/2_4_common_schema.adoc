...
_[The <<independent>> variable should be treated as any other variables when it comes to dependencies.
As only explicit dependencies are listed it is required that unknowns that depends on the <<independent>> variable has a partial derivative that is different from identical zero.
If the FMU suports <<directionDerivatives,directional derivatives>> or <<adjointDerivatives,adjoined derivatices>> the <<independent>> variable is a valid input to those functions if it is listed, just as any other listed variable.
Example, for a mechanical system, if it has the <<independent>> variable listed is called a rheonomous system, while if the <<independent>> variable is not listed among the dependencies, it is a scleronomous system.]_
