...
|`intervalAndShiftDenominator`
|[[intervalAndShiftDenominator, `intervalAndShiftDenominator`]]Instead of defining clock timing using floating point numbers, FMI allows the definition of rational numbers using <<intervalNumerator>>, <<shiftNumerator>> and <<intervalAndShiftDenominator>>.
This attribute is required for <<periodic-clock,time-based periodic clocks>> if <<supportsFraction,`supportsFraction = true`>> and <<intervalNumerator>> is present.
This attribute must only be used for <<periodic-clock,periodic Clocks>>.

|`intervalNumerator`
|[[intervalNumerator,`intervalNumerator`]]This attribute defines (together with <<intervalAndShiftDenominator>>) the interval between consecutive clock ticks:

latexmath:[T_{\mathit{interval}} = ] `intervalNumerator / intervalAndShiftDenominator`.

<<intervalNumerator>> and <<intervalAndShiftDenominator>> have no default value.
...
|`shiftNumerator`
|[[shiftNumerator,`shiftNumerator`]]This value defines (together with <<intervalAndShiftDenominator>>) the delay of the first clock activation relative to latexmath:[t_{\mathit{start}}] (<<startTime>>):

latexmath:[T_{\mathit{shift}} = ] `shiftNumerator / intervalAndShiftDenominator`.

This attribute requires that <<intervalNumerator,`intervalNumerator`>> is present.
