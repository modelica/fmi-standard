...
_[Example: Assume that an (FMI 3.0) FMU has a one-dimensional array variable `x` of lenght 3 as member variable of a terminal and is connected to another (FMI 2.0 or FMI 3.0) FMU supporting structured naming convention, which has scalar variables `x[1]`, `x[2]`, `x[3]` as member variables of a terminal.
