...
This attribute defines the time interval latexmath:[T_{interval}] between consecutive <<Clock>> ticks.
...
latexmath:[T_{interval} = ] `intervalCounter / resolution`.
...
latexmath:[T_{shift} = ] `shiftCounter / resolution`.
...
latexmath:[{\mathbf{v}_{\mathit{initialUnknowns}} := \mathbf{f}_{\mathit{init}}(\mathbf{u}_c, \mathbf{u}_d, t_{start}, \mathbf{v}_{\mathit{initial=exact}})}]
...
latexmath:[{(\mathbf{y}_{c+d}, \dot{\mathbf{x}}_c) := \mathbf{f}_{\mathit{init}}(\mathbf{x}_c, \mathbf{u}_{c+d}, t_{start}, \mathbf{p})}] +

latexmath:[{(\mathbf{y}_{c+d}, \dot{\mathbf{x}}_c) := \mathbf{f}_{\mathit{sim}}(\mathbf{x}_c, \mathbf{u}_{c+d}, t_i, \mathbf{p})}] +

_Therefore, the initial state_ latexmath:[{\mathbf{x}_c(t_{start})}] _has <<initial>> = <<exact>> and the initial state derivative_ latexmath:[{\dot{\mathbf{x}}_c(t_{start})}] _has <<initial>> = <<calculated>>._
_The importer can still initialize this FMU in steady-state, by using_ latexmath:[{\mathbf{x}_c(t_{start})}] _as iteration variables and adding the equations_ latexmath:[{\dot{\mathbf{x}}_c(t_{start}) = \mathbf{0}}] _in the importer.]_
