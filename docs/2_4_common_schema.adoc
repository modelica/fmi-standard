...
|
[[previous,`previous`]]
If present, this variable contains the previous value of the <<discrete>> state with value reference specified by this attribute.
...
A <<ClockedState>> is a <<discrete>>, <<local>> variable that changes only when at least one clock is active.
...
|
[[ClockElement,`<Clock>`]]
List of all clocks and which clocked variables depend on that clock.
