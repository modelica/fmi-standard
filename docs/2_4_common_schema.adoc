...
A Model Exchange FMU must expose all <<derivative,`derivatives`>> of its continuous-time <<state,`states`>> in `<ModelStructure><Derivative>` elements and must expose all event indicators in `<EventIndicator>` elements.
