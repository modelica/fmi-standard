...
If a `TerminalMemberVariable` or `TerminalStream memberVariable` is an array, "matching" includes "matching based on structured naming convention" besides pure name-equality.  

_[
  This means: 
  Assume that an (FMI3) FMU has a one-dimensional array variable `x` of lenght 3 as member variable of a terminal, and is connected to another (FMI2 or FMI3) FMU that has scalar variables `x[1]`, `x[2]`, `x[3]` as member variables of a terminal.
