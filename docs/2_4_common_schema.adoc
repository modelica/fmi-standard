...
If present this value must be greater than 0.
This attribute is can be present if <<supportsFraction,`supportsFraction = true`>> and Clock type is <<constant-clock,`constant`>>, <<fixed-clock,`fixed`>> or <<tunable-clock,`tunable`>> <<periodic-clock,periodic Clocks>>.
It must not be used otherwise.
