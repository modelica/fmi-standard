...
Terminals and Icons are a <<VersioningLayered,layered standard>> that defines semantic groups of variables to ease connecting compatible signals on the system level.
...
Build Configurations are a <<VersioningLayered,layered standard>> that provides the necessary information to compile and link the sources of the model into a dynamic library or as part of an executable.
