...
For continuous variables this corresponds to that there exist values for the knowns where the partial derivative of the unknown with respect to a known is non-zero.
...
When the independent variable is listed among the dependencies this implicit dependency is not what is meant, but rather that the unknown's partial derivative with respect to the independent variable can be non-zero.
