...
If <<variability>> = <<clock>> the variable defines an <<inputClock>> that is controlled by the importer.
An <<inputClock>> can be left unconnected, can be connected to an <<outputClock>>, or the importer generates the clock variable.
...
If <<variability>> = <<clock>> the variable defines an <<outputClock>> that is controlled by the FMU.
An <<outputClock>> can be left unconnected or can be connected to an <<inputClock>>, usually of another FMU (for exceptions see <<triggeredBy>>).
