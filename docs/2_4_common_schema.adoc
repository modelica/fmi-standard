...
The namespace mechanism for the `type` attribute is based on <<reverse-DNS>>:
...
In order to avoid ambiguities and conflicts, rule names must follow the <<reverse-DNS>> of a domain that is controlled by the entity defining the semantics and content of the additional entries.
