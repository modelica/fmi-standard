...
_For example, the order of the <<ContinuousStateDerivative>> elements defines the order to be used in <<fmi3GetContinuousStates>>, <<fmi3SetContinuousStates>>, and <<fmi3GetContinuousStateDerivatives>>.]_
...
_For example, when loading an FMU with `needsExecutionTool = true`, the environment can inform the user that a tool has to be available on the computer where the FMU is instantiated._
...
This attribute defines the time interval latexmath:[\mathbf{T}_{interval}] between consecutive <<Clock>> ticks.
...
This attribute defines the delay of the first clock activation relative to latexmath:[\mathbf{t}_{\mathit{start}}] (<<startTime>>).
...
|`shiftCounter`
|[[shiftCounter,`shiftCounter`]]This value defines (together with <<resolution>>) the delay of the first clock activation relative to latexmath:[\mathbf{t}_{\mathit{start}}] (<<startTime>>):
...
iteratative solution process in <<InitializationMode>> starts with the <<start>> value as guess value.
...
Variables with <<causality>> = <<parameter>> or <<input>>, as well as variables with <<variability>> = <<constant>>, must have a <<start>> attribute wich has the following meaning:
...
In the following definitions, the http://en.wikipedia.org/wiki/Extended_BNF[Extended Backus-Naur Form (EBNF)] is used:
...
latexmath:[{(\mathbf{y}_c, \mathbf{y}_d) := \mathbf{f}_{\mathit{output}}(\mathbf{x}_c, \mathbf{u}_c, \mathbf{u}_d, \mathbf{t}, \mathbf{p})}]
...
The functional dependency is defined as: +
latexmath:[{\dot{\mathbf{x}_c} := \mathbf{f}_{\mathit{der}}(\mathbf{x}_c, \mathbf{u}_c, \mathbf{u}_d, \mathbf{t}, \mathbf{p})}]
...
latexmath:[{\mathbf{v}_{\mathit{initialUnknowns}} := \mathbf{f}_{\mathit{init}}(\mathbf{u}_c, \mathbf{u}_d, \mathbf{t}_{start}, \mathbf{v}_{\mathit{initial=exact}})}]
...
latexmath:[{(\mathbf{y}_{c+d}, \dot{\mathbf{x}}_c) := \mathbf{f}_{\mathit{init}}(\mathbf{x}_c, \mathbf{u}_{c+d}, \mathbf{t}_{start}, \mathbf{p})}] +

latexmath:[{(\mathbf{y}_{c+d}, \dot{\mathbf{x}}_c) := \mathbf{f}_{\mathit{sim}}(\mathbf{x}_c, \mathbf{u}_{c+d}, \mathbf{t}_i, \mathbf{p})}] +

_Therefore, the initial state_ latexmath:[{\mathbf{x}_c(\mathbf{t}_{start})}] _has <<initial>> = <<exact>> and the initial state derivative_ latexmath:[{\dot{\mathbf{x}}_c(\mathbf{t}_{start})}] _has <<initial>> = <<calculated>>._
_The importer can still initialize this FMU in steady-state, by using_ latexmath:[{\mathbf{x}_c(\mathbf{t}_{start})}] _as iteration variables and adding the equations_ latexmath:[{\dot{\mathbf{x}}_c(\mathbf{t}_{start}) = \mathbf{0}}] _in the importer.]_
...
Optional attribute defining the dependencies as list of value references of the unknown latexmath:[{\mathbf{v}_{\mathit{unknown}}}] directly with respect to latexmath:[{\mathbf{v}_{\mathit{known}}}].
For continuous variables this corresponds to that there exist values for the knowns where the partial derivative of the unknwon with respect to a known is non-zero.
...
_[This means that all dependencies must be listed, also the dependencies of clocked variables on their input clocks, even if redundant information.]_ +
...
_[For a dynamic system most of the unknowns would have an implict dependency on the independent variable.
When the independent variable is listed among the dependencies this this implicit dependency is not what is ment, but rather that the unknown's partial derivative with respect to the independent variable can be non-zero.
For a mechanical system this is called a rheonomous system, while if the independent variable is not listed in the dependencies, it is a scleronomous system.]_
...
If <<dependenciesKind>> is not present, it must be assumed that the unknown latexmath:[{\mathbf{v}_{\mathit{unknow}}}] depends on the knowns latexmath:[{\mathbf{v}_{\mathit{known}}}] without a particular structure.
...
_Note that_ latexmath:[{y = d \cdot u}] _where_ latexmath:[{d}] _changes only during <<EventMode>> (_ latexmath:[{d = 2 \cdot u}] _or_ latexmath:[{3 \cdot u\ }] _depending on relation_ latexmath:[{u > 0}] _that changes only at <<EventMode>>)._
...
_Note that_ latexmath:[{y = c}] _where_ latexmath:[{c}] _changes only during <<EventMode>> (_ latexmath:[{c = 2}] _or_ latexmath:[{3\ }] _depending on relation_ latexmath:[{u > 0}] _that changes only at <<EventMode>>)._
...
_where `u` is continuous-time input value reference `1`, `y` is a continuous-time output with value reference `2` and `dxdt` is a continuous-time derivative with value reference `4`._
_The definition of the model structure is then:_
