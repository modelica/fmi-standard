...
<<exporter,Exporters>> must not generate capability flags without an <<XML>> namespace, unless specified by this standard or any later release of this standard.
...
If a <<state>> or a <<derivative>> of a <<state>> shall not be exposed from the FMU, or if states are not statically associated with a variable (due to dynamic state selection), then dummy variables have to be introduced, for example, `x[4]`, or `xDynamicStateSet2[5]`.
...
The graphical representation of the FMU and terminals are needed to more easily comprehend the meaning of connected FMUs and to help importers to display the terminals and the FMU icon in the way the <<exporter,exporter>> intended.
