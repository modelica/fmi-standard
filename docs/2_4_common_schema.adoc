...
If a <<structuralParameter,`structural parameters`>> is referenced in `<Dimension>` elements, it must be of type `<UInt64>` and its <<start>> attribute must larger than 0.
