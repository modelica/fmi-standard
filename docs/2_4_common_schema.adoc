...
|`priority`
|[[priority,`priority`]]
This attribute is only considered for <<Clock,`Clocks`>> in Scheduled Execution.
...
The attributs defined in the corresponding `<TypeDefinition>` (see <<definition-of-types>>) are used as defaults.
...
|`unit`
|This attributes defines the unit of the variable by referencing a `<UnitDefinitions><Unit>` with the same `name`, which must exist.
...
Optional attribute defining the dependencies as list of value references of the unknown latexmath:[{\mathbf{v}_{\mathit{unknown}}}] (directly or indirectly via other variables) with respect to latexmath:[{\mathbf{v}_{\mathit{known}}}].
