...
|
[[previous,`previous`]]
If present, this variable contains the previous value of the <<ClockedState>> with value reference specified by this attribute.
Only clocked variables (they are listed in at least one of the <<clockedVariables>> lists) may have the <<previous>> attribute.
All clocked variables must have <<variability,`variability == discrete`>>.
Clocks must not have a <<previous>> attribute.
_[For example, if `previous == 3` for variable `8`, then variable `8` is the previous value of variable `3`. See also <<fmi3UpdateDiscreteStates>>.]_
...
For each <<ClockedState>> there must be one and only one clocked variable with the attribute <<previous>> to represent the previous value of this <<ClockedState>>.
