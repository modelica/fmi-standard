...
|The <<instantiationToken>> is a string that may be used by the FMU to check that the XML file is compatible with the implementation of the FMU.
...
|If `true`, the environment may serialize the internal FMU state, in other words, functions <<fmi3SerializedFMUStateSize>>, <<fmi3SerializeFMUState>>, <<fmi3DeSerializeFMUState>> are supported by the FMU.
...
|If `true`, the FMU supports <<fmi3EvaluateDiscreteStates>>.
...
This attribute is  required if <<supportsFraction,`supportsFraction = false`>> and clock type is <<constant-clock,`constant`>>, <<fixed-clock,`fixed`>> or <<tunable-clock,`tunable`>> <<periodic-clock,periodic Clocks>> it must not be used otherwise.
This attribute cannot be combined with <<intervalCounter,`intervalCounter`>>
...
This attribute is  required if <<supportsFraction,`supportsFraction = true`>> and clock type is <<constant-clock,`constant`>>, <<fixed-clock,`fixed`>> or <<tunable-clock,`tunable`>> <<periodic-clock,periodic Clocks>> it must not be used otherwise.
This attribute cannot be combined with <<intervalDecimal,`intervalDecimal`>>
...
Allowed values of this enumeration are:
...
The algebraic relationships to the <<input,`inputs`>> can be defined via the <<dependencies>> attribute of <<Output,`<fmiModelDescription><ModelStructure><Output>`>>.

[[local,`local`]]
`= local`: Local variables are: +

* continuous <<state,states>> and their <<ContinuousStateDerivative,`ContinuousStateDerivatives`>>, <<ClockedState,`ClockedStates`>>, <<EventIndicator,`EventIndicators`>> or <<InitialUnknown,`InitialUnknowns`>>.
These variables are listed in the <<ModelStructure>>. +
* [[localClock,`local clock`]] internal, intermediate variables or local clocks which can be read for debugging purposes and are not listed in the <<ModelStructure>>.
...
Optional attribute defining the dependencies of the unknown latexmath:[{\mathbf{v}_{\mathit{unknown}}}] (directly or indirectly via other variables) with respect to latexmath:[{\mathbf{v}_{\mathit{known}}}].
