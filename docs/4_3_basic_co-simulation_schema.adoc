...
|If `true`, the slave is able to return early from <<fmi3DoStep>> if the master returns <<fmi3EarlyReturn>> from the callback <<intermediateUpdate>> and <<canReturnEarly,`canReturnEarly == fmi3True`>>.
