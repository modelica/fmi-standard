...
_[If set to `true`, a Basic Co-Simulation FMU supports ending <<fmi3DoStep>> before the planned next communication point._
_This is can be used by the simulation master to avoid unnecessary computations and roll backs of the FMU due to external events known by the simulation master.]_
