...
The accessible variables are marked with attribute <<intermediateAccess,`intermediateAccess = true`>>.

|<<canReturnEarlyAfterIntermediateUpdate>>
|If `true`, the slave is able to return early from <<fmi3DoStep>> if the master returns <<earlyReturnRequested,`earlyReturnRequested == fmi3True`>> from the callback <<intermediateUpdate>> and <<canReturnEarly,`canReturnEarly == fmi3True`>>.
...
|A value of latexmath:[k] with latexmath:[k>0] signals the to the master, that it is beneficial for the solver to receive intermediate inputs that are k-time continuously differentiable (latexmath:[C^k]) on the current communication interval. latexmath:[k=0] means "smooth" on the current communication interval, latexmath:[k=1000] can be used to signal the requirement (latexmath:[C^\infty]) and latexmath:[k=-1] decodes that there are no smoothness requirement on the inputs.
_[If for example the FMU uses internally a variable step BDF solver of order 4, in order to fulfill the convergence rate, (latexmath:[C^4]) is required for the inputs. If on the other hand, an FMU sub-smaples the communication step with 10 fixed steps and ueses an first order explicit Euler method as solver, no or_ latexmath:[C^0] _smoothness will be suffient ]_
