...
Please note that in mode `fmi3ScheduledExecutionSimulation` the handling of `fmi3IntermediateUpdate()` callbacks is the same as in section 4.[TBD] the only difference is that the parameter value for earlyReturn is always set to `fmi3False`.
...
The `fmi3DoStep()`, `fmi3SetClock()`, `fmi3SetIntervalDecimal()`, `fmi3SetIntervalFraction()`, `fmi3BreakStep()` functions are not called if fmuType is set to `fmi3ScheduledExecutionSimulation`.
