...
If required, the FMU can internally derive the clock interval latexmath:[\mathbfT_i] based on the last clock tick time latexmath:[t_(i-1)] i.e. last activationTime for this clockReference (based on last `fmi3ActivateModelPartition()` call)
...
We consider one slave with two model partitions.

[source, c]
----
include::examples/co_simulation_scheduled_execution.pseudo[]
----