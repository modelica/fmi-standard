...
_[Please note: It is possible to define only inferred or only triggered clocks in the modelDescription.xml.]_ #[TODO]: MN: Do we have any other clock types to stress on that?#
...
If a clock, triggered or inferred, ticks,  the FMU invokes the `{INTERMEDIATEUPDATE}` callback and sets `{STIUI}.clocksTicked` to `fmi3True`. 
Then the master must call `{DER}` to do an early return from `{DOSTEP}`.
...
Note that, the master checks the occurrence of triggered clocks through the `{INTERMEDIATEUPDATE}` callback. 
It shall not be allowed to call `{INTERMEDIATEUPDATE}` prior to a previous call with `{STIUI}.intermediateStepFinished` is `fmi3True`. #[TODO]: Recheck if this is the correct position. (Issue #546 https://github.com/modelica/fmi-standard/issues/546#issuecomment-505755107)#
...
If a triggered clock happens, i.e., if `{DOSTEP}` returns with `earlyReturn` = `fmi3True` and `{STIUI}.eventOccurred` or `{STIUI}.clocksTicked` is `fmi3True`, the FMU must be pushed into the Event Mode by calling  `{ENTEREVENTMODE}`;
