...
It shall not be allowed to call `{INTERMEDIATEUPDATE}` prior to a previous call with `{INTERMEDIATEUPDATEINFO}.intermediateStepFinished` is `fmi3True`. #[TODO]: Recheck if this is the correct position. (Issue #546 https://github.com/modelica/fmi-standard/issues/546#issuecomment-505755107)#
