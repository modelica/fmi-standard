...
:INTERMEDIATESTEPPFINISHED: fmi3IntermediateUpdate()
...
Note that if a triggered clock happens, i.e., if `{DOSTEP}` returns with `earlyReturn` = `fmi3True` and `fmi3IntermediateInfo.eventOccurred` or `fmi3IntermediateInfo.clocksTicked` is `fmi3True`, a switch to Event Mode is mandatory.
