...
After <<fmi3ActivateModelPartition>> has been called for a <<fixedComputed-clock>>, <<tunableComputed-clock>> or <<changing-aperiodic-clock>> Clock the FMU provides the information on when the Clock will tick again, i.e. when the corresponding model partition must be scheduled the next time.
