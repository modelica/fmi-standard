...
#TODO: Discuss when to use italic and when to use brackets and update the spec accordingly.#
...
_An example for scheduling based on priorities and preempting model partitions is given in section <<example-scheduled-execution>>._
...
In general this means that the FMU's code has to secure access to its global states and variable wherever data inconsistencies due to potential preemptions are anticipated.
...
_If every #variable TODO: Needs to be more precise? "Model variable of ..."# is assigned uniquely to a model partition it is not necessary to use <<preemption-support, `preemption locks`>> in the context of <<get-and-set-variable-values,`fmi3Get{VariableType}`>> and <<get-and-set-variable-values,`fmi3Set{VariableType}`>>._
* _for <<fmi3GetClock>> the FMU has to ensure that the active state of an output Clock is securely reset and cannot be observed twice for the same Clock tick in case this call is preempted._
...
_The values returned by <<get-and-set-variable-values,`fmi3Get`>> that is placed after <<fmi3ActivateModelPartition>> may return the values computed by the other model partition if that model partition has higher priority and preempted the execution meanwhile._
* _An input variable is used by two different model partitions._
_The variable values of a model partitions of lower priority may have been overwritten by a call to <<get-and-set-variable-values,`fmi3Set`>> for a model partition of higher priority.
]_