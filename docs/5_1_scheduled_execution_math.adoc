...
If a new interval latexmath:[T_{\mathit{interval, i}}] is provided, i.e. return argument <<qualifiers>> = <<fmi3IntervalChanged>>, the importer initiates the scheduling of the associated model partitions regarding the delay of the returned interval.
