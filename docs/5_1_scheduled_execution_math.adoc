...
_An example is given in <<example-scheduled-execution>>._
* _depending on their implementation <<preemption-support,`fmi3LockPreemptionCallback`>> and <<preemption-support,`fmi3UnlockPreemptionCallback`>> have a strong impact on the simulator's OS so their use should be as rare and short as possible._
