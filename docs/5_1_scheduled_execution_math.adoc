...
The Scheduled Execution interface has a different timing concept compared to the other two Co-Simulation interfaces.
This is required to cover <<clock>> ticks for <<periodic,aperiodic>> <<inputClock,`input clocks`>> which may tick at time instances that are not predictable in advance for the simulation master.
...
The activation of such an <<outputClock>> is not controlled by the Co-Simulation master but internally by the FMU.
