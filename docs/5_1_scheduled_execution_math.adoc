...
_See <<textExample, example>> at the end of this section._
...
_An example is given in <<example-scheduled-execution>>._
* _depending on their implementation <<preemption-support,`fmi3CallbackLockPreemption`>> and <<preemption-support,`fmi3CallbackUnlockPreemption`>> have a strong impact on the scheduler and therefore the use should be as rare and short as possible._
...
_In such cases no locks are necessary.]_

[[textExample]]
_[The following example illustrates why every variable should be assigned uniquely to one model partition via its associated <<Clock>>:_
