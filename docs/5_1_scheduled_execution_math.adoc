...
If required, the FMU can internally derive the <<clock>> interval latexmath:[\mathbf{T}_\mathit{interval, i}] based on the last <<clock>> tick time latexmath:[t_{i-1}] i.e. last activation time for this model partition.
