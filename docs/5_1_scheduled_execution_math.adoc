...
This is required to cover <<clock>> ticks for <<aperiodic-clocks,aperiodic>> <<inputClock,`input clocks`>> which may tick at time instances that are not predictable in advance for the simulation algorithm.
