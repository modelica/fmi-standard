...
//EDITQ: for the sake of good readability, it is imho ambiguous to use here "thanks to" "Early Return" because this is actually the name of the callback used to interrupt other FMUs running concurrently. instead I would say:
//EDITQ: "the master is informed of an early return using `fmi3IntermediateStepFinished()` callback function" 
