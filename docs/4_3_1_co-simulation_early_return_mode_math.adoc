...
//TODO: "the master is informed of an early return using `fmi3IntermediateUpdate()` callback function"
...
In order to stop the execution of ongoing `{DOSTEP}`, the Master invokes `fmi3DoEarlyReturn()` during (i.e. within)  the `fmi3IntermediateUpdate()` callback function and provides an `earlyReturnTime`.
