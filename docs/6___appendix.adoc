...
The ticking of a <<output-clock>> may lead to an <<input-clock>> tick for another FMU (has to be defined via Co-Simulation master) or for a model partition of the same FMU (is defined in `modelDescription.xml`).
...
The computation of model partitions can be externally controlled based on <<clock>> ticks of associated <<input-clock,`input clocks`>>.
...
Nevertheless, it is assumed that the activation of all exposed <<input-clock,`input clocks`>> results in the computation of the complete FMU.
