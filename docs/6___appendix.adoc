...
The ticking of a _triggered clock_ may lead to an _inferred clock_ tick for another FMU (has to be defined via co-simulation master) or for a model partition of the same FMU (is defined in modelDescription.xml).
...
|_interrupt_
|_Event_ connected to the tick of an inferred clock of an FMU that is caused by an external potentially random process, i.e. hardware interrupt, software interrupt. 
...
The computation of model partitions can be externally controlled based on clock ticks of associated inferred clocks.
...
Nevertheless, it is assumed that the activation of all exposed inferred clocks results in the computation of the complete FMU.
