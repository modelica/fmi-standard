...
invoke the  `fmi3IntermediateUpdate()` callback function, so that the
`fmi3IntermediateInfo` is sent back to the co-simulation master.

[source, C]
----
include::../headers/fmi3FunctionTypes.h[tag=IntermediateInfo]
----
