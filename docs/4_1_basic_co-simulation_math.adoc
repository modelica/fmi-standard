...
The values of these variables are defined outside of the model.
latexmath:[\mathbf{u}_{i,IVA}] are smooth functions (latexmath:[C^{∞}]) defined on the interval latexmath:[ [tc_i, tc_{i+1}] with latexmath:[\mathbf{u}_{i,IVA}(tc_i)=\mathbf{u}(tc_i)]
...
| latexmath:[\mathbf{y}_{i, IVA}(t)= \mathbf{f}_{\mathit{Intermediate}}(\mathbf{u}_{i, c+d}, \mathbf{u}_{i, IVA} ( t \in [tc_i, t]),  t, hc_i, \mathbf{p}_{\mathit{tune}}, \mathbf{p}_{\mathit{other}})_{tc_i}]
|`[blue]#fmi3Get{VariableType}#`
