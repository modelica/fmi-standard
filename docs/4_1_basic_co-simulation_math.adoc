...
The communication step size is denoted as latexmath:[hc_i = tc_{\mathit{i+1}} - tc_i]
...
\mathbf{x}_{\mathit{i+1}} = \Phi_i \left( \mathbf{x}_i \left\{ \mathbf{u}_i^{(j)} \right\}_{\mathit{j=0,\cdots,m_{ido}}}, \mathbf{p}_{\mathit{tune},\mathit{i}}, hc_i  \right)

\\

\left( \left\{ \mathbf{y}^{(j)}_{\mathit{i+1}} \right\}_{\mathit{j=0,\cdots,m_{odo}}}, \mathbf{w}_{\mathit{i+1}}\right) = \Gamma_i \left( \mathbf{x}_i, \left\{ \mathbf{u}^{(j)}_i \right\}_{\mathit{j=0,\cdots,m_{ido}}}, \mathbf{p}_{\mathit{tune}}, hc_i \right)

\end{Bmatrix}
++++

where latexmath:[\mathbf{\Phi}_i] and latexmath:[\mathbf{\Gamma}_i] define the system behavior for the time interval latexmath:[tc_i \leq t < tc_{\mathit{i+1}}],
...
_In this case, the following relationship should hold (note the use of_ latexmath:[\mathbf{x}_{\mathit{i+1}}] _here):_
...
** The current output variables latexmath:[\mathbf{y}_{\mathit{i+1}}^{(0)}]of the subsystem (same remark as above), along with some of their successive <<derivative,`derivatives`>> latexmath:[\left\{ \mathbf{y}_{\mathit{i+1}}^{(j)} \right\}_{\mathit{j=1,\cdots,m_{odo}}}](in case of continuous-time variables).

** Observation variables and <<calculated>> varying <<parameter,`parameters`>> latexmath:[\mathbf{w}_{\mathit{i+1}}], along with directional derivatives estimated at latexmath:[t = tc_{\mathit{i+1}}] (in case of continuous-time variables).
...
|[blue]#latexmath:[\begin{matrix} tc_{\mathit{i+1}} := tc_i + hc_i \\ (\mathbf{y}_{\mathit{c+d}}, \mathbf{y}_c^{(j)}, \mathbf{w}_{\mathit{c+d}}) := \mathbf{f}_{\mathit{doStep}}(\mathbf{u}_{\mathit{c+d}}, \mathbf{u}_{c}^{(j)}, tc_i, hc_i, \mathbf{p}_{\mathit{tune}}, \mathbf{p}_{\mathit{other}})_{\mathit{tc_i}} \\ tc_i := tc_{\mathit{i+1}} \end{matrix}]# +
...
Each FMU starts integration from communication point latexmath:[t_{i}] to reach the next communication point latexmath:[t_{\mathit{i+1}}].
Assuming an unexpected internal event is detected at latexmath:[t^{'}_{\mathit{i+1}}< t_{\mathit{i+1}}] during FMU~1~ integration, the master is informed of this early return.
...
* In the case of FMU~1~, the internal event time becomes the new latexmath:[t_{\mathit{i+1}}] time, i.e. this is the source of the event.
* In the case of FMU~2~, a complete rollback from latexmath:[t_{i}] to latexmath:[t^{'}_{\mathit{i+1}}] is necessary.
* In the case of FMU~3~, computation is immediately interrupted and only a partial rollback is necessary to reach latexmath:[t^{'}_{\mathit{i+1}}] time.
* In the case of FMU~4~, the current step integration has been interrupted at latexmath:[t^{'}_{\mathit{i+1}}] and no rollback is necessary.
