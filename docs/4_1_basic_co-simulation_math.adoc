...
_If the maste algorithm implements an extrapolation method of order_ latexmath:[m_{extra}] _, it  provides intermediate inputs of the form:_
...
++++
_where_ latexmath:[\mathbf{C}_{i, j}] _are coeffients that can be equal to output derivatives of a connectd FMU to realize a taylor polynomial._
