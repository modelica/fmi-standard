...
Both clock types can handle two different variants, the <<inputClock,`input clocks`>> and the <<output-clock,`output clocks`>>.
...
If <<output-clock,`output clocks`>> and <<inputClock,`input clocks`>> are defined it is possible to define a tick relationship from an <<output-clock>> to an <<inputClock>>.
This is needed to allow for an external resource management to achieve an optimal control of model partitions.
The <<output-clock,`output clocks`>> and the associated <<inputClock,`input clocks`>> define a union of <<clock,`clocks`>>.
In that sense, if an <<output-clock>> ticks the associated <<inputClock>> ticks at the same time instant in such a union.
It is only meaningful to combine one or multiple <<output-clock,`output clocks`>> with a single aperiodic <<inputClock>> in such a union.
