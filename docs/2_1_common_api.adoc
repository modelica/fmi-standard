...
_[These macros can be modified in a target specific variant of `"fmi2Functions.h"` to compile the shared library from the source code e.g. to simulate an FMU that has been exported on a different platform.]_
...
_[This can be realized in the case of a source code FMU with a target-specific version of `fmi2Functions.h` that modifies the macros.]_
