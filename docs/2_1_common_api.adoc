...
For this serialization of array variables the array is considered to be dense.
Hence, the dependencies of single elements, which can by inquired using `fmi3GetVariableDependencies`, is not taken into account by the serialization.
