...
_[Example:]_
_[A clocked PI controller can be defined as FMU and the input and output signal of the controller can be defined to be on the same `Inferred` clock. In such a case, the environment deduces the clock ticks from the components that are connected to the PI controller (e.g. a component connected to the input, or to the output may define the relevant clock).]_
