...
The arguments `toleranceDefined` and `tolerance` depend on the interface type:
...
The arguments `startTime` and `stopTime` can be used to check whether the model is valid within the given boundaries, or to allocate the necessary memory for storing results.
