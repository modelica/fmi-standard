...
The variables that can be retrieved by `fmi3Get{VariableType}` calls are (1) defined in the XML file as <<initial-unknown,initial unknowns>> and (2) variables with <<causality>> = <<output>>.
...
Discrete-time <<state,`states`>> are listed in the <<model-structure,model structure>>.
...
** *Initialization Mode*: The <<initial-unknown,initial unknowns>> that have a floating point type.

** *Continuous-Time Mode* (Model Exchange): The continuous-time outputs and state derivatives (= the variables with a floating point type, <<causality>> = <<output>>, and <<variability>> = <<continuous>> and the <<derivative,derivatives>>).

** *Event Mode* (Model Exchange): The same variables as in the *Continuous-Time Mode* and additionally variables with a floating point type, <<causality>> = <<output>>, and <<variability>> = <<discrete>>.

** *Step Mode* (Co-Simulation): The variables with a floating point type, <<causality>> = <<output>>, and <<variability>> = <<continuous>> or <<discrete>>.
Each state derivative variable with attribute <<derivative>>.
...
Details are given in the description of the <<model-structure,model structure>>.
...
. _The incidence information of the matrix (whether an element is zero or not zero) is extracted from the <<model-structure,model structure>>._
...
_[If the event indicator variable is an array variable and the `<Dimension>` element maps to a dependent variable, then this dependent variable is a dependency for the number of event indicators.]_
...
_[Each state derivative variable points to the corresponding continuous state variable with its <<derivative>> attribute._
