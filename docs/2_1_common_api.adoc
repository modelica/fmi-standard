...
[[intermediateVariablesGetRequired,`intermediateVariablesGetRequired`]]
* `intermediateVariablesGetRequired` is a vector of the value references of all variables that the master intends to get during intermediate updates.
This set can be empty (indicated by this argument being `NULL` and  <<nIntermediateVariablesGetRequired>> being zero) when the master does not intend to use intermediate variable read access at all.
...
[[nIntermediateVariablesGetRequired,`nIntermediateVariablesGetRequired`]]
* `nIntermediateVariablesGetRequired` gives the number of entries in <<intermediateVariablesGetRequired>>.
If there are no entries, this argument is zero, and <<intermediateVariablesGetRequired>> is `NULL`.

[[intermediateVariablesSetRequired,`intermediateVariablesSetRequired`]]
* `intermediateVariablesSetRequired` is a vector of the value references of all variables that the master intends to set during intermediate updates.
This set can be empty (indicated by this argument being `NULL` and  <<nIntermediateVariablesSetRequired>> being zero) when the master does not intend to use intermediate variable write access at all.
All variables referenced in this set must be marked with <<intermediateAccess,`intermediateAccess = true`>> in the model description.
...
[[nIntermediateVariablesSetRequired,`nIntermediateVariablesSetRequired`]]
* `nIntermediateVariablesSetRequired` gives the number of entries in <<intermediateVariablesSetRequired>>.
If there are no entries, this argument is zero, and <<intermediateVariablesSetRequired>> is `NULL`.
