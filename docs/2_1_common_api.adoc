...
An FMU C-file must include at the beginning a `define` of `FMI3_FUNCTION_PREFIX` with the same value as the value of the `modelIdentifier` attribute defined in `<fmiModelDescription><ModelExchange>`, `<fmiModelDescription><BasicCoSimulation>`, `<fmiModelDescription><HybridCoSimulation>`, `<fmiModelDescription><ScheduledCoSimulation>` together with `pass:[_]` at the end (see <<ModelExchange>>, <<fmi-for-basic-co-simulation>>, <<fmi-for-hybrid-co-simulation>>, <<fmi-for-scheduled-co-simulation>>).
...
These initialization functions must be called successfully before any of the following functions can be called.
...
The `modelDescription` has to include a `ModelExchange` element to allow calling `fmi3InstantiateModelExchange`.

`fmi3InstantiateBasicCoSimulation`::
Black box interface for Basic Co-Simulation (see <<fmi-for-basic-co-simulation>>).
The `modelDescription` has to include a `BasicCoSimulation` element allow calling `fmi3InstantiateBasicCoSimulation`.

`fmi3InstantiateHybridCoSimulation`::
Black box interface for Basic Co-Simulation (see <<fmi-for-hybrid-co-simulation>>).
The `modelDescription` has to include a `HybridCoSimulation` element allow calling `fmi3InstantiateHybridCoSimulation`.

`fmi3InstantiateScheduledCoSimulation`::
Black box interface for Basic Co-Simulation (see <<fmi-for-scheduled-co-simulation>>).
The `modelDescription` has to include a `ScheduledCoSimulation` element allow calling `fmi3InstantiateScheduledCoSimulation`.
...
If attribute `canNotUseMemoryManagementFunctions = true` in `<fmiModelDescription><ModelExchange / BasicCoSimulation/ HybridCoSimulation/ ScheduledCoSimulation>`, then function <<allocateMemory>> is not used in the FMU and a null pointer can be provided.
...
If attribute `canNotUseMemoryManagementFunctions = true` in `<fmiModelDescription><ModelExchange / BasicCoSimulation/ HybridCoSimulation/ ScheduledCoSimulation>`, then function <<freeMemory>> is not used in the FMU and a null pointer can be provided.
...
These functions are only supported by the FMU, if the optional capability flag `canGetAndSetFMUState` in `<fmiModelDescription> <ModelExchange / BasicCoSimulation / HybridCoSimulation / ScheduledCoSimulation>` in the XML file is explicitly set to `true` (see <<ModelExchange>>, <<fmi-for-basic-co-simulation>>, <<fmi-for-hybrid-co-simulation>>, <<fmi-for-scheduled-co-simulation>>).
...
These functions are only supported by the FMU, if the optional capability flags `canGetAndSetFMUState` and `canSerializeFMUState` in `<fmiModelDescription><ModelExchange / BasicCoSimulation / HybridCoSimulation / ScheduledCoSimulation>` in the XML file are explicitly set to `true` (see <<ModelExchange>>, <<fmi-for-basic-co-simulation>>, <<fmi-for-hybrid-co-simulation>>, <<fmi-for-scheduled-co-simulation>>).
