...
A function that is defined as `fmi3GetInt8` is changed by the macros to a function name as follows:

- If the FMU is shipped with C source code or with static link library: +
The constructed function name is `MyModel_fmi3GetInt8`.
...
The constructed function name is `fmi3GetInt8`, in other words, it is not changed.
...
If a function in the following sections is called with a wrong `fmi3ValueReference` value _[for example, setting a constant with a `fmi3SetInt8` function call]_, then the function has to return with an error ( `fmi3Status = fmi3Error`, see <<status-returned-by-functions>>).
...
* latexmath:[\color{blue}{\mathbf{v}_{unknown}}] is the vector of unknown variables computed in the actual mode:

** *Initialization Mode*: The exposed unknowns listed under `<ModelStructure><InitialUnknowns>` that have a float type.

** *Continuous-Time Mode* (Model Exchange): The continuous-time outputs and state derivatives (= the variables listed under `<ModelStructure><Outputs>` with a float type and <<variability>> = <<continuous>> and the variables listed as state derivatives under `<ModelStructure><Derivatives>`).

** *Event Mode* (Model Exchange): The same variables as in the *Continuous-Time Mode* and additionally variables under `<ModelStructure><Outputs>` with a float type and <<variability>> = <<discrete>>.

** *Step Mode* (Co-Simulation): The variables listed under `<ModelStructure><Outputs>` with a float type and <<variability>> = <<continuous>> or <<discrete>>.
If `<ModelStructure><Derivatives>` is present, also the variables listed here as state derivatives.

* latexmath:[\color{blue}{\mathbf{v}_{known}}] is the vector of <<input>> variables of function *h* that changes its value in the actual mode.
...
* latexmath:[\color{blue}{\mathbf{v}_{rest}}] is the set of <<input>> variables of function *h* that either changes its value in the actual mode but are non-float variables, or do not change their values in this mode, but change their values in other modes _[for example, discrete-time <<input,`inputs`>> in *Continuous-Time Mode*]_.
