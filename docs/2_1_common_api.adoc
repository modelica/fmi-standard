...
There are two access functions:
...
_Then one can get this with one function call of <<fmi3GetAdjointDerivative>> (with arguments_ latexmath:[v_{unknown} = \left \{ y_1, y_2 \right \},  v_{known} = \left \{ u_1, u_2 \right \}, \Delta v_{unknown} = \left( w_1 w_2 \right)^T] _), while with <<fmi3GetDirectionalDerivative>> at least two calls would be necessary to first construct the Jacobian column-wise and then multiplying from the right with_ latexmath:[\left( w_1  w_2 \right)^T] _]_
