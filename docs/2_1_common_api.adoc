...
Directional derivative for higher dimension variables are almost teated in the same way. 
Consider, for example, an FMU which multiplies its 2x2 input by a 3x2 constnat gain latexmath:[\color{blue}{Y=K U}] , i.e., where 
...
\end{bmatrix}
++++
The output is a matrix of size 3x2. 
The directional derivative of an output element latexmath:[\color{blue}{Y(i,j)}] with respect to the input is: 

latexmath:[\color{blue}{\Delta Y(i,j) = 
...
++++
 ...
 So in order to retrieve the directional derivative of Y with respect to U(2,1) this command should be used
 `fmi3GetDirectionalDerivative(m, vr_Y, 1, vr_U, 1, {0,0,1.0,0}, 4, dd, 6);`
 where vr_Y and vr_U are references of the variable Y and U respectively. 
 Note that in order to retrive the directional derivative of Y with respect to U(2,1), the seed value {0, 0, 1.0, 0} has been used.
 The output dd is a matrix of size 3x2, so  nDvUnknown is 6.
