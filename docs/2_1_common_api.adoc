...
Argument `fmuGUID` can be used by the FMU to check that the `modelDescription.xml` file (see section 2.3) is compatible with the implementation of the FMU.
It is an opaque string generated by the FMU exporter that is stored in the XML file in the mandatory attribute `guid` (see section 2.2.1).
It must be passed unchanged to the FMU and must not be `NULL`.
