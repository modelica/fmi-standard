...
_and latexmath:[\left( v_1  v_2 \right) \cdot \mathbf{ \frac{\partial h}{\partial u} }] for some vector latexmath:[\left( v_1  v_2 \right)^T] is needed._
_Then one can get this with one function call of <<fmi3GetAdjointDerivative>> (in other words, latexmath:[v_{unknown} = \left \{ y_1, y_2 \right \},  v_{known} = \left \{ u_1, u_2 \right \}, \Delta v_{unknown} = \left( v_1 v_2 \right)^T] ), while with <<fmi3GetDirectionalDerivative>> at least two calls would be necessary. ]_
