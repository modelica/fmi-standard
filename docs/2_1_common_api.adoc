...
* Argument `status` contains the severity of the message.
If <<logMessage>> is called with `status = fmi3OK`, then the message is a pure information message.

* Argument `category` is the category of the message.
...
* Argument vector `categories` has `nCategories` elements.
...
* Arguments `startTime` and `stopTime` can be used to check whether the model is valid within the given boundaries or to allocate memory which is necessary for storing results.
