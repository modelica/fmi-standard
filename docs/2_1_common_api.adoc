...
is a unique identifier for the FMU instance.
...
===== Argument fmuType
defines the type of the FMU:

====== Value fmi3ModelExchange
[[fmi3ModelExchange,`fmi3ModelExchange`]]
...
====== Value fmi3CoSimulation
[[fmi3CoSimulation,`fmi3CoSimulation`]]
...
===== Argument fmuInstantiationToken
can be used by the FMU to check that the `modelDescription.xml` file (see <<fmu-distribution>>) is compatible with the implementation of the FMU.
...
===== Argument fmuResourceLocation
is a URI according to the http://datatracker.ietf.org/doc/rfc3986/[IETF RFC3986] syntax to indicate the location to the `resources` directory of the unzipped FMU archive.
...
===== Argument functions
provides callback functions to be used from the FMU functions to utilize resources from the environment (see type <<fmi3CallbackFunctions>> below).

===== Argument visible
If `visible = fmi3False` defines that the interaction with the user should be reduced to a minimum (no application window, no plotting, no animation, etc.).
...
===== Argument fmuCoSimulationConfiguration
is used to configure Co-Simulation features used by the Co-Simulation master.
...
===== Function pointer allocateMemory
...
===== Function pointer intermediateUpdate
...
===== Function pointers lockPreemption and unlockPreemption
