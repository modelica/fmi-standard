...
Variables _uj_ are the (external or local) inputs and _yj_ are the (external or local) outputs of a clocked partition. 
...
_[Modelica uses this mode for the first tick in clocked continuous equations. This mode can also be requested by the calling environment with `fmi3SetClock()`, e.g. to form partial derivatives ∂yj/∂xj-1.]_
...
Such clocks are called Communication Point Clocks (CPClocks). 
...
Either triggered or inferred, a periodic clock ticks at equidistant sample time points that are known a priori (defined in modelDescription.xml) or are alternatively determined by the environment in case of inferred clocks. Mathematically, the next clock tick at time instant ti is defined as:
...
The following functions can be used to get or set this interval either as a rational or real number:

[source, C]
----
include::../headers/fmi3FunctionTypes.h[tags=SetInterval]
----

[source, C]
----
include::../headers/fmi3FunctionTypes.h[tags=GetInterval]
----

The interval ∆T and toffset must be defined for periodic clocks in the xml-file. 
In case of periodic inferred clocks these values are the start interval values if they are not given by the simulation master _[this information can be used for emulating periodic inferred clocks in co-simulation mode `fmi3ModeCoSimulation`].

If ∆T is fixed for an FMU and cannot be changed it is a strictly periodic clock. 
The clock is periodic, if ∆T can be defined before calling fmi3EnterInitializationMode via `fmi3SetIntervalDecimal()` or `fmi3SetIntervalFraction()`.
The clock is aperiodic if ∆T is not constant during a simulation run (`fmi3SetIntervalDecimal()` or `fmi3SetIntervalFraction()` can be called for that clock during event mode).
...
The triggered clocks and the associtated inferred clocks define a union of clocks. 
In that sense, if a triggered clock ticks the associated inferred clock ticks at the same time instant in such a union. 
It is only meaningful to combine triggered clocks with a aperiodic inferred clock in such a union.
...
A clock union can be defined based on the `clockReference` attribute of inferred clock variables.
...
It is required for an FMU to directly internally set back the activation state of a triggered clock _i_ to `fmi3False` if the function `fmi3GetClock()` is called for a clock _i_ and the co-simulation mode is set to `fmi3ModeScheduledExecutionSimulation`_[this is required to allow preemption]_. 
See 4.4.5 
