...
It is NULL for all other fmuTypes (i.e. `fmi3ModelExchange`).
...
This callback allows internal events of different types (e.g. associated to triggered clock ticks or intermediate variable access) to be signaled from an FMU to the co-simulation master. 
...
Function `startPreemptionLock` and `stopPreemptionLock`::
To provide more options to secure the code against unwanted preemption these callback functions are defined.
They can be used to prevent preemption for certain code parts enclosed by `startPreemptionLock` and `stopPreemptionLock` in co-simulation mode `fmi3ModeScheduledExecutionSimulation`. 
Even if the co-simulation master does not support preemption, at least an empty implementation of these callback functions is required to allow the reuse of code and an efficient preemption.
...
This change proposal offers two types of clocks, desinged to address similar use cases with differences in details. 
One clock type (Synchrounous Clocks) complies with the limitations imposed by the synchronous clock theory, the other clock type (Communication Point Clocks) is a general purpose co-simulation clock for providing suitable communication points for the timed evaluation of model partitions. 
Please note that therefore the term clock and clocked is used in both cases but has a more general meaning in this FCP. 
Both clock types can handle two different variants, the *inferred clocks* and the *triggered clocks*. First the two different clock types are presented with the mathematical definition of their use cases. 
...
Variables _xj_ are called `states of a clocked partition`, or `discrete-time states` and _j_ is the _j+1th_ tick of the associated clock. 
Variables uj are the (external or local) inputs and yj are the (external or local) outputs of a clocked partition. 
A discrete-time state can be from any type of a Variable (with exception of a `Clock`) such as of type Real or Boolean. 
...
Such clocks are called CommunicationPointClocks (CPClocks). 
...
Furthermore the periodic clock tick is described which is used by both variants.

*Triggered Clocks*
A triggered clock is a general clock that ticks when a time-, state- or step-event occurs in the continuous-model partition of the FMU (for CPClocks these triggered clock tick events can occour in all model partitions) and are identified based on their clockReference (see 2.2.3). 
It can be periodic or non-periodic. [Example: If `b = x > 0`, and a state event is defined when b changes from false to true, and b is defined as triggered clock, then this clock is active whenever x changes from negative to positive values.]

Since triggered clocks are ticking based on model internal information it is required in case of FMI for Co-Simulation to signal the ticking of a triggered clock to the simulation environment during a _fmi3DoStep()_ computation. 
In most cases the signaling of a triggered clock tick is a `strong event` that requires creating a communication point for exchanging additional information (i.e. variable values) with the simulation environment. Additional cases need to be handled if a clock tick is not associated to an event that requires the creation of a communication point for the model partition that generated the event.
...
*Inferred Clocks*
An inferred clock is a periodic or non-periodic clock that is defined directly or indirectly by a clock outside of the FMU. 
...
//EDITQ: Periodic Clock Ticks on the same text level (i.e. same text formatting) as Inferred Clocks and Triggered Clocks is confusing. 
//EDITQ: It should not look like a new type of clock but as a derivate of the two primitive clock types

*Periodic Clock Ticks*
Either triggered or inferred, a periodic clock ticks at equidistant sample time points that are known a priori (defined in modelDescription.xml) or are alternatively determined by environment in case of inferred clocks. Mathematically, the next clock tick at time instant ti is defined as:
...
|stem:[\Delta T>0]
|The constant time interval from the previous clock tick to the current clock tick. It is either defined as a real number (xml-attribute _interval_ or API function `fmi3GetIntervalDecimal()`) or as a rational number (xml-attributes _intervalCounter_ and _resolution_  or API function `fmi3GetIntervalFraction()`).
|===

The interval ∆T and toffset for periodic clocks must be defined in the xml-file.

If ∆T is fixed for an FMU and cannot be changed it is a strictly periodic clock. 
The clock is periodic, if ∆T can be defined before calling fmi3EnterInitializationMode. 
The clock is aperiodic if ∆T is not constant during a simulation run (`fmi3SetIntervalDecimal()` or `fmi3SetIntervalFraction()` can be called during event mode).

*Clock Relationships for Communication Point Clocks*
If triggered clocks and inferred clocks are defined it is possible to define a tick relationship from a triggered clock to an inferred clock. 
This is needed to allow for an external resource management to achieve an optimal control of model partitions. 
If a triggered clock ticks the associated inferred clock ticks at the same time instant. 
In that sense the triggered clocks and the associtated inferred clocks define a union of clocks. 
It is only meaningful to combine triggered clocks with an aperiodic inferred clock in that union.
...
[Example: Elementary blocks like a PI controller will have all variables on an inferred clock with identical properties. 
