...
Since <<outputClock,`output clocks`>> are ticking based on model internal information it is required in case of FMI for Co-Simulation to signal the ticking of an output <<clock>> to the simulation environment during a _fmi3DoStep()_ computation.
...
If <<outputClock,`output clocks`>> and <<inputClock,`input clocks`>> are defined it is possible to define a tick relationship from an output <<clock>> to an <<inputClock>>.
