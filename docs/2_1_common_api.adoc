...
Each <<inputClock,`input clock`>> may be contained in at most one clock union _[naturally limited by the single <<triggeredBy>> attribute]_.
...
The <<outputClock>> may depend on other unknowns.
