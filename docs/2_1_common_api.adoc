...
So in order to get the directional derivative of latexmath:[{Y}] with respect to latexmath:[{U(2,1)}] this command should be used: `fmi3GetDirectionalDerivative(m, vr_Y, 1, vr_U, 1, {0.0, 0.0, 1.0, 0.0}, 4, dd, 6)` where `vr_Y` and `vr_U` are references of the variable latexmath:[{Y}] and latexmath:[{U}], respectively.
