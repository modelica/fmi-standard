...
[latexmath]
++++
A = \left( \begin{array}{cc} a_{\mathit{11}}&a_{\mathit{12}}\\
                             a_{\mathit{21}}&a_{\mathit{22}}\\
                             a_{\mathit{31}}&a_{\mathit{32}}\\
            \end{array} \right)
++++
...
++++
x_j=\begin{cases}
    x_{\mathit{j-1}} & \text{if subactive}\\
    f_j(x_{\mathit{j-1}},u_j,t_j) & \text{else}
    \end{cases}
++++

[latexmath]
++++
y_j=g_j(x_{\mathit{j-1}},u_j,t_j);
++++
...
_This mode can also be requested by the calling environment with <<fmi3SetClock>>, e.g. to form partial derivatives_ latexmath:[\partial y_j / \partial x_{\mathit{j-1}}] _.]_
...
t_i &:= t_{\mathit{i-1}} + \Delta T, i = 1,2,3,{...}
\end{align*}
++++
...
=
\begin{bmatrix}
\frac{\partial h_1}{\partial v_{\mathit{known},\mathit{1}}} & \cdots & \frac{\partial h_1}{\partial v_{\mathit{known},\mathit{n}}} \\
\vdots & \ddots & \vdots \\
\frac{\partial h_m}{\partial v_{\mathit{known},\mathit{1}}} & \cdots & \frac{\partial h_m}{\partial v_{\mathit{known},\mathit{n}}}
\end{bmatrix}
++++
...
_Then one can get this with one function call of <<fmi3GetAdjointDerivative>> (with arguments_ latexmath:[\mathbf{v}_{\mathit{unknown}} = \text{valueReferences of} \left \{ y_1, y_2 \right \},  \mathbf{v}_{\mathit{knwon}} = \text{valueReferences of} \left \{ u_1, u_2 \right \},  \mathbf{v}_{\mathit{seed}} = \left( w_1, w_2 \right)^T] _), while with <<fmi3GetDirectionalDerivative>> at least two calls would be necessary to first construct the Jacobian column-wise and then multiplying from the right with_ latexmath:[\left( w_1,  w_2 \right)^T] _._
