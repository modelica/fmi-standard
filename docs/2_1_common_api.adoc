...
An FMU C-file must include at the beginning a `define` of `FMI3_FUNCTION_PREFIX` as the `modelIdentifier` attribute defined in `<fmiModelDescription><ModelExchange>` or `<fmiModelDescription><CoSimulation>`together with `pass:[_]` at the end (see sections <<ModelExchange>> and <<schema-co-simulation>>).
...
There are two access functions provided: one to compute the directional derivatives, and another to calculate the adjoint derivatives.
...
* latexmath:[\color{blue}{\mathbf{v}_{unknown}}] is the vector of unknown Real variables computed in the actual state:
...
* latexmath:[\color{blue}{\mathbf{v}_{known}}] is the vector of Real <<input>> variables of function *h* that changes its value in the actual state.
...
* latexmath:[\color{blue}{\mathbf{v}_{rest}}] is the set of <<input>> variables of function *h* that either changes its value in the actual state but are non-Real variables, or do not change their values in this state, but change their values in other states _[for example, discrete-time <<input,`inputs`>> in *Continuous-Time Mode*]_.

If the capability attribute `providesDirectionalDerivative` is `true`, <<fmi3GetDirectionalDerivative>> computes a linear combination of the partial derivatives of *h* with respect to all elements of the selected <<input>> variables latexmath:[\color{blue}{\mathbf{v}_{known}}]:
...
The directional derivative of an output element latexmath:[{Y(i,j)}] with respect to the input latexmath:[{U}]) is:
...
The retrieved directional derivative `dd` is stored in a matrix of size 3x2, so `nDvUnknown` is 6.

===== Adjoint derivatives

If the capability attribute `providesAdjointDerivative = true`, <<fmi3GetAdjointDerivative>> computes the adjoint deriatives of an FMU.
...
Using the same notation as for the directional derivatives for the different states in which the FMU can be, the adjoint derivative is

[latexmath]
++++
\mathbf{v}_{sens}^T = \mathbf{v}_{seed}^T \cdot \mathbf{J}
++++
...
=
\begin{bmatrix}
\frac{\partial u_1}{\partial k_1} & \cdots & \frac{\partial u_1}{\partial k_n} \\
\vdots & \ddots & \vdots \\
\frac{\partial u_m}{\partial k_1} & \cdots & \frac{\partial u_m}{\partial k_n}
\end{bmatrix}
++++

where latexmath:[\mathbf{u}] are the latexmath:[m] unknowns and latexmath:[\mathbf{k}] are the latexmath:[n] knowns.
...
Note that array variables will be serialized, so `nSeed` is only equal to `nUnknowns`, if all value references of `unknowns` point to scalar variables.
Likewise `nSensitivity` is only equal to `nKnowns` if all value references of `knowns` point to scalar variables.
...
_Then one can get this with one function call of <<fmi3GetAdjointDerivative>> (with arguements latexmath:[v_{unknown} = \left \{ y_1, y_2 \right \},  v_{known} = \left \{ u_1, u_2 \right \}, \Delta v_{unknown} = \left( w_1 w_2 \right)^T] ), while with <<fmi3GetDirectionalDerivative>> at least two calls would be necessary to first construct the Jacobian column-wise and then multiplying from the right with latexmath:[\left( w_1  w_2 \right)^T] ]_
