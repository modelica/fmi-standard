...
In Co-Simulation, time advances in variable steps negotiated between the co-simulation algorithm of the importer and the FMU.
...
With the mechanisms of the callback <<fmi3CallbackIntermediateUpdate>> and/or <<earlyReturn>>, both importer and FMU can determine if such a step-size reduction is required.
The output argument <<lastSuccessfulTime>> of <<fmi3DoStep>> allows the FMU to signal the master its current internal time.
...
However, the time itself is not communicated to the FMU nor its partitions.
It is assumed that model partitions will keep an activation count to estimate time with sufficient accuracy.
