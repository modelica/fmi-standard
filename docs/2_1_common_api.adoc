...
`fmi3PlatformTypes.h`::
contains the type definitions of the input and output parameters of the functions as well as some C preprocessor macro definitions for constants.
...
The parameters of the instantiation functions are detailed as follows:

* Parameter `instanceName` is a unique identifier for the FMU instance.
...
* Parameter `instantiationToken` can be used by the FMU to check that the `modelDescription.xml` file (see <<fmu-distribution>>) is compatible with the implementation of the FMU.
...
* Parameter `resourceLocation` is a URI according to the http://datatracker.ietf.org/doc/rfc3986/[IETF RFC3986] syntax to indicate the location to the `resources` directory of the unzipped FMU archive.
...
* If argument `loggingOn = fmi3False`, then any logging is disabled and the logger function is not called by the FMU.
If argument `loggingOn = fmi3True`, the FMU enables a vendor defined set of `<LogCategories>`.
...
* Parameter `instanceEnvironment` is a pointer that can be passed to the <<fmi3CallbackIntermediateUpdate>> function in order that the simulation environment can provide an efficient way to identify the slave that called <<fmi3CallbackIntermediateUpdate>>.

* Parameter `intermediateVariableGetRequired` TODO explanation required

* Parameter `intermediateInternalVariableGetRequired` TODO explanation required

* Parameter `intermediateVariableSetRequired` TODO explanation required

The parameters below are function pointers provided by the simulation environment to be used by the FMU.
...
* Parameter `instanceName` is the instance name of the model that calls this function.
// TODO fix following sentence
* Parameter `status` contains the severity of the message.
If <<logMessage>> is called with `status = fmi3OK`, then the message is a pure information message.

* Parameter `category` is the category of the message.
...
* Parameter `message` is a string that contains the message.
...
[source, C]
----
include::../headers/fmi3FunctionTypes.h[tags=CallbackAllocateMemory]
----

Pointer to a function that is called in the FMU if memory needs to be allocated.
If attribute `canNotUseMemoryManagementFunctions = true` in `<fmiModelDescription><ModelExchange|{Basic|Hybrid|Scheduled}CoSimulation>`, then function <<allocateMemory>> is not used in the FMU and a null pointer can be provided.
...
_[A simple implementation is to use `calloc` from the C standard library.]_

[[freeMemory,`freeMemory`]]
Function `freeMemory`::
[source, C]
----
include::../headers/fmi3FunctionTypes.h[tags=CallbackFreeMemory]
----

Pointer to a function that must be called in the FMU if memory is freed that has been allocated with <<allocateMemory>>.
If attribute `canNotUseMemoryManagementFunctions = true` in `<fmiModelDescription><ModelExchange|{Basic|Hybrid|Scheduled}CoSimulation>`, then function <<freeMemory>> is not used in the FMU and a null pointer can be provided.
If a null pointer is provided as input argument `obj`, the function shall perform no action.
_[A simple implementation is to use `free` from the C standard library; in ANSI C89 and C99, the null pointer handling is identical as defined here.]_
...
* Parameter `intermediateUpdateTime` indicates the internal time of the FMU at the time of calling the callback function.

* Argument `eventOccurred = fmi3True` means that an internal event has occurred.

* Argument `clocksTicked = fmi3True` means that an FMU clock has ticked.

* Argument `intermediateVariableSetAllowed = fmi3True` means the master algorithm is allowed to set FMU variables using `fmi3Set{VariableType}`.

* Argument `intermediateVariableGetAllowed = fmi3True` means the master algorithm is allowed to get FMU variables using `fmi3Get{VariableType}`.

* Argument `intermediateStepFinished = fmi3True` that an internal step was finished.

* Argument `canReturnEarly = fmi3True` means the master is allowed to ask the FMU to return early from a <<fmi3DoStep>>.
...
If a null pointer is provided for parameter `instance`, the function call is ignored (does not have an effect).
...
* Parameter `nCategories` defines the length of the next parameter `categories`
If `loggingOn = fmi3True` and `nCategories = 0`, then all debug messages shall be output.
If `loggingOn = fmi3True` and `nCategories > 0`, then only debug messages according to the `categories` parameter shall be printed via the <<logMessage>> function.

* Parameter vector `categories` has `nCategories` elements.
...
This function call informs the FMU to setup the experiment.
This function must be called after <<fmi3Instantiate>> and before <<fmi3EnterInitializationMode>> is called.

* Parameters `toleranceDefined` and `tolerance` depend on the FMI interface type:
...
An FMU for Co-Simulation might ignore this parameter.

* Parameters `startTime` and `stopTime` can be used to check whether the model is valid within the given boundaries, or to allocate the necessary memory for storing results.
Parameter `startTime` is the <<fixed>> <<initial>> value of the <<independent>> variable.
+
...
* If argument `stopTimeDefined = fmi3True`, then `stopTime` is the final value of the <<independent>> variable.
If the environment tries to compute past `stopTime`, the FMU has to return `fmi3Status = fmi3Error`.
If `stopTimeDefined = fmi3False`, then no final value of the <<independent>> variable is defined and parameter `stopTime` is meaningless.
+
...
* Parameter `valueReferences` is a vector of `nValueReferences` value handles that define the variables that shall be inquired.

* Parameter `values` is a vector with the actual values of these variables.

* Parameter `sizes` is a vector with the actual sizes of the values for binary variables.

* Parameter `nValues` provides the number of values in the `values` vector (and `sizes` vector, where applicable) which is only equal to `nValueReferences` if all <<valueReference>>pass:[s] point to scalar variables.
...
* Parameter `valueReferences` is a vector of `nValueReferences` value handles that define the variables that shall be set.

* Parameter `values` is a vector with the actual values of these variables.

* Parameter `sizes` is a vector with the actual sizes of the values of binary variables.
...
Communication Point Clocks, introduced here, are used only in FMI for Basic Co-Simulation to define the communication points of model partitions (defined below).
...
An <<outputClock>> is a general <<clock>> that ticks when a time-, state- or step-event occurs in the continuous-model partition of the FMU (for Communication Point Clocks these <<outputClock>> tick events can occur in all model partitions) and are identified based on their <<clockReference>> (see <<operation-on-clocks>>).
The events can be periodic or non-periodic.
...
An <<inputClock>> is a periodic or non-periodic <<clock>> that is defined by a <<clock>> outside of the FMU.
...
A <<clock>> is strictly periodic if latexmath:[\Delta T] is fixed for an FMU and cannot be changed.
...
_[For real-time computation use cases i.e. in Scheduled Co-Simulation the priority information is used also for task preemption configurations.
...
* Parameter `valueReferences[]` points to an array of value references of clocks to be activated or deactivated at the current time instant depending on

* parameter `values[]` content: if the nth element is `fmi3ClockActive` the nth clock is activated, otherwise deactivated.

* Parameter `nValueReferences` indicates the size of the parallel arrays `valueReferences[]` and `values[]` (and potentially `subactive[]`).

* Parameter `subactive[]` requires evaluation of a clocked partition in subactive mode (only output equations) if the argument subactive is non NULL and `subactive[i] = fmi3True`. `subactive[i]` has no meaning for Communication Point Clocks and is ignored for such <<clock,`clocks`>>.
...
* Parameter `valueReferences[]` contains a list of value references to <<clock,`clocks`>> of the size indicated by

* parameter `nValueReferences`.

* Parameter `values[]` will return the activation status at the current time instant of the <<clock,`clocks`>> referenced by `valueReferences[]`.
...
The parameter to this function is the `FMUState` to be freed.
If a null pointer is provided, the call is ignored.
The function returns a null pointer in parameter `FMUState`.
...
* Parameter `unknowns[]` contains value references to the unknowns.
* Parameter `nUnknowns` contains the length of parameter `unknowns[]`.
* Parameter `knowns[]` contains value references of the knowns.
* Parameter `nKnowns` contains the length of parameter `knowns[]`.
* Parameter `seed[]` contains the components of the seed vector.
* Parameter `nSeed` contains the length of `seed[]`.
* Parameter `sensitivity[]` contains the components of the sensitivity vector.
* Parameter `nSensitivity` contains the length of `sensitivity[]`.
...
* Parameter `nz` points to the `size_t` variable that will receive the number of event indicators.
...
* Parameter `valueReference` specifies the <<valueReference>> of the variable for which the number of <<dependencies>> should be returned.

* Parameter `nDependencies` points to the `size_t` variable that will receive the number of <<dependencies>>.
...
* Parameter `dependent` specifies the <<valueReference>> of the variable for which the dependencies should be returned.

* Parameter `nDependencies` specifies the number of dependencies that the calling environment allocated space for in the result buffers, and should correspond to value obtained by calling <<fmi3GetNumberOfVariableDependencies>>.

* Parameter `elementIndicesOfDependent` must point to a buffer of `size_t` values of size `nDependencies` allocated by the calling environment.
...
* Parameter `independents` must point to a buffer of `fmi3ValueReference` values of size `nDependencies` allocated by the calling environment.
...
* Parameter `elementIndicesIndependents` must point to a buffer of `size_t` values of size `nDependencies` allocated by the calling environment.
...
* Parameter `dependencyKinds` must point to a buffer of `fmi3DependencyKind` values of size `nDependencies` allocated by the calling environment.
