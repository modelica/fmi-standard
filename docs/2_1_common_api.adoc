...
Black box interface for Basic Co-Simulation (see <<fmi-for-hybrid-co-simulation>>).
...
Black box interface for Basic Co-Simulation (see <<fmi-for-scheduled-co-simulation>>).
...
* Argument `resourceLocation` is a URI according to the http://datatracker.ietf.org/doc/rfc3986/[IETF RFC3986] syntax to indicate the location to the `resources` directory of the unzipped FMU archive.
...
These following arguments are function pointers provided by the environment to be used by the FMU.
...
These callback functions have the following meaning:
...
* Argument `valueReferences` is a vector of `nValueReferences` value handles that define the clock variables that shall be set.

* Argument `values` is a vector with the actual activation values of the clock variables, `fmi3ClockActive` specifying that the clock is activated, otherwise it is deactivated.

* Argument `subactive` is a vector of booleans for each actual clock specifying whether evaluation of a clocked partition should occur in subactive mode (only output equations are evaluated) or not. This value has no meaning for Communication Point Clocks and is ignored for such <<clock,`clocks`>>.

* Argument `nValues` provides the number of values in the `values` and `subactive` vectors, which is only equal to `nValueReferences` if all <<valueReference>>pass:[s] point to scalar clock variables.
...
* Argument `valueReferences` is a vector of `nValueReferences` value handles that define the clock variables that shall be inquired.
...
* Argument `nValues` provides the number of values in the `values` vector, which is only equal to `nValueReferences` if all <<valueReference>>pass:[s] point to scalar clock variables.
