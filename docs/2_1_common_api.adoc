...
An FMU C-file must include at the beginning a `define` of `FMI3_FUNCTION_PREFIX` with the same value as the value of the `modelIdentifier` attribute defined in `<fmiModelDescription><ModelExchange>` or `<fmiModelDescription><CoSimulation>` together with `pass:[_]` at the end (see <<ModelExchange>> and <<schema-co-simulation>>).
...
An <<outputClock>> is a general <<clock>> that ticks when a time-, state- or step-event occurs in the continuous-model partition of the FMU (for CPClocks these <<outputClock>> tick events can occour in all model partitions) and are identified based on their <<clockReference>> (see <<operation-on-clocks>>).
...
* latexmath:[\mathbf{v}_{unknonwn}] is the vector of unknown Real variables computed in the actual state:
...
* latexmath:[\mathbf{v}_{known}] is the vector of known Real <<input>> variables of function *h* that changes its value in the actual state.
...
_and latexmath:[\left( w_1  w_2 \right) \cdot \mathbf{ \frac{\partial h}{\partial u} }] for some vector latexmath:[\left( w_1  w_2 \right)^T] is needed._
_Then one can get this with one function call of <<fmi3GetAdjointDerivative>> (with arguments_ latexmath:[\mathbf{v}_{unknown} = \left \{ y_1, y_2 \right \},  \mathbf{v}_{knwon} = \left \{ u_1, u_2 \right \},  \mathbf{v}_{seed} = \left( w_1 w_2 \right)^T] _), while with <<fmi3GetDirectionalDerivative>> at least two calls would be necessary to first construct the Jacobian column-wise and then multiplying from the right with_ latexmath:[\left( w_1  w_2 \right)^T] _]_
