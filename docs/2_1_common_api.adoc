...
Using this pointer, data from the <<modelDescription.xml>> file _[for example, mapping of <<valueReference, value references>> to variable names]_ can be transferred between the simulation environment and the <<logMessage>> function (see <<FMUStateSetable>>).
...
The FMU enters this state when the FMU calls <<fmi3CallbackIntermediateUpdate>> in state *Step Mode* (CS) or in state *Clock Activation Mode* (SE).
When the callback function returns, the FMU returns to *Step Mode* (CS) or to *Clock Activation Mode* (SE).
...
The FMU enters this state by calling <<fmi3CallbackIntermediateUpdate>> within <<fmi3DoStep>> (CS) or <<fmi3ActivateModelPartition>> (SE) and leaves the state towards state *Step Mode* (CS) or *Clock Activation Mode* (SE) if the function returns <<fmi3OK>> or <<fmi3Warning>>.
...
The term <<clock>> is therefore used for both <<clock>> types.
...
The main use case of the synchronous clocks is to represent sampled data systems.
...
- The controller equations are executed synchronously whenever there is a new sample latexmath:[y_d]. This is implicit in the equations.
...
- Previous(xd) means the value of xd computed at the previous sample.

- y is a continuous variable, and u is piecewise constant.

- The importer may execute both controller and plan fmus at times other than sample times as part of the time integration of the plant. In this case, the controller must produce the zero order hold value u.
...
Now suppose the above architecture is realized with the ME FMUs as illustrated in <<figure-synchronous-clock-scenario>>.
Since there's no guarantee that the equations in Ctrl, Actuator, and Sensor FMUs will be invoked exactly at the times that coincide with the sampling times of the Sensor FMU (recall that it is the importer that picks the time advancement, based on the needs of the different FMUs), the Ctrl, Actuator, and Sensor FMUs need to be configured with the same sample rate. There are two problems associated with this approach:

- It is error-prone for more complex controller architectures; and

- represents time and the sample rate as real numbers, there are numerical difficulties in guaranteeing that all three fmus will execute exactly at the same wallclock time.

These are the problems that Synchronous Clocks solve.
...
Such clock would say, e.g., that the there is a tick every 100 ms, to represent the capabilities of the controller execution platform.

Getting back to the example in <<figure-synchronous-clock-scenario>>, the controller FMU may declare a clock that ticks every 100ms.
In order to inform the importer that the sensor and actuator FMUs are supposed to execute whenever the controller FMU executed, we need to connect that clock to those FMUs,
as shown in <<figure-synchronous-clock-scenario-clocked>>.
