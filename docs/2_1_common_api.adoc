...
If an `fmi3String` variable is passed as [underline]#input# argument to an FMI function and the FMU needs to use the string later,
...
If an `fmi3String` variable is passed as [underline]#output# argument from an FMI function and the string shall be used in the target environment,
...
- Argument `"value"` is a vector with the actual values of these variables.
- All strings passed as arguments to `fmi3etString` must be copied inside this function,
