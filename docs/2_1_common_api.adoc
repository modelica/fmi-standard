...
_[Note that all pointers passed into this function are only valid during the extent of the call.
This also applies to the struct pointer passed as the `functions` argument, even though the pointers to the callback functions themselves have a longer lifetime, as specified below. ]_
...
If the FMU does not define any log categories, `category` must be a null pointer.
...
each of size `size` or a null pointer, if the request cannot be satisfied.
