...
With the mechanisms of the callback <<fmi3CallbackIntermediateUpdate>> and <<earlyReturn>>, both importer and FMU can determine if such a step-size reduction is required.
_[TODO: how could a time-step be shorter without earlyReturn?]_
...
_[TODO: How could this be done with a zero crossing function? Wouldn't this be simple with a time-event?]_
...
_[TODO: The elements of the example are not FMUs. Hence we cannot talk about an importer]_
...
These are the variables that _[TODO:might?]_ change their value when the model partition is evaluated. Each variable in latexmath:[x] is associated with one, and only one, model partition.
...
An <<outputClock>> is a general <<clock>> that ticks when a time-, state- or step-event occurs in the continuous-model partition of the FMU (for <<CommunicationPointClocks,Communication Point Clocks>> these <<outputClock>> tick events can occur in all model partitions) and are identified based on their <<valueReference>>.
The events can be <<periodic>> or <<periodic,aperiodic>>.

_[TODO: "tick events" used but never defined]_
...
_[TODO: local clocks should be introduced here too, they are just used afterwards]_
...
An <<inputClock,input clocks>> must be independent to all other input clocks. _[TODO: what does that mean?]
...
_[TODO: Do we need this non-normative text at all?]_

===== Clock Unions [[clock-relationships-for-communication-point-clocks]]

It is possible to define a dependency from an <<outputClock>> to an <<inputClock>> of the same FMU using the <<triggeredBy>> attribute of <<inputClock>> variables.
...
_[TODO: ??? model partitions only belong to Communication point clocks, how can this be true??]_
...
_The ticks of a <<periodic>> <<input>> <<clock>> are given by the importer.]_
...
_[TODO: shouldn't we use constant and fixed like for other variable variabilities (strictly periodic = constant periodic; periodic = fixed periodic; aperiodic = tunable periodic!)_
_Does the importer have to call fmi3SetInterval for aperiodic clocks at each clock tick if the clock should tick with the same interval? Or will there be only one clock tick per fmi3SetInterval call?]_
