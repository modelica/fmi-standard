...
Advanced master implementations can try alternative approaches to drive the simulation by calling the function with different arguments or calling another function.
Otherwise the master has to treat this return code like <<fmi3Error>> and has to terminate the simulation.
...
** if the environment is not able to provide an URI to the resources folder _[e.g., if the environment does not have a file system. If the FMU in such a case cannot be sinmulated, as it depends on the resources folder, it shall terminate with an error.]_
...
[[intermediateVariableGetRequiredValueReferences,`intermediateVariableGetRequiredValueReferences`]]
* `intermediateVariableGetRequiredValueReferences` is a vector of the value references of all variables that the master intends to get during intermediate updates.
This set can be empty (indicated by this argument being `NULL` and  <<intermediateVariableGetRequiredNValueReferences>> being zero) when the master does not intend to use intermediate variable read access at all.
All variables referenced in this set must be marked with <<intermediateAccess,`intermediateAccess = true`>> in the model description.
It is an error if a master tries to access variables that have not been included in this set using `fmi3Get{VariableType}` during <<fmi3CallbackIntermediateUpdate>> callbacks.

[[intermediateVariableGetRequiredNValueReferences,`intermediateVariableGetRequiredNValueReferences`]]
* `intermediateVariableGetRequiredNValueReferences` gives the number of entries in <<intermediateVariableGetRequiredValueReferences>>.
If there are no entries, this argument is zero, and <<intermediateVariableGetRequiredValueReferences>> is `NULL`.

[[intermediateVariableSetRequiredValueReferences,`intermediateVariableSetRequiredValueReferences`]]
* `intermediateVariableSetRequiredValueReferences` is a vector of the value references of all variables that the master intends to set during intermediate updates.
This set can be empty (indicated by this argument being `NULL` and  <<intermediateVariableSetRequiredNValueReferences>> being zero) when the master does not intend to use intermediate variable write access at all.
All variables referenced in this set must be marked with <<intermediateAccess,`intermediateAccess = true`>> in the model description.
It is an error if a master tries to access variables that have not been included in this set using `fmi3Set{VariableType}` during <<fmi3CallbackIntermediateUpdate>> callbacks.

[[intermediateVariableSetRequiredNValueReferences,`intermediateVariableSetRequiredNValueReferences`]]
* `intermediateVariableSetRequiredNValueReferences` gives the number of entries in <<intermediateVariableSetRequiredValueReferences>>.
If there are no entries, this argument is zero, and <<intermediateVariableSetRequiredValueReferences>> is `NULL`.
...
This function switches off all initialization equations, and the FMU enters *Event Mode* for Model Exchange and Hybrid Co-Simulation, *Step Mode* for Basic Co-Simulation and *Clock Activation Mode* for Scheduled Co-Simulation.
