...
The standard header file as documented in this specification has version `"3.0.0"`, so this function returns `"3.0.0"`.
Importing tools supporting the FMI 3.0 standard, MUST be able to import FMUs with `fmi3Version`=`"3.0.x"` where x is a one- or mutliple- digit number.
...
* `subactive` is a vector of booleans for each actual clock specifying whether evaluation of a clocked partition should occur in subactive mode (only output equations are evaluated) or not. This value has no meaning for Communication Point Clocks and is ignored for such <<clock,`clocks`>>.
