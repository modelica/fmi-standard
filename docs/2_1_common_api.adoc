...
===== Adjoint derivatives derivatives

This function computes the adjoint deriatives of an FMU.

_[Adjoint derivatives are beneficial in several contexts_

* _in AI frameworks the adjoint derivatives are called "VJP"s meaning vector-gradient-products. There adjoint derivatives are used in the backpropagation process to do gradient-based optimization of parameters ussind Automatic Differentiation (AD)._
* _in pararameter estimaation (see <<BKF17>>)_
* _in contexts where the inside the FMU reverse mode automatic differentiation (AD) frameworks are used instead of forward mode AD that is better suitable for directional derivatives.]_
