...
_Using the standard-supplied version of `fmi2Functions.h`, the same effect can be achieved by defining the `FMI2_OVERRIDE_FUNCTION_PREFIX` precompiler macro prior to the inclusion of the `fmi2Functions.h` header, for example using precompiler command-line flags.]_
