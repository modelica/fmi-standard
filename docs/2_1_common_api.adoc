...
Note: In FMI 3.0, memory callback functions were removed, because their intended uses failed to materialize and the implementations often had issues.
It is now discuouraged to use the memory callback functions.
...
then only debug messages according to the `categories` argument shall be output.
