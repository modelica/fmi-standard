...
_[Adjoint derivatives are beneficial in several contexts_

* _in Artifical Intelligence (AI) frameworks the adjoint derivatives are called "VJP"s meaning vector-gradient-products. There adjoint derivatives are used in the backpropagation process to do gradient-based optimization of parameters using reverse mode automatic differentiation (AD), see, e.g., <<BPRS15>>._
...
_If one would like to construct the full Jacibian matrix, one can use either <<fmi3GetDirectionalDerivative>> (to column-wise construct it) or <<fmi3GetAdjointDerivative>> (to row-wise construct it, possibly improved with coloring methods as mentioned above)._
_However in the applications motivating the adjoint derivatives, one does not need the full Jacobian matrix latexmath:[\mathbf{J}], but vector  latexmath:[\mathbf{J}] mulitplied from the left to the Jacobian, i.e.  latexmath:[\mathbf{v^T J}] ]_
