...
_[Adjoint derivatives are beneficial in sever
al contexts_

* in AI frameworks the adjoint derivatives are called ?VJP?s meaning vector-gradient-products. There adjoint derivatives are used in the backpropagation process to do gradient-based optimization of parameters ussind Automatic Differentiation (AD).
* in pararameter estimaation (see Braun et. al 2017)
* in contexts where the inside the FMU reverse mode automatic differentiation (AD) frameworks are used instead of forward mode AD that is better suitable for directional derivatives.
]_
