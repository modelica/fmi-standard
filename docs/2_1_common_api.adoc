...
As `FMI3_FUNCTION_PREFIX` the `modelIdentifier` attribute defined in `<fmiModelDescription><ModelExchange>`, or `<fmiModelDescription><CoSimulation>` is used, together with `pass:[_]` at the end (see <<ModelExchange>> and <<fmi-for-co-simulation>>).
...
If `fmu3Type` is set to `fmi3HybridCoSimulation` or `fmi3ScheduledExecutionCoSimulation` or any of the intermediate variable access modes is set to `true`, then the <<fmi3CallbackIntermediateUpdate>> callback must be defined in <<fmi3CallbackFunctions>> in <<fmi3Instantiate>>.
...
They can be used to prevent preemption for certain code parts enclosed by `lockPreemption` and `unlockPreemption` in `fmi3ScheduledExecutionCoSimulation`.
...
Both clock types can handle two different variants, the <<input-clock,`input clocks`>> and the <<output-clock,`output clocks`>>.
...
An <<output-clock>> is a general <<clock>> that ticks when a time-, state- or step-event occurs in the continuous-model partition of the FMU (for CPClocks these <<output-clock>> tick events can occour in all model partitions) and are identified based on their <<clockReference>> (see 2.2.3).
...
Since <<output-clock,`output clocks`>> are ticking based on model internal information it is required in case of FMI for Co-Simulation to signal the ticking of an <<output-clock>> to the simulation environment during a `fmi3DoStep` computation.
In most cases the signaling of an <<output-clock>> tick is a `strong event` that requires creating a communication point for exchanging additional information (i.e. variable values) with the simulation environment.
...
_Example:_
_A clocked PI controller can be defined as an FMU and the input and output signal of the controller can be defined to be on the same <<input-clock>>._
...
Either <<output>> or <<input>>, a periodic <<clock>> ticks at equidistant sample time points that are known a priori (defined in `modelDescription.xml`) or are alternatively determined by the environment in case of <<input-clock,`input clocks`>>.
...
In case of periodic <<input-clock,`input clocks`>> these values are the start interval values if they are not given by the simulation master.
_[This information can be used for emulating periodic <<input-clock,`input clocks`>> in the interface `fmi3CoSimulation`]_.
...
If <<output-clock,`output clocks`>> and <<input-clock,`input clocks`>> are defined it is possible to define a tick relationship from an <<output-clock>> to an <<input-clock>>.
This is needed to allow for an external resource management to achieve an optimal control of model partitions.
The <<output-clock,`output clocks`>> and the associated <<input-clock,`input clocks`>> define a union of <<clock,`clocks`>>.
In that sense, if an <<output-clock>> ticks the associated <<input-clock>> ticks at the same time instant in such a union.
It is only meaningful to combine one or multiple <<output-clock,`output clocks`>> with a single aperiodic <<input-clock>> in such a union.
It is possible to define multiple unions of <<clock,`clocks`>>, with the maximum number defined by the number of available aperiodic <<input-clock,`input clocks`>>.
...
It is required for an FMU to directly internally set back the activation <<state>> of an <<output-clock>> latexmath:[i] to `fmi3False` if the function <<fmi3GetClock>> is called for a <<clock>> latexmath:[i] and the interface is set to `fmi3ScheduledExecutionCoSimulation`.
