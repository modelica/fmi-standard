...
* `requiredIntermediateVariables` is an array of the value references of all <<input>> variables that the master intends to set and all <<output>> variables it intends to get during intermediate updates.
This set may be empty (<<nRequiredIntermediateVariables>> == 0) when the master does not intend to use intermediate variable access.
...
Only the variables in <<requiredIntermediateVariables>> may be accessed by the master using `fmi3Set{VariableType}` and `fmi3Get{VariableType}` during <<fmi3CallbackIntermediateUpdate>> callbacks.
