...
_[Example: If_ latexmath:[b = x > 0] _, and a state event is defined when b changes from false to true, and b is defined as triggered clock, then this clock is active whenever x changes from negative to positive values.]_
...
In case of periodic inferred clocks these values are the start interval values if they are not given by the simulation master.
_[This information can be used for emulating periodic inferred clocks in co-simulation mode `fmi3ModeCoSimulation`]_.
...
It is required for an FMU to directly internally set back the activation state of a triggered clock latexmath:[i] to `fmi3False` if the function `fmi3GetClock()` is called for a clock latexmath:[i] and the co-simulation mode is set to `fmi3ModeScheduledExecutionSimulation`. _[This is required to allow preemption.]_
