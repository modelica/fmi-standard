...
* If `eventModeRequired == fmi3True` the master handles events, otherwise `fmi3EnterEventMode` must not not be called.
...
Since values of variables will be set at specific communication points by the importing tool, the FMU must make assumptions about the values between these communication points.

<<continuous, `Continuous`>> variables can be assumed to continue changing during these intervals according to smoothness and continuity assumptions (see <<recommendedIntermediateInputSmoothness>>).
If a <<continuous>> variable changes `discontinuously` (the smoothness assumption fails at this communication point), the co-simulation algorithm must raise an event to indicate the discrete change to the FMU.

<<discrete, `Discrete`>> variables keep their values between communication points changing their values.
Furthermore, changing <<discrete>> variables will likely require special handling within the FMU.
