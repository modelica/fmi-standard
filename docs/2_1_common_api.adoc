...
_[If only one FMU is simulated, as `instanceName` attribute `modelName` or `<ModelExchange|CoSimulation|ScheduledCoSimulation modelIdentifier="..">` from the XML schema `fmi3ModelDescription` might be used.]_
...
* If `eventModeRequired == fmi3True` the master handles events, otherwise `fmi3EnterEventMode` may not be called.
The flag may only be `fmi3True`, if `hasEventMode == true`.
...
This function switches off all initialization equations, and the FMU enters *Event Mode* for Model Exchange, *Step Mode* for Co-Simulation and *Clock Activation Mode* for Scheduled Co-Simulation.
...
Communication Point Clocks, introduced here, are used only in Co-Simulation and Scheduled Co-Simulation to define the communication points of model partitions (defined below).
...
These functions are only supported by the FMU, if the optional capability flag `canGetAndSetFMUState` in `<fmiModelDescription><ModelExchange|CoSimulation|ScheduledCoSimulation>` in the XML file is explicitly set to `true` (see <<ModelExchange>>, <<fmi-for-co-simulation>>, <<fmi-for-scheduled-co-simulation>>).
...
These functions are only supported by the FMU, if the optional capability flags `canGetAndSetFMUState` and `canSerializeFMUState` in `<fmiModelDescription><ModelExchange|CoSimulation|ScheduledCoSimulation>` in the XML file are explicitly set to `true` (see <<ModelExchange>>, <<fmi-for-co-simulation>>, <<fmi-for-scheduled-co-simulation>>).
