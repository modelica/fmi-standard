...
the header file (for example, `typedef float fmi3Float64` instead of `typedef double fmi3Float64`),
...
`fmi3Functions.h`::
contains the function prototypes of an FMU that can be accessed in simulation environments and that are defined in <<fmi-common-concepts>>, <<fmi-for-model-exchange>> and <<fmi-for-co-simulation>>.
+
...
_[Functions `fmi3SetXXX` are usually not performing calculations but just store the passed values in internal buffers._
_The actual calculation is performed by `fmi3GetXXX` functions._
_Still `fmi3SetXXX` functions could check whether the input arguments are in their validity range._
_If not, these functions could return with `fmi3Discard`.]_
(Model Exchange: `fmi3SetReal`, `fmi3SetInteger`, `fmi3SetBoolean`, `fmi3SetString`, <<fmi3SetContinuousStates>>, `fmi3GetReal`, <<fmi3GetDerivatives>>, <<fmi3GetContinuousStates>>, <<fmi3GetEventIndicators>>; Co-Simulation: `fmi3SetReal`, `fmi3SetInteger`, `fmi3SetBoolean`, `fmi3SetString`, <<fmi3DoStep>>, `fmi3GetXXXStatus`):
+
...
If `fmu3Type` is set to `fmi3HybridCoSimulation` or `fmi3ScheduledExecutionSimulation` or any of the intermediate variable access modes is set to `true`, then the <<fmi3CallbackIntermediateUpdate>> callback must be defined in <<fmi3CallbackFunctions>> in <<fmi3Instantiate>>.
...
They can be used to prevent preemption for certain code parts enclosed by `lockPreemption` and `unlockPreemption` in `fmi3ScheduledExecutionSimulation`.
...
Argument `startTime` is the <<fixed>> <<initial>> value of the <<independent>> variable _[(that is defined with <<causality>> = <<independent>> in the `fmiModelDescription.xml`]_). _[If the <<independent>> variable is `time`, `startTime` is the starting time of initializaton.]_
...
Before calling this function, all variables with attribute <<initial>> = <<exact>> or <<approx>> can be set with the `fmi3SetXXX` functions (the variable attributes are defined in the Model Description File, see <<definition-of-model-variables>>).
...
[latexmath]
++++
\mathbf{u} = \mathbf{h}(\mathbf{k}, \mathbf{v}_{rest}),
++++

where

* latexmath:[{\mathbf{u}}] is the vector of unknown Real variables computed in the actual state:
...
* latexmath:[{\mathbf{k}] is the vector of Real <<input>> variables of function *h* that changes its value in the actual state.
...
* latexmath:[{\mathbf{v}_{rest}}] is the set of <<input>> variables of function *h* that either changes its value in the actual state but are non-Real variables, or do not change their values in this state, but change their values in other states _[for example, discrete-time <<input,`inputs`>> in *Continuous-Time Mode*]_.
...
- <<fmi3GetDirectionalDerivative>> to compute the directional derivatives latexmath:[\mathbf{v}_{sens} = \mathbf{J} \cdot \mathbf{v}_{seed}], and

- <<fmi3GetAdjointDerivative>> to calculate the adjoint derivatives latexmath:[\mathbf{v}_{sens}^T = \mathbf{v}_{seed}^T \cdot \mathbf{J}]
...
=
\begin{bmatrix}
\frac{\partial h_1}{\partial k_1} & \cdots & \frac{\partial h_1}{\partial k_n} \\
\vdots & \ddots & \vdots \\
\frac{\partial h_m}{\partial k_1} & \cdots & \frac{\partial h_m}{\partial k_n}
\end{bmatrix}
++++

where latexmath:[\mathbf{u}] are the latexmath:[m] unknowns and latexmath:[\mathbf{k}] are the latexmath:[n] knowns, and latexmath:[\mathbf{h}] is the function to calculate the unknowns from the knowns.
...
(a) _with one wrapper function around function <<fmi3GetDirectionalDerivative>> to compute the directional derivatives with respect to these two variables (in other words, latexmath:[{v_{unknown} = y_1}], latexmath:[{v_{known} = \left \{ u_1, u_3 \right \}}]), and then the environment calls this wrapper function with latexmath:[{\Delta v_{known} = \left \{ 1, 0 \right \}}] to compute the partial derivative with respect to latexmath:[{u_1}] and latexmath:[{\Delta v_{known} = \left \{ 0, 1 \right \}}] to compute the partial derivative with respect to latexmath:[{u_3}], or_

(b) _with two direct function calls of <<fmi3GetDirectionalDerivative>> (in other words, latexmath:[{v_{unknown} = y_1, v_{known} = u_1, \Delta v_{known} = 1}]; and latexmath:[{v_{unknown} = y_1, v_{known} = u_3, \Delta v_{known} = 1}])._

_Note that a direct implementation of this function with analytic derivatives:_

(a) _Provides the directional derivative for all <<input>> variables; so in the above example: latexmath:[{\Delta y_1 = \frac{\delta g_1}{\delta x} \cdot \Delta x + \frac{\delta g_1}{\delta u_1} \cdot \Delta u_1 + \frac{\delta g_1}{\delta u_3} \cdot \Delta u_3 + \frac{\delta g_1}{\delta u_4} \cdot \Delta u_4}]_

(b) _Initializes all seed-values to zero; so in the above example: latexmath:[{\Delta x = \Delta u_1 = \Delta u_3 = \Delta u_4 = 0}]_

(c) _Computes the directional derivative with the seed-values provided in the function arguments; so in the above example: latexmath:[{\Delta v_{unknown} = \Delta y_1 (\Delta x = 0, \Delta u_1 = 1, \Delta u_3 = 1, \Delta u_4 = 0)}]]_

_[Note, function <<fmi3GetDirectionalDerivative>> can be utilized for the following purposes:_

- _Numerical integrators of stiff methods need matrix latexmath:[{\frac{\delta \mathbf{f}}{\delta \mathbf{x}}}]._
...
- _If the FMU is used as the model for an extended Kalman filter, latexmath:[{\frac{\delta \mathbf{f}}{\delta \mathbf{x}}}] and latexmath:[{\frac{\delta \mathbf{g}}{\delta \mathbf{x}}}] are needed._
...
_For example, constructing the system Jacobian latexmath:[{\mathbf{A} = \frac{\delta \mathbf{f}}{\delta \mathbf{x}}}] as dense matrix can be performed in the following way:_
...
_The directional derivative of an output element latexmath:[{Y(i,j)}] with respect to the input latexmath:[{U}]) is:_

[latexmath]
++++
\Delta Y(i,j) =
\frac{\delta Y}{\delta U(1,1)} \cdot \Delta U(1,1) +
\frac{\delta Y}{\delta U(1,2)} \cdot \Delta U(1,2) +
\frac{\delta Y}{\delta U(2,1)} \cdot \Delta U(2,1) +
\frac{\delta Y}{\delta U(2,2)} \cdot \Delta U(2,2)
++++
...
_The retrieved directional derivative `dd` is stored in a matrix of size 3x2, so `nDvUnknown` is 6._
...
_and latexmath:[\left( w_1  w_2 \right) \cdot \mathbf{ \frac{\partial h}{\partial x} }] for some vector latexmath:[\left( w_1  w_2 \right)^T] is needed._
_Then one can get this with one function call of <<fmi3GetAdjointDerivative>> (with arguments_ latexmath:[v_{unknown} = \left \{ y_1, y_2 \right \},  v_{known} = \left \{ x_1, x_2 \right \}, \Delta v_{unknown} = \left( w_1 w_2 \right)^T] _), while with <<fmi3GetDirectionalDerivative>> at least two calls would be necessary to first construct the Jacobian column-wise and then multiplying from the right with_ latexmath:[\left( w_1  w_2 \right)^T] _]_
