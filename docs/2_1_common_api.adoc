...
_[Example of a definition in this header file: +
`typedef fmi3Status fmi3SetTimeTYPE(fmi3Instance, fmi3Float64);`]_
...
- Argument `nValues` provides the number of values in the `values` vector (and `sizes` vector, where applicable) which is only equal to `nValueReferences` if all `valueReferences` point to scalar variables.
...
- For ModelExchange: `fmi3Status = fmi3Discard` is possible for `fmi3GetFloat*` only, but not for `fmi3Get*Int*`, `fmi3GetBoolean`, `fmi3GetString`, `fmi3GetBinary`, because these are discrete-time variables and their values can only change at an event instant where `fmi3Discard` does not make sense.
...
- Argument `nValues` provides the number of values in the `values` vector which is only equal to `nValueReferences` if all `valueReferences` point to scalar variables.
