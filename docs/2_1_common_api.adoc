...
With this return value, the FMU signals that the function called could not achieve the defined effect and was able to keep its state as before the call.
Reasons could be arguments with illegal values, or wrong function calls for the current FMU state.
This return value essentially indicates that the function call had no effect on the FMU state.
The master can now try alternative approaches to drive the simulation.
If the master is incapable of such an alternative, it needs to treat this just like the `fmi3Error` return code.
...
// TODO: check this list: if we allow illegal arguments to be rejected with this return code, more functions might have to be listed here
...
+
In the co-simulation interfaces, the `fmi3Discard` return status is only possible for the functions:
`fmi3Set{VariableType}`, and
<<fmi3DoStep>>.
When <<fmi3Discard>> is returned to the importer, the importer has to decide if the simulation run can be continued or if to treat the <<fmi3Discard>> like <<fmi3Error>>.
+
...
_Still, `fmi3Set{VariableType}` functions could check whether the input arguments are valid (e.g. <<valueReference,`valueReferences`>>) or in their validity range (e.g. between min and max as defined in the `modelDescription.xml`)._
_If not, these functions could return with <<fmi3Discard>> and leave the internal state unchanged.]_
+
In Model Exchange and all Co-Simulation interfaces, function <<logMessage>> was called by the FMU, and it is expected that this function has shown the prepared information message to the user if the FMU was called in debug mode (`loggingOn == fmi3True`). If the FMU was not called in debug mode, <<logMessage>> should not show a message.

`fmi3EarlyReturn`::
[[fmi3EarlyReturn,`fmi3EarlyReturn`]]
This status can only be returned from an <<fmi3DoStep>> call and the <<fmi3CallbackIntermediateUpdate>> in Basic Co-Simulation or Hybrid Co-Simulation.
Returned by <<fmi3DoStep>> the FMU indicates, that the simulation step was only partially successful and `lastSuccessfulTime` returns the time inside the FMU when returning. 
Returned by <<fmi3CallbackIntermediateUpdate>> the importer signals to the FMU the desire to stop the simulation step, but no earlier than the <<earlyReturnTime>> argument indicates.
