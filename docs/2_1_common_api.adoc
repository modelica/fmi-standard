...
There are two access functions provided: One function is provided to compute directional derivatives, another to calculate adjoint derivatives.
Both function can also be used to construct desired partial derivative matrices.
...
_Typically, reverse mode automatic differentiatian (AD) is much more efficient for these use cases than forward mode AD, as explained in the cited references._
...
Using the same notation as for the directional derivatives for the different modes in which the FMU can be, the adjoint derivative is
