...
|A value of latexmath:[k] with latexmath:[k>0] signals to the master, that it is beneficial for the solver to receive intermediate inputs that are k-time continuously differentiable (latexmath:[C^k]) on the current communication interval. latexmath:[k=0] means continuous.
...
Even if this flag is `true`, the co-simulation master can chose to delegate event handling to the FMU by calling <<fmi3InstantiateCoSimulation>> with `eventModeRequired == fmi3False`.
If `eventModeRequired == fmi3True`, the co-simulation master will have to actively trigger event handling in the FMU using `fmi3EnterEventMode` and `fmi3ExitEventMode`.
...
_The co-simulation master is informed this way if calling these functions triggers event handling functionalities._
|====
