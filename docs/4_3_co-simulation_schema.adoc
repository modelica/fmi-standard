...
_[If <<dependencies>> are defined in the <<model-structure,model structure>>, it is recommended to define such <<dependencies>> only within a model partition of a model (i.e. between variables that are assigned to the same <<clock>>).]_
