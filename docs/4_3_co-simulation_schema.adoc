...
If the XML file defines an FMU for Clocked Co-Simulation, element `CoSimulation` must be present as described in section <<basic-co-simulation>>.
...
==== Scheduled Co-Simulation Support [[schema-scheduled-co-simulation]]
