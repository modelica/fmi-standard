...
In order to support the early return in `fmi3DoStep` the capability flag  `canReturnEarlyAfterIntermediateUpdate` in the XML should be `true`. 
 ...
 #[TODO]: New image similar to images/cs_xml_1.png where the capability flag `canReturnEarlyAfterIntermediateUpdate` is set?#
...
==== Intermediate Variable Access [[schema-intermediate-variable-access]] 
