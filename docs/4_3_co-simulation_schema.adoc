...
Each <<inputClock>> that ticks outside of the FMU, is activated for an FMU based on their `valueReferences`.
<<outputClock,`Output clocks`>> inside of an FMU signal their activation based on their `valueReferences`.
