...
If `eventModeUsed == fmi3True`, the co-simulation algorithm will have to actively trigger event handling in the FMU using `fmi3EnterEventMode` and `fmi3ExitEventMode`.
...
If the FMU has synchronous clocks, then `eventModeUsed == fmi3True` and the importer must call <<fmi3InstantiateXXX>> with `eventModeUsed == fmi3True`.
