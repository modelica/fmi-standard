...
In the following example, the usage of the `fmi3XXX` functions is sketched in order to clarify the typical calling sequence of the functions in an importer.
Furthermore, it is assumed that one FMU is directly integrated in an importer.
