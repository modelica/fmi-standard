...
sources                       // directory containing the C sources (optional)
   ...
   // <ModelExchange><BuildConfiguration><SourceFileSet><SourceFile>...
...
resources                     // resources needed by the FMU
   ...
   // must be reported via parameter "resourceLocation" of fmi3InstantiateXXX.
...
All C source file names that need to be defined in a compiler directive have to be defined in the XML file under structure `<ModelExchange><BuildConfiguration><SourceFileSet><SourceFile>`.
