...
resources                     // resources used by the FMU (optional)
   ...
   // can be reported via argument "resourceLocation" of fmi3InstantiateXXX.
...
_[Note that an URI pointing to the resources directory is provided by the initialization functions. If the environment is not able to do so, a NULL pointer will be provided instead, and the FMU can react with an error, if it necessarily depends on the content of the resources folder.]_
