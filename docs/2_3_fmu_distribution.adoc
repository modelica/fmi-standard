...
If necessary, further information on include search paths, preprocessor definitions and additional static libraries have to be defined in the XML file under `<ModelExchange>` and `<CoSimulation>` (elements `<IncludeDirectories>`, `<PreprocessorDefinition>` and `<Libraries>`). 
