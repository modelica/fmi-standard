...
[source, C]
----
include::examples/se_example_sim_alg.c[lines=25..87]
----

==== FMU Implementation

The FMU implements <<fmi3ActivateModelPartition>> dispatching for every <<clocks, `clock`>> with <<causality, `causality == input`>>.
...
[source, C]
----
include::examples/se_example_fmu.c[lines=508..623]
----
