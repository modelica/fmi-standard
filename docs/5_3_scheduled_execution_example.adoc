...
During the execution of the model partition of <<Clock>> `10msClock`, the FMU changes the <<interval>> for <<countdown,countdown clock>> `AperiodicClock` and calls <<fmi3ClockUpdateCallback>> to inform the importer that the interval has changed.
...
In the task being executed every 10 ms, the FMU initiates the scheduling of a task by setting a new interval to <<countdown,countdown Clock>> `AperiodicClock` and evoking <<fmi3ClockUpdateCallback>>.
