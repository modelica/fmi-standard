...
During the execution of the model partition of <<Clock>> `10msClock`, the FMU changes the <<interval>> for <<countdown,countdown clock>> `AperiodicClock` and calls <<fmi3CallbackClockUpdate>> to inform the importer that the interval has changed.
...
It calls <<fmi3CallbackClockUpdate>> to enable the importer to check whether the FMU has defined a new interval for `AperiodicClock`.
...
In the task being executed every 10 ms, the FMU initiates the scheduling of a task by setting a new interval to <<countdown,countdown Clock>> `AperiodicClock` and evoking <<fmi3CallbackClockUpdate>>.
