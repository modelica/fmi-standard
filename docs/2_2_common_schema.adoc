...
\left\{\begin{array}{ll}
q_{\mathit{$i$, outStream}} &\textit{if $\dot{m}$ is outflowing through terminal $i$}\\
q_{\mathit{$i$, inStream}} &\textit{if $\dot{m}$ is inflowing through terminal $i$}
\end{array}\right.]_
...
latexmath:[\color{blue}{(\mathbf{y}_{\mathit{c+d}}, \dot{\mathbf{x}}_c) := \mathbf{f}_{\mathit{init}}(\mathbf{x}_c, \mathbf{u}_{\mathit{c+d}}, t_0, \mathbf{p})}] +

latexmath:[\color{blue}{(\mathbf{y}_{\mathit{c+d}}, \dot{\mathbf{x}}_c) := \mathbf{f}_{\mathit{sim}}(\mathbf{x}_c, \mathbf{u}_{\mathit{c+d}}, t_i, \mathbf{p})}] +
...
Otherwise, the corresponding known latexmath:[\color{blue}{v_{\mathit{known},\mathit{i}}}] enters the equation as:

`= dependent`: no particular structure, latexmath:[\color{blue}{{h(..,\ v}_{\mathit{known},\mathit{i}}}],..)

Only for floating point type unknowns latexmath:[\color{blue}{v_{\mathit{unknown}}}]:

`=` <<constant>>: constant factor, latexmath:[\color{blue}{c \cdot v_{\mathit{known},\mathit{i}}}] where latexmath:[\color{blue}{c}] is an expression that is evaluated before <<fmi3EnterInitializationMode>> is called.
...
`=` <<fixed>>: fixed factor, latexmath:[\color{blue}{p \cdot v_{\mathit{known},\mathit{i}}}] where latexmath:[\color{blue}{p}] is an expression that is evaluated before <<fmi3ExitInitializationMode>> is called.

`=` <<tunable>>: tunable factor, latexmath:[\color{blue}{p \cdot v_{\mathit{known},\mathit{i}}}] where latexmath:[\color{blue}{p}] is an expression that is evaluated before <<fmi3ExitInitializationMode>> is called and in *Event Mode* due to an external event (ME) or at a communication point (xCS)

`=` <<discrete>>: discrete factor, latexmath:[\color{blue}{d \cdot v_{\mathit{known},\mathit{i}}}] where latexmath:[\color{blue}{d}] is an expression that is evaluated before <<fmi3ExitInitializationMode>> is called and in *Event Mode* due to an external or internal event or at a communication point (xCS).
|====
