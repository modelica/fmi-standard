...
If an <<input-clock>> has `fmi3True` as an <<start>> value the master should activate the <<clock>> the first time it enters *Event Mode*.
...
If <<output-clock,`output clocks`>> and <<input-clock,`input clocks`>> of `clockType = CPClock` are defined in the `modelDescription.xml` it is possible to define a tick relationship from a <<output-clock>> to an aperiodic <<input-clock>> based on <<clockReference>> (<<clock-relationships-for-communication-point-clocks>>).
This is done for aperiodic <<input-clock,`input clocks`>> that have the clockType = `CPClock` via providing a list of <<valueReference>> of <<output-clock,`output clocks`>> of the same FMU.
...
It is not allowed to combine <<output-clock,`output clocks`>> with periodic or strict periodic <<input-clock,`input clocks`>> based on <<clockReference>>.
