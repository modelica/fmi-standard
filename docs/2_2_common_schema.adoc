...
If <<outputClock,`output clocks`>> and <<inputClock,`input clocks`>> of clockType equals `CPClock` are defined in the modelDescription.xml it is possible to define a tick relationship from a <<outputClock>> to an aperiodic <<inputClock>> based on `clockReference` (<<clock-relationships-for-communication-point-clocks>>).
