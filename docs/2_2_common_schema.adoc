...
_This could lead to unpredictable behaviour of the FMU in different importing tools, as it is not mandatory to call `fmi3SetXXX` to set start values during initialization. Instead it is recommended to use the SSP Standard (https://ssp-standard.org/) to handle modified parameters of FMUs or different parameter sets._

The interpretation of <<start>> is defined by `Variable` / <<initial>>.
