...
This could lead to unpredictable behaviour of the FMU in different importing tools, as it is not mandatory to call `fmi2SetXXX` to set start values during initialization. Instead it is recommended to use the SSP Standard to handle modified parameters of FMUs or different parameter sets.]_
...
_[Clarification for FMI 2.0.1, the scalarVariables of the xml must not be ordered in depth first, they can be ordered arbitrary.
