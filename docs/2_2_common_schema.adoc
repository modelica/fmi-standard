...
//_[If a derivative of an output terminal member variable is provided as another output terminal member variable using the derivative attribute, then the importing tool may ignore this terminal member variable [TODO: better?: There is no special handling of derivatives. If a derivative is a terminal member variable, then the importing tool has to be treated like any another terminal member variable.]]_


===== Terminal Member Variable

//image::images/fmi3Terminal_Terminal_Schema.png[width=70%, pdfwidth=70%, align="center"]
...
_[Suggested variable naming scheme for the structured naming convention:
<ModelVariable name> = <terminal name>.<memberName>]_ [TODO: Maybe add example for this? Also hierarchical terminal example]

_[Suggested variable naming scheme for the non-structured naming convention:
<ModelVariable name> = <memberName>]_

_[Matching is not restricted by variability, causality or variable type. Example: A fixed variable may be connected to a tunable variable, a variable of type Real may be connected to a variable of type Integer.
...
|"common" / "variable" / "property" / "signal"
|Common signal flow.

Intended for input and output.

|"equalityRelation"
|Direction of propagation could be defined on system level.

Intended for parameter and calculatedParameter. 

_[Example: Parameter propagation]_

|"equalAssert"
|The exporting tool suggests that a connection is invalid if these values in connected terminals are different. This could be implemented as assertion. This feature can be used for compatibility checks for terminals.

Intended for parameter, calculatedParameter and local variables.

_[Example: Compatibility checks]_
...
===== General remark on inStream and outStream

appendix D.3 of the Modelica speicifcation
_[The Stream concept is used to describe convective transport phenomena [Rüdiger Franke et al., 2009, Stream Connectors - An Extension of Modelica for Device-Oriented Modeling of Convective Transport Phenomena, Proceedings 7th Modelica Conference, Como, Italy, Sep. 20-22]. This concept allows only one flow variable per terminal. The concept is used for variables which have to fulfill the balance equation for transported quantities:]_

∑▒〖h_i∙m ̇_i 〗=0
0=∑▒〖m ̇_i∙{■(h_(i,"outflow" )  "if " m ̇" is outflowing through terminal i" @h_(i,inflow)  "if " m ̇" is inflowing through terminal i" )┤ 〗

_[The h(i,"outflow" ) is the convective quantity in case the mass flows out of the FMU. h(i,inflow) is the convective quantity in case the mass flows into the FMU. Both variables are present in the terminal. Naturally the outflowing value is defined by the FMU itself and hence an output. The inflowing value is an input. Although both flow direction cases are always present, the actual value has to be selected by the importing tool. However, calculating the actual value is not necessary.]_

_[If only two terminals with a stream quantity are connected and their causality matches, then the inputs and outputs can be connected directly. The mass flow rate may have to be negated as mentioned above.]_


_[In Modelica the inStream variable is not directly visible, the value can only be accessed using “inStream()”, therefore an additional ModelVariable has to be added during the export. 
...
If a member variable has the variableKind "inStream" or "outStream", then there must be one and only one member variable which has the variableKind "inflow" or "outflow". 

The order of the inStream variables must be equal to the order of the outStream variables. _[I.e. the first inStream variable belongs to the first outStream variable.]_
