...
|`guid`
a|The "Globally Unique IDentifier" is a 128-bit number encoded as a string (as defined in https://www.rfc-editor.org/info/rfc4122[RFC 4122]) that is used to check that the XML file is compatible with the C functions of the FMU.
Typically when generating the XML file, a fingerprint of the "relevant" information is stored in the attribute `guid` and in the C code / shared library.
The sixteen octets of a UUID are represented as 32 hexadecimal (base 16) digits, displayed in five groups separated by hyphens, in the form 8-4-4-4-12 for a total of 36 characters (32 alphanumeric characters and four hyphens).
For example:

  123e4567-e89b-12d3-a456-426655440000
  xxxxxxxx-xxxx-Mxxx-Nxxx-xxxxxxxxxxxx

The four bits of digit M indicate the GUID version, and the one to three most significant bits of digit N indicate the UUID variant.
