...
_[For example, if_ latexmath:[\color{blue}{p_{\mathit{bar}}}] _is a pressure value in unit `bar`, and_ latexmath:[\color{blue}{p_{\mathit{Pa}}}] _is the pressure value in `<BaseUnit>`, then_

latexmath:[\color{blue}{p_{\mathit{Pa}} = 10^5 p_{\mathit{bar}}}]
...
`= communicationPoint` <<clock,`clocks`>> define sampling points (i.e. communication points) for the variables of model partitions and are used in Hybrid Co-Simulation and FMI for Scheduled Co-Simulation.
...
_[It is suggested that Modelica tools store the Modelica annotation of the connector under the `type` `org.modelica.Modelica3Annotation` in the annotations of an element `connector`._
...
[[parameter,`parameter`]]
`= parameter`: A data value that is constant during the simulation and is provided by the environment and cannot be used in connections.
...
[[inputClock,`inputClock`]] If <<variability>> = <<clock>> the variable defines an <<inputClock, `input clock`>> that is controlled by the master.
...
[source, xml]
----
include::examples/structural_paramer_example.xml[tags=structuralParameter]
----
...
Variables with <<causality>> <<parameter>> must not be marked with <<intermediateAccess,`intermediateAccess = true`>>.
...
|_<<tunable,`Tunable`>> <<parameter>> (changing such a <<parameter>> triggers an external event (ME) or takes effect at the next communication point (xCS), and <<tunable>> <<calculatedParameter>>/<<output>>/<<local>> variables might change their values)._
...
|_<<tunable,`Tunable`>> <<dependenciesKind,`dependent`>> <<parameter>> (changing a <<parameter>> triggers an external event (ME) or takes effect at the next communication point (xCS), and <<tunable>> <<dependenciesKind,`dependent`>> <<parameter,`parameters`>> and <<tunable>> <<local>> variables might change their values)._
...
[blue]#latexmath:[\color{blue}{(\mathbf{y}_c, \mathbf{y}_d) := \mathbf{f}_{\mathit{output}}(\mathbf{x}_c, \mathbf{u}_c, \mathbf{u}_d, t, \mathbf{p}_{\mathit{tune}})}]#
...
The functional dependency is defined as (dependencies of variables that are fixed in Event and *Continuous-Time Mode* and at communication points are not shown): +
[blue]#latexmath:[\color{blue}{\dot{\mathbf{x}_c} := \mathbf{f}_{\mathit{der}}(\mathbf{x}_c, \mathbf{u}_c, \mathbf{u}_d, t, \mathbf{p}_{\mathit{tune}})}]#
...
[blue]#latexmath:[\color{blue}{\mathbf{v}_{\mathit{initialUnknowns}} := \mathbf{f}_{\mathit{init}}(\mathbf{u}_c, \mathbf{u}_d, t_0, \mathbf{v}_{\mathit{initial=exact}})}]#
...
latexmath:[\color{blue}{(\mathbf{y}_{c+d}, \dot{\mathbf{x}}_c) := \mathbf{f}_{\mathit{init}}(\mathbf{x}_c, \mathbf{u}_{c+d}, t_0, \mathbf{p})}] +

latexmath:[\color{blue}{(\mathbf{y}_{c+d}, \dot{\mathbf{x}}_c) := \mathbf{f}_{\mathit{sim}}(\mathbf{x}_c, \mathbf{u}_{c+d}, t_i, \mathbf{p})}] +
...
|`valueReference`
|The value reference of the unknown latexmath:[\color{blue}{v_{\mathit{unknown}}}].


|`dependencies`
|
[[dependencies,`dependencies`]]
Optional attribute defining the dependencies of the unknown latexmath:[\color{blue}{v_{\mathit{unknown}}}] (directly or indirectly via auxiliary variables) with respect to latexmath:[\color{blue}{\mathbf{v}_{\mathit{known}}}].
...
Knowns latexmath:[\color{blue}{\mathbf{v}_{\mathit{known}}}] in *Event Mode* and *Continuous-Time Mode* (ME) and at communication points (xCS) for `<Output>` and `<Derivative>` elements:
...
`Knowns` latexmath:[\color{blue}{\mathbf{v}_{\mathit{known}}}] in *Initialization Mode* (for elements <<InitialUnknown>>):
...
If not present, it must be assumed that the unknown latexmath:[\color{blue}{v_{\mathit{unknow}}}}] depends on the knowns latexmath:[\color{blue}{\mathbf{v}_{\mathit{known}}}] without a particular structure.
Otherwise, the corresponding known latexmath:[\color{blue}{v_{\mathit{known},i}}] enters the equation as:

`= dependent`: no particular structure, latexmath:[\color{blue}{{h(..,\ v}_{\mathit{known},i}}],..)

Only for floating point type unknowns latexmath:[\color{blue}{v_{\mathit{unknown}}}]:

`=` <<constant>>: constant factor, latexmath:[\color{blue}{c \cdot v_{\mathit{known},i}}] where latexmath:[\color{blue}{c}] is an expression that is evaluated before <<fmi3EnterInitializationMode>> is called.

Only for floating point type unknowns latexmath:[\color{blue}{v_{\mathit{unknown}}}] in Event and *Continuous-Time Mode* (ME) and at communication points (xCS), and not for <<InitialUnknown>> for *Initialization Mode*:

`=` <<fixed>>: fixed factor, latexmath:[\color{blue}{p \cdot v_{\mathit{known},i}}] where latexmath:[\color{blue}{p}] is an expression that is evaluated before <<fmi3ExitInitializationMode>> is called.

`=` <<tunable>>: tunable factor, latexmath:[\color{blue}{p \cdot v_{\mathit{known},i}}] where latexmath:[\color{blue}{p}] is an expression that is evaluated before <<fmi3ExitInitializationMode>> is called and in *Event Mode* due to an external event (ME) or at a communication point (xCS)

`=` <<discrete>>: discrete factor, latexmath:[\color{blue}{d \cdot v_{\mathit{known},i}}] where latexmath:[\color{blue}{d}] is an expression that is evaluated before <<fmi3ExitInitializationMode>> is called and in *Event Mode* due to an external or internal event or at a communication point (xCS).
|====
