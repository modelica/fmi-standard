...
_[If the graphical representation is used for a single <<input>> or <<output>> (e.g. a `fmi3Float64` <<input>> `u`), then a `Terminal` has to be added to the `Terminals` element which has one `TerminalMemberVariable`._
