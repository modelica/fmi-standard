...
Hence, only top level `Terminal`s from the terminal hierarchy defined under `Terminals` can have a graphical representation.
_[If the graphical representation is used for an single <<input>> or <<output>> (e.g. a Real <<input>> `u`), then a `Terminal` has to be added to the `Terminals` element which has one `TerminalMemberVariable`._
