...
The `ModelVariables` element consists of an ordered set of `Variable` elements (see figure above). A `Variable` represents one function type, like a real or integer variable. 
The schema definition is present in a separate file `fmiVariable.xsd`.
...
|clockReference
|The additional optional attribute `clockReference` is used to define the clocks this variable is assigned to. 
Please note that this attribute can be used for all variable sub types with restrictions based on the clockType attribute. 
The `clockReference` holds the `valueReference` information of the variable that defines the clock.
...
It is not possible to associate more than one clock to a variable (based on clockReference).
...
It is possible to associate multiple clocks to a variable (based on clockReference). Variables that are assigned to a CommunicationPointClock are not necessarily `clocked`. Such variables can be Continuous-time or Discrete-time variables.
If triggered clocks and inferred clocks are defined in the modelDescription.xml it is possible to define a tick relationship from a triggered clock to an aperiodic inferred clock. 
For aperiodic inferred clocks that do not have the clockType = `STClock` a list of valueReferences of triggered clocks of the same FMU can be provided. 
If a triggered clock of this valueReference list ticks the master has to create a tick for the acociated aperiodic infrerred clock. 
It is not allowed to combine triggered clocks with a periodic or strict periodic inferred clocks based on clockReference.
...
|causality
|The two clock variants can be defined via the attribute `causality` of the `Variable` element. 
The clock variant inferred clock is defined with causality = `inferred` and the clock variant triggered clock is defined with causality = `triggered`.

|valueReference
|Clocks are identified based on their valueReference, e.g. a clock is referenced in API-function calls with its valueReference information. Therefore the clock valueReference has to be unique for an FMU. 
Note that the attribute valueReference information of the clock variable is used to accociate the clock to other variables (see clockReference). 
It is therefore not possible to use clocks in FMI 3.0 arrays, since array elements do not have individual valueReferences.
...
|This attribute should not be used for Co-Simulation – we do not change the current use of this flag. 
Flag is meant for inputs, a clock is not an input
|===
...
For FMI for Co-Simulation discrete parts that do not necessarily apply to synchroneous theory or continuous parts of the FMU can also be associated to model partitions that define the sampling points (i.e. communication points) for the variables of these model partitions. 
Inferred clocks that are defined for such cases have the clockType attribute value `CPClock` and have to be ignored for FMI for Model Exchange. 
...
In this case no ordering is defined for clocks of the same priority. 
...
The meaning of the attributes is defined in section 2.1.8.1 (Mathematical Definition). 
If the optional attributes _interval_, _intervalCounter_, _shiftCounter_ and _resolution_ are not present, then this clock is not periodic. 
If the optional attribute _strict_ is set to true, then the the FMU and/or environment have to respect the predefined interval and offset. 
If the optional attribute _strict_ is set to false another interval or offset can be used, based on requirements derived from the current co-simulation setup.
