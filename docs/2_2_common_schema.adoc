...
- _On an <<input>>, the FMU decides what to return (If no computation is possible, it could return <<fmiDiscard,`fmi3Status == fmi3Discard`>>, in other situations it may return `fmi3Warning` or `fmi3Error`, or `fmi3OK`, if it is uncritical)._
