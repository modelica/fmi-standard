...
_The `matchingRule` refers to the `<TerminalMemberVariable>` on the same level only. Nested terminals can have different `matchingRules`._
...
With attribute `<fmiModelDescription><variableNamingConvention>`, the convention is defined how the variable names have been constructed.
