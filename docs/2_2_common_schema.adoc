...
|"signal"
|The values in connected terminals must be equal.

Intended for input and output, parameter and calculatedParameter.
...
_[used for input output signal flow]_

_[can be used for parameter propagation]_

_[Example: Medium name. constant+parameter, parameter+parameter...]_

_[can be used for used for equal assert]_

_[Example: Cross sectional area calculated from geometry parameters...]_
...
_[ See appendix D.3 of the Modelica specification
. This concept allows only one flow variable per terminal. The concept is used for variables which have to fulfill the balance equation for transported quantities:_

_latexmath:[0 = \sum{h_i\dot{m}_i}]
latexmath:[0 = \sum{\dot{m}_i}\cdot
\left\{\begin{array}{ll}
h_{\textnormal{$i$, outStream}} &\textnormal{if $\dot{m}$ is outflowing through terminal $i$}\\
h_{\textnormal{$i$, inStream}} &\textnormal{if $\dot{m}$ is inflowing through terminal $i$}
\end{array}\right.]_


_The latexmath:[h_\textnormal{$i$, outStream}] is the convective quantity in case the mass flows out of the FMU. latexmath:[h_\textnormal{$i$, inStream}] is the convective quantity in case the mass flows into the FMU. Both variables are present in the terminal. Naturally the outflowing value is defined by the FMU itself and hence an output. The inflowing value is an input. Although both flow direction cases are always present, the actual value has to be selected by the importing tool. However, calculating the actual value is not necessary.]_

_If only two terminals with a stream quantity are connected and their causality matches, then the inputs and outputs can be connected directly. The mass flow rate may have to be negated as mentioned above._
...
The order of the inStream variables must be equal to the order of the outStream variables. _[I.e. the first inStream variable belongs to the first outStream variable.
