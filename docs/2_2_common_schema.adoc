...
Both, the terminals and the graphical representation are fully optional and can be ignored by any importing tool. The graphical representation of a terminal or FMU ModelVariable is separate from the terminal definitions.

Definition "Terminal": A terminal is...

* a structured interface for connections to other models
* a sequence of TerminalMemberVariables which have a variableKind
...
This specification just provides predefined variable matching rules for connections and predefined variable kinds to describe the member variables. Domain specific terminals and their member variables can be provided by other standards.

CoSimulation errors are not addressed by this FCP. The CoSimulation algorithm has to be chosen and implemented by the importing tool. Features that might be required for specific CoSimulation algorithms had to be implemented by the FMU exporting tool.

Algebraic loops in systems of connected ModelExchange FMUs are not discussed or resolved by this FCP. It is not required that the causality of the terminal member variables in connected terminals matches. Sophisticated, domain specific terminals and connection rules can be developed using the terminal information from this FCP.
...
image::images/fmiModelDescription_Terminals_Schema.png[width=70%, align="center"]

All instances of `Terminal` are listed in the `Terminals` sequence. In addition to that vendor anntotations can be provided:

image::images/fmiModelDescription_Terminals_Terminal_Schema.png[width=70%, align="center"]

The attribute `name` of the Terminal element is the instance name of the terminal. 
The terminal name may contain dots, to enable structured terminals.
If the structured naming convention is used, then it is suggested that this name is equal to the prefix of the member variables. 

The attribute `matchingRule` describes the rules for variable matching in a connection. 
There are three predefined matching rules: plug, bus, and sequence. 
Other standards may define new matching rules. 
Vendor specific rules should start with "__vendorName_".
...
It is intended that an importing tool shall not ignore terminal member variables.

_[If a derivative of an output is provided as another output using the derivative attribute, then the importing tool may ignore this terminal member variable]_
...
It is intended that an importing tool may ignore some or all terminal member variables.
...
`terminalKind` is an optional attribute. Other standards may define new terminalKinds. 
It is recommended that other terminalKinds refer to the predefined matchingRules.
Vendor specific terminal kinds should start with "__vendorName_".

_[Example: StandardXXX_Mechanical_Translational, ____vendorNameA_customTypeA, ____VendorNameB_customLibrary_customTypeB]_

Inputs must not appear in more than one terminal. 

_[Not all variables with the prefix "<terminalName>." are part of the terminal with the name "<terminalName>". 
So for instance it is possible to have a global parameter which is part of a terminal. 
It might be handled similar to an alias variable.]_
...
image::images/fmi3Terminal_TerminalMemberVariable_Schema.png[width=70%, pdfwidth=70%, align="center"]

The string `variableKind` is used to provide general information about the variable. This information defines how the connection of this variable has to be implemented (e.g. inflow variable).

`memberName` is the representation in name-based matching tools. 
The attribute has to be provided to identify aliases (local variables, calculatedParameters). 
The member name may contain dots, to enable structured terminals.

Suggested variable naming scheme for the structured naming convention:
<ModelVariable name> = <terminal name>.<memberName> 

Suggested variable naming scheme for the non-structured naming convention:
<ModelVariable name> = <memberName>

_[If a tool applies sequence based variable matching, then it still makes sense to provide the information, how these terminals should be imported into tools with name-based variable matching.]_

_[The variability of the member variables is not restricted, i.e. a fixed variable might be connected to a tunable variable.
The variable type of the member variables is not restricted, a Real might be connected to an Integer.
However, it is suggested that the variable types and variabilities are equal.]_
...
|"in/outflow"
|Variables which fulfill Kirchhoff’s current law Intended for input and output. Only for Real variables.

|"in/outStream"
|Variables which fulfill the balance equation for transported quantities.

Intended for input and output. Only for Real variables.


|====

If a stream variable is present, then only one flow variable is allowed. 
The order of the inStream variables must be equal to the order of the outStream variables. I.e. the first inStream variable belongs to the first outStream variable.
...
The graphical representation of models and terminals are needed in order to more easily comprehend the meaning of connected FMUs and to help an importing tool to display the terminals and the FMU icon in the way the exporter intended.



image::images/GraphicalRepresentation.svg[width=70%, align="center"]


There is an optional `CoordinateSystem` defining the extent of the whole icon (graphical items may exceed that rect), an optional element `Icon` defining an image source, and a `Terminal` sequence.
The element `GraphicalRepresentation` of `fmiModelDescription` is defined as:

image::images/fmiModelDescription_GraphicalRepresentation_Schema.png[width=70%, pdfwidth=70%, align="center"]

===== CoordinateSystem

image::images/fmiModelDescription_GraphicalRepresentation_CoordinateSystem_Schema.png[width=70%, pdfwidth=70%, align="center"]
...
The coordinate system is defined by the coordinates of two points, the left (x1) lower (y1) corner and the right (x2) upper (y2) corner, where the coordinates of the first point shall be less than the coordinates of the second point _[a first quadrant coordinate system]_. 
...
The area defined by the coordinate system suggested to be used as “clickable icon size” in other tools. Terminals might be placed outside of this area, so the visible bounding box has to be determined by the importing tool.

The coordinate system default is x1=-100, y1=-100, x2=100, y2=100. This extent is used if the CoordinateSystem element is missing. The default suggestedScalingFactorTo_mm is 0.1. So the default coordinate system display size should be 20mm width and 20mm height.
...
image::images/fmiModelDescription_GraphicalRepresentation_Icon_Schema.png[width=70%, pdfwidth=70%, align="center"]
...
Optionally an SVG file can be provided. This enables high quality rendering and printing in importing tools.


===== Terminals

image::images/fmiModelDescription_GraphicalRepresentation_Terminal_Schema.png[width=70%, pdfwidth=70%, align="center"]
...
If the graphical representation is used for a ModelVariable (e.g. a Real input “u”) which is not in the Terminals element, then the name is equal to the name of the ModelVariable.
...
There is no restriction on the variable type, variability or causality for the graphical representation of a ModelVariable.
...
However, an importing tool has to determine the outer bounding box enclosing all graphical items.
...
The first element in the GraphicalRepresentation is painted first and therefore behind the others, the last element is painted last and because of that in front of the others. 
So the FMU icon should be placed first, terminal below.
