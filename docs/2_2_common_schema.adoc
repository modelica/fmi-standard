...
If <<output-clock,`output clocks`>> and <<inputClock,`input clocks`>> of `clockType = CPClock` are defined in the `modelDescription.xml` it is possible to define a tick relationship from a <<output-clock>> to an aperiodic <<inputClock>> based on <<clockReference>> (<<clock-relationships-for-communication-point-clocks>>).
This is done for aperiodic <<inputClock,`input clocks`>> that have the clockType = `CPClock` via providing a list of <<valueReference>> of <<output-clock,`output clocks`>> of the same FMU.
...
It is not allowed to combine <<output-clock,`output clocks`>> with periodic or strict periodic <<inputClock,`input clocks`>> based on <<clockReference>>.
...
_[For example, if `min` is present both in `Real` (of `TypeDefinition`) and in `Real`, then the `min` of variables is actually used.]_
...
_[Therefore, calling_ `fmi3SetXXX` _to set <<start>> values is only necessary, if a different value as stored in the XML file is desired.]_ The interpretation of <<start>> is defined by variable attribute <<initial>>.
...
// TODO: is it helpful to include also <<clock>> dependencies here --> i.e. changing this variable will lead to a <<output-clock>> tick.
