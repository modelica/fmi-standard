...
_[An example of use for stream is a gas mixture flow. The gas composition could be implemented as a mass fraction vector. The `outStreamValueReference` refers to latexmath:[portA.x_\textrm{outStream}[]] and the `inStreamValueReference` refers to latexmath:[portA.x_\textrm{inStream}[]]. The `inStreamMemberName` and `outStreamMemberName` are "latexmath:[x_\textrm{inStream}[]]" and "latexmath:[x_\textrm{outStream}[]]".
]_
