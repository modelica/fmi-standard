...
For more details on ModelStructure, see section <<ModelStructure>>.
...
Variable elements can uniformly represent variables of primitive (atomic) types, like single float or integer variables, or as well as arrays of an arbitrary (but fixed) number of dimensions. The schema definition is present in a separate file `fmi3Variable.xsd`.
...
If one variable is defined as <<independent>>, it must be defined with a float type without a <<start>> attribute.
...
_This could lead to unpredictable behavior of the FMU in different importing tools, as it is not mandatory to call `fmi3SetXXX` to set start values during initialization. Instead it is recommended to use the SSP Standard (https://ssp-standard.org/) to handle modified parameters of FMUs or different parameter sets._
...
_[Note that only <<continuous>> float variables are listed here._
...
Only for float type unknowns latexmath:[\color{blue}{v_{\text{unknown}}}]:
...
Only for float type unknowns latexmath:[\color{blue}{v_{\text{unknown}}}] in Event and *Continuous-Time Mode* (Model Exchange) and at communication points (Co-Simulation), and not for <<InitialUnknowns>> for *Initialization Mode*:
...
_where_ latexmath:[\color{blue}{u_{1}}] _is a continuous-time <<input>> (<<variability>> = <<continuous>>),_ latexmath:[\color{blue}{u_{2}}] _is any type of <<input>>,_ latexmath:[\color{blue}{u_{3}}] _is a float discrete-time <<input>> (<<variability>> = "discrete"`), and_ latexmath:[\color{blue}{p}] _is a <<fixed>> <<parameter>> (<<variability>> = <<fixed>>)._
