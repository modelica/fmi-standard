...
If <<eventIndicator>> = `true`, this variable is an event indicator.
|====
...
All continuous state variables must have a corresponding variables with attribute `derivative` in order that the continuous <<state,`states`>> are uniquely defined.
