...
|The FMU is able to provide access to selected variables during callback function call <<fmi3CallbackIntermediateUpdate>>.
The accessible variables are marked with attribute <<intermediateUpdate,`intermediateUpdate = true`>>.
This flag is not supported in `ModelExchange`.
...
latexmath:[k=0] means continuous.
_[If for example the FMU uses internally a variable step BDF solver of order 4, in order to fulfill the convergence rate, latexmath:[C^4] is required for the inputs._
_If on the other hand, an FMU sub-samples the communication step with 10 fixed steps and uses a first order explicit Euler method as solver,_ latexmath:[C^0] _smoothness will be sufficient.]_
This flag is not supported in `ModelExchange`.
