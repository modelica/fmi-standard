...
`= synchronousTime`: If the properties of a <<clock>> adhere to synchronous clock theory it is defined with `clockType = synchronousTime`.
`= communicationPoint`: Otherwise, if the <<clock>> is to be used in FMI for Hybrid Co-Simulation, FMI for Scheduled Co-Simulation, or can be associated to model partitions in continuous parts of the FMU, then `clockType = communicationPoint`.
