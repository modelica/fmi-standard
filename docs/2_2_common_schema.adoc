...
If output clocks and <<input>> clocks of clockType equals `CPClock` are defined in the modelDescription.xml it is possible to define a tick relationship from a output <<clock>> to an aperiodic <<inputClock>> based on `clockReference` (<<clock-relationships-for-communication-point-clocks>>).
This is done for aperiodic <<input>> clocks that have the clockType = `CPClock` via providing a list of valueReferences of output clocks of the same FMU.
