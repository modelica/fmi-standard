...
// TODO still unclear: where can I use which clockType?

|`relatedOutputClockReference`
|
[[relatedOutputClockReference,`relatedOutputClockReference`]]
The optional attribute <<relatedOutputClockReference>> is used to define a tick relationship from an <<outputClock>> to an <<periodic,aperiodic>> <<inputClock>> (see <<clock-relationships-for-communication-point-clocks>>).

Only <<clock,`clocks`>> with <<causality>> = <<input>> and <<clockType>> = <<communicationPoint>> can have this attribute.
It is not allowed to combine <<outputClock,`output clocks`>> with <<periodic>> or <<strict>> <<periodic>> <<inputClock,`input clocks`>> based on <<relatedOutputClockReference>>.
