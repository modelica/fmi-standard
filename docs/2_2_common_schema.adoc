...
At least one element of `<ModelExchange>`, `<CoSimulation>` or `<ScheduledCoSimulation>` must be present to identify the type of the FMU.
If multiple elements are defined, different types of models are included in the FMU.
The details of these elements are defined in <<fmi-for-model-exchange>>, <<fmi-for-co-simulation>> or <<fmi-for-scheduled-co-simulation>>.
...
`= communicationPoint` <<clock,`clocks`>> define sampling points (i.e. communication points) for the variables of model partitions and are used in Co-Simulation and FMI for Scheduled Co-Simulation.
