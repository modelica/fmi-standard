...
|`guid`
|The GUID is an opaque string that can be used by the FMU to check that the XML file is compatible with the implementation of the FMU.
For this purpose the importing tool must pass the GUID unchanged to `fmi3Instantiate`.
