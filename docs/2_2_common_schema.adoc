...
Only `Terminal` elements which are direct childs of the `Terminals` element can have a graphical representation.
_[If the graphical representation is used for a single <<input>> or <<output>> (e.g. a Real <<input>> `u`), then a `Terminal` has to be added to the `Terminals` element which has one `TerminalMemberVariable`._
