...
_If an FMU supports both Model Exchange and Co-Simulation, then the `Derivative` elements might be present, since it is needed for Model Exchange._
_If the above flag is set to `false` for the Co-Simulation case, then the `Derivative` elements are ignored for Co-Simulation._
...
Knowns latexmath:[\color{blue}{\mathbf{v}_{\text{known}}}] in *Event Mode* and *Continuous-Time Mode* (Model Exchange) and at communication points (Co-Simulation) for `Output` and `Derivative` elements:
...
Only for floating point type unknowns latexmath:[\color{blue}{v_{\text{unknown}}}] in Event and *Continuous-Time Mode* (Model Exchange) and at communication points (Co-Simulation), and not for <<InitialUnknown>> for *Initialization Mode*:
