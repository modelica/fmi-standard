...
* a sequence of `TerminalMemberVariable` each having a `variableKind`
...
Predefined rules for variable matching in a connection are given below. Predefined variable kinds are used to describe how the member variables have to be handled. Domain specific terminals and their member variables can be provided by other standards.
...
_[Algebraic loops in systems of connected ModelExchange FMUs are not addressed or resolved by the terminals. It is not required that the causality of the terminal member variables in connected terminals matches. Sophisticated, domain specific terminals and connection rules can be defined by other standards.]_
...
_[The terminal name may contain dots, to enable structured terminals.
If the structured naming convention is used, then it is suggested that this name is equal to the prefix of the member variables.]_
...
_[Vendor specific rules should start with "_vendorName" or "_toolName" to avoid namespace clashes.]_
...
The string normalized string `terminalKind` is an optional attribute. Other standards may define more `terminalKinds`. 
_[It is recommended that other `terminalKinds` refer to the predefined matchingRules.
Vendor specific terminal kinds should start with "_vendorName" or "_toolName" to avoid namespace clashes.]_

_[Example: StandardXXX_Mechanical_Translational, _vendorNameA_customTypeA, _VendorNameB_customLibrary_customTypeB]_
...
<ModelVariable name> = <terminal name>.<memberName>]_
