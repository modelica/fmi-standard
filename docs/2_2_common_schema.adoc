...
`= communicationPoint` <<clock,`clocks`>> define communication points for the variables of model partitions and are used in FMI for Co-Simulation and FMI for Scheduled Execution.
...
An <<inputClock, `input clock`>> can be left unconnected, can be connected to an <<outputClock,`output clock`>>, or the importer generates the clock signal.
...
An <<outputClock,`output clock`>> can be left unconnected or can be connected to an <<inputClock, `input clock`>>, usually of another FMU.
...
Co-Simulation without events: Apply the same heuristics as in FMI 2.0 to detect discontinuous changes to continuous variables for solver resets.
...
_[If an <<outputClock,`output clock`>> depends on an <<inputClock,`input clock`>>, then clock ticks of the <<inputClock,`input clock`>> in *Event Mode* may create <<outputClock,`output clock`>> ticks during this event handling.]_
