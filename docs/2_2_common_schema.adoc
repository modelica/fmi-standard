...
Since, <<output,`outputs`>>, continuous-time <<state,`states`>> and state derivatives are either present as `Knowns` (if `initial` = <<exact>>) or as `Unknowns` (if `initial` = <<approx>> or <<calculated>>),
