...
footnote:[Note, the screenshots of this section have been generated from the schema files with the tool "Altova XMLSpy" (www.altova.com).
...
_[Note, the definition of an encoding scheme is a prerequisite,
...
_[Note, child information items,
...
The approach used here is slightly different to FMI 1.0 to reduce the need for standardized string representations.]_
...
then the value of the variable has to be provided with the `fmi2SetXXX` functions or is returned by the `fmi2GetXXX` functions with respect to this `Unit`.
...
(Note, if in column `"exponents"` the definition "latexmath:[\color{blue}{kgm^2 / s^2}]"
...
_Note, "Hz" is typically used as `Unit.name` for a frequency quantity,
...
since absolute or relative quantities):_
...
_[Note, this approach is not satisfactory for variables belonging to different quantities that have,
...
|unit
|Unit of the variable defined with `UnitDefinitions.Unit.name` that is used for the model equations _[,
for example "N.m": in this case a `Unit.name = "N.m"` must be present under `UnitDefinitions`]_.
...
(for example 10 degree Celsius = 10 Kelvin if `"relativeQuantity = true"` and not 283,15 Kelvin).
...
The min definition is an information from the FMU to the environment defining the region in which the FMU is designed to operate,
see also comment after this table.

|max
|Maximum value of variable (variableValue ≤ `max`).
If not defined, the maximum is the largest positive number that can be represented on the machine.
The max definition is an information from the FMU to the environment defining the region in which the FMU is designed to operate,
see also comment after this table.
...
_where `tolerance` is, e.g., the relative tolerance defined in `<DefaultExperiment>`,
see section 2.2.5.]_

|unbounded
|If true,
indicates that the variable gets during time integration much larger than its nominal value `nominal`.
...
(e.g. if "u" is an input and "sqrt(u)" is computed in the FMU,
...
(e.g. for debug purposes) then the following strategy should be used:_
...
_[Note, since element `<LogCategories>` is optional,
...
it is implicitely present with name = `"time"` and `unit = "s"`.
...
then an event is triggered externally (ModelExchange) or the change is performed at the next _Communication Point_ (CoSimulation) and the variables with `variability = "tunable"` and `causality = "calculatedParameter"` or `"output"` must be newly computed.
...
_[Note, the information about continuous states is defined with element
...
For example,
there is no mechanism in the Modelica language to formulate a connection restrictions for C-functions (the FMU) called in a Modelica model.]_
|====
...
- `initial = "exact"` or `"approx"` _[in order to set the corresponding `start` value.]_
...
- `initial = "exact"` _[in order to set the corresponding `start` value]_,

or if

- `causality = "input"` _[in order to provide new values for inputs]_,
...
and *for Co-Simulation at every communication point*, if

- `causality = "parameter"` and `variability = "tunable"` _[in order to change the value of the tunable parameter at an event instant or at a communication point]_, or if
...
then the variable is explicitely defined by its start value in Initialization Mode (so directly after calling `fmi2ExitInitializationMode`,
...
_Changing of tunable parameters is allowed before an `fmi2DoStep` call (so whenever an input can be set with `fmi2SetXXX`) and before `fmi2ExitInitializationMode` is called (so before and during Initialization Mode).
...
as if `a` and `b` would not be treated as alias variables (that is,
would have different `valueReference`pass:[s]).
...
and then the variables are no longer `"independent"`. +
...
like `min/max/nominal` values or description texts.
...
In particular,
the "direct dependency" definition can be a super set of the "real" direct dependency definition,
...
_[If, for example `"min"` is present both in `Real` (of `TypeDefinition`) and in `"Real"` (of `ScalarVariable`),
...
(e.g. changing from initialization of states to steady-state initialization),
it is also possible to use the start value as iteration variable of an algebraic loop: Via an additional condition in the environment,
...
The min/max definitions are an information from the FMU to the environment defining the region in which the FMU is designed to operate,
...
_[For example, when exporting a model both in Model-Exchange and in Co- Simulation format,
...
A Co- Simulation FMU need not to expose these state derivatives.
_[If a Co-Simulation FMU exposes its state derivatives,
they are usually not utilized for the co-simulation,
but, e.g., to linearize the FMU at a communication point.]_
...
e.g., when connecting FMUs together,
...
_[Note, all output variables are listed here,
...
This is e.g. important for linearization,
...
_[Note, only continuous Real variables are listed here.
...
_[Note, higher order derivatives must be mapped to first order derivatives but the mapping definition can be preserved due to attribute `derivative`.
Example:_ latexmath:[\color{blue}{\frac{\text{ds}}{\text{dt}} = v,\ \frac{\text{dv}}{\text{dt}} =f(..)}] _,then_ latexmath:[\color{blue}{\left\{ v,\ \frac{\text{dv}}{\text{dt}} \right\}}] _is the vector of state derivatives and attribute `derivative` of_ latexmath:[\color{blue}{v}] _references_ latexmath:[\color{blue}{s}] _,
...
in other words cannot be computed _[which is the default.
...
(e.g. if a state is also an output, it is included only once in the list).
...
If an independent variable is not explicitely defined under `ScalarVariable`pass:[s],
it is assumed that the `Unknown` depends explicitely on the independent variable.
...
_[Note,_ latexmath:[\color{blue}{y = d \cdot u}] _where_ latexmath:[\color{blue}{d}] __changes only during Event Mode (__latexmath:[\color{blue}{d = 2 \cdot u}] _or_ latexmath:[\color{blue}{3 \cdot u\ }] _depending on relation_ latexmath:[\color{blue}{u > 0}] _that changes only at Event Mode).
...
_[Note,_ latexmath:[\color{blue}{y = c}] _where_ latexmath:[\color{blue}{c}] __changes only during Event Mode (__latexmath:[\color{blue}{c = 2}] _or_ latexmath:[\color{blue}{3\ }]__depending on relation__ latexmath:[\color{blue}{u > 0}] _that changes only at Event Mode).
...
_[Note, via the `dependencies` list the supported features of the FMU can be defined.
Examples:_
...
(e.g. to iterate over p such that the state derivative `der_x` is zero),
then the dependencies list of `der_x` should not include `p`.
If an FMU is imported in an environment and such an iteration is setup,
...
- _For standard CoSimulation FMUs it is common that no algebraic loops over the input/output variables nor over start-values is supported.
In such a case,
...
way (for example as tree and not as a linear list).
