...
_[An example of use for stream is a gas mixture flow. The gas composition could be implemented as a mass fraction vector. The `outStreamValueReference` refers to latexmath:[portA.q_\textit{outStream}\textit{[\]}] and the `inStreamValueReference` refers to latexmath:[portA.q_\textit{inStream}\textit{[\]}]. The `inStreamMemberName` and `outStreamMemberName` are "latexmath:[q_\textit{inStream}\textit{[\]}]" and "latexmath:[q_\textit{outStream}\textit{[\]}]"._
