...
If output clocks and <<inputClock,`input clocks`>> of clockType equals `CPClock` are defined in the modelDescription.xml it is possible to define a tick relationship from a output <<clock>> to an aperiodic <<inputClock>> based on `clockReference` (<<clock-relationships-for-communication-point-clocks>>).
This is done for aperiodic <<inputClock,`input clocks`>> that have the clockType = `CPClock` via providing a list of valueReferences of output clocks of the same FMU.
...
It is not allowed to combine output clocks with periodic or strict periodic <<inputClock,`input clocks`>> based on `clockReference`.
