...
The optional attribute <<relatedOutputClockReference>> is used to define a tick relationship from a <<outputClock>> to an aperiodic <<inputClock>> (see <<clock-relationships-for-communication-point-clocks>>).
...
If <<variability>> = <<clock>> the variable defines an <<inputClock, `input clock`>> that is controlled by the master.
...
If <<causality>> = <<output>> and <<variablity>> = <<clock>> the <<outputClock>> tick event can occur only in the model partition that is related to the referenced <<inputClock>>, i.e. the <<outputClock,`output clock's`>> value (active/inactive) can only change in this model partition.
