...
A clock is active at the current time instant if active[i] = fmi3True, otherwise the clock is not active. It is also allowed to call this function within the callback function _fmi3IntermediateUpdate()_.
