...
==== Definition of Clocks [[definition-of-clocks]]
...
|variability 
|Variables of type clocks require variability = ?clock? defined via the variability attribute of the element ?Variable?. 
It is not allowed to use the variability ?clock? with other available variable base types (e.g. fmi3Float64, fmi3Int32, ?). 
...
|initial
|If causality equals `inferred` the value equals `exact` and it is required to provide a start value for describing the expected initial contidion of master clocks for that FMU. 
The master can choose different start values if it is not possible to fulfill the conditions in a simulation setup. 
If causality equals `triggered` the initial attribute value equals `calculated` and no start value is provided.
If an inferred clock has `fmi3True` as an start value the master should activate the clock the first time it enters event mode. 
...
The ModelStructure includes the `DiscreteStates` element:
...
Set the clock activation status by providing the value references of the corresponding clock variables and their values. 
A clock is activated at the current time instant if active[i] = fmi3True, otherwise the clock is deactivated.
...
The parameter `subactive` has no meaning for CommunicationPointClocks (fmi3CPClock) and is ignored for such clocks.

:: _fmiStatus fmi3GetClock (fmi3Component c, const fmi3valueReference vr[],
                        size_t nvr, fmi3Boolean value[]);_
                        
Inquire whether a set of clocks is active by providing the value references of the corresponding clock varaibels. 
A clock is active at the current time instant if active[i] = fmi3True, otherwise the clock is not active. It is also allowed to call this function within the callback function _fmi3IntermediateStepFinished()_.  
Please note that it is required for an FMU to internally set back the activation state of a triggered clock to _fmi3False_ after _fmi3GetClock()_ is called for such a clock and _fmuType_ is set to _fmi3ScheduledExecutionSimulation_. 
See 4.4.5 
...
fmi3Status fmi3SetIntervalTYPE		 (fmi3Component c,  const fmi3ValueReference vr[], size_t nvr, fmi3UInt64 intervalCounter[], fmi3UInt64 resolution[]);

                         
Both functions set the interval value between the previous and the present tick of the clock. It is not allowed to call fmi3SetInterval for clock variables with attribute ?strict? equals true.

:: fmi3Status fmi3GetInterval(fmi3Component c, const fmi3valueReference vr[],
                         size_t nvr, fmi3Real interval[]);

typedef fmi3Status fmi3GetIntervalTYPE(fmi3Component c, const fmi3ValueReference vr[], size_t nvr, fmi3UInt64 intervalCounter[], fmi3UInt64 resolution[]); 
