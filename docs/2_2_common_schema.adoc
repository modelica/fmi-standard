...
latexmath:[{p_{\mathit{Pa}} = 10^5 p_{\mathit{bar}}}]

_and therefore, `factor = 1.0e5` and `offset = 0.0`._

_In the following table several unit examples are given (Note that if in column `exponents` the definition "latexmath:[{kgm^2 / s^2}]" is present, then the attributes of `<BaseUnit>` are: `kg=1, m=2, s=-2`):_
...
_[For example, if latexmath:[{T_K}] is the temperature value of `Unit.name` (in `K`) and latexmath:[{T_F}] is the temperature value of `<DisplayUnit>` (in `&#176;F`), then_
...
`= communicationPoint` <<clock,`clocks`>> define sampling points (i.e. communication points) for the variables of model partitions and are used in FMI for Co-Simulation and FMI for Scheduled Execution.
These clocks are ignored by Model Exchange.
...
Only <<clock,`clocks`>> with <<causality>> = <<input>> and <<clockType>> = <<communicationPoint>> can have this attribute.
...
|
[[periodic,`periodic`]]
Clocks can be periodic or aperiodic. If a <<clock>> is periodic, the attribute `periodic = true`.
...
The optional image file of the FMU icon is placed at the path `icons/icon.png` in the ZIP archive of the FMU. The terminals should not be visible in the image.
...
latexmath:[q_{i,\mathit{inStream}}] _is the convective quantity in case the matter flows into the FMU. Both variables are present in the terminal._
...
_[The order of painting of the `<TerminalGraphicalRepresentation>` of terminals on each level is equal to the order of appearance in the `<Terminal>` element. So graphical representations appearing first, are painted first, are behind graphical representations which appear below.]_
...
The algebraic relationship to the <<input,`inputs`>> is defined via the <<dependencies>> attribute of the <<output>> variables.

[[outputClock,`outputClock`]]
If <<variability>> = <<clock>> the variable defines an <<outputClock,`output clock`>> that is controlled by the FMU.

[[local,`local`]]
`= local`: Local variable that is calculated from other variables or is a continuous-time <<state>> or an event indicator (see <<model-structure>>).
It is not allowed to use the variable value in another model or FMU.
...
A continuous-time <<state>> or a event indicator must have <<causality>> = <<local>> or <<output>>, see also <<model-structure>>.
...
[[tunable,`tunable`]]
`= tunable`: The value of the variable is constant between external events (ME) and between communication points (CS and SE) due to changing variables with <<causality>> = <<parameter>> or <<input>> and <<variability>> = <<tunable>>.
Whenever a <<parameter>> or <<input>> signal with <<variability>> = <<tunable>> changes, an event is triggered externally (ME), or the change is performed at the next communication point (CS and SE) and the variables with <<variability>> = <<tunable>> and <<causality>> = <<calculatedParameter>> or <<output>> must be newly computed.
...
_[If the simulation algorithm notices a change in a discrete variable during <<intermediateUpdate>>, the simulation algorithm will delay the change, raise an event with early return and during the communication point it can change the <<discrete>> variable, followed by event handling.]_
...
Changes to <<continuous>> variables are only considered discrete during event handling.
Changes to <<continuous>> variables during <<intermediateUpdate>> and normal communication points (communication points without event handling) are considered continuous.
...
Only useful for Model Exchange FMUs, if only a Co-Simulation FMU, this attribute must not be present.
If Model Exchange and Co-Simulation are combined in one FMU, this attribute is ignored for Co-Simulation: +
...
If the <<clock>> referenced by <<clockReference>> is a synchronous clock (<<clockType>> = <<synchronousTime>>), the variable is a clocked variable associated uniquely with this <<clock>>.
It is not possible to associate more than one <<clock>> to a variable.

If the <<clock>> referenced by <<clockReference>> is a communication point clock (<<clockType>> = <<communicationPoint>>) the variable is not necessarily `clocked` in the sense of synchronous time clock theory. Such variables can also be continuous-time or discrete-time variables.

If <<causality>> = <<output>> and <<variability>> = <<clock>> the <<outputClock>> tick event can occur only in the model partition that is related to the referenced <<inputClock>>, i.e. the <<outputClock,`output clock's`>> value (active/inactive) can only change in this model partition.
...
|`intermediateUpdate`
|
[[intermediateUpdateAttr,`intermediateUpdate`]]
If this boolean attribute is `true`, the variable can be accessed during a communication step.
Variables with <<causality>> <<parameter>> must not be marked with <<intermediateUpdateAttr,`intermediateUpdate = true`>>.
...
Optional attribute defining the dependencies of the unknown latexmath:[{\mathbf{v}_{\mathit{unknown}}}] (directly or indirectly via auxiliary variables) with respect to latexmath:[{\mathbf{v}_{\mathit{known}}}].
...
Knowns latexmath:[{\mathbf{v}_{\mathit{known}}}] in *Event Mode* and *Continuous-Time Mode* (ME) and at communication points (CS and SE) for output (<<causality>> = `true`) and <<derivative>> variables:

- inputs (variables with <<causality>> = <<input>>)

- continuous-time states (variables referenced with <<derivative>>)

- the <<independent>> variable (<<causality>> = <<independent>>; _[usually time]_).
...
`Knowns` latexmath:[{\mathbf{v}_{\mathit{known}}}] in *Initialization Mode* (for <<initial-unknown, initial unknowns>>):
...
For Co-Simulation, if `providesDirectionalDerivatives = false`, then <<dependencies>> does not list the dependency on continuous-time.
...
If not present, it must be assumed that the unknown latexmath:[v_{\mathit{unknow}}] depends on the knowns latexmath:[v_{\mathit{known}}] without a particular structure.
...
Only for floating point type unknowns latexmath:[{v_{\mathit{unknown}}}] in *Event Mode* and *Continuous-Time Mode* (ME) and at communication points (CS and SE), and not for initial unknowns for *Initialization Mode*:
...
`=` <<tunable>>: tunable factor, latexmath:[{p \cdot v_{\mathit{known},i}}] where latexmath:[{p}] is an expression that is evaluated before <<fmi3ExitInitializationMode>> is called and in *Event Mode* due to an external event (ME) or at a communication point (CS and SE)
...
If <<eventIndicator>> = `true`, the variable is an event indicator.
In Model Exchange event indicators may only be accessed via <<fmi3GetEventIndicators>>.
|====
...
_[Note: For local and output signals and <<initial>> = <<exact>>, then the variable is explicitly set in *Initialization Mode*._
...
|_<<tunable,`Tunable`>> <<parameter>> (changing such a <<parameter>> triggers an external event (ME) or takes effect at the next communication point (CS and SE), and <<tunable>> <<calculatedParameter>>/<<output>>/<<local>> variables might change their values)._
...
|_<<tunable,`Tunable`>> <<dependenciesKind,`dependent`>> <<parameter>> (changing a <<parameter>> triggers an external event (ME) or takes effect at the next communication point (CS and SE), and <<tunable>> <<dependenciesKind,`dependent`>> <<parameter,`parameters`>> and <<tunable>> <<local>> variables might change their values)._
...
All continuous state variables must have a corresponding variable with attribute `derivative` in order that the continuous <<state,`states`>> are uniquely defined.
In Model Exchange these continuous states may only be accessed via <<fmi3GetContinuousStates>> and <<fmi3SetContinuousStates>>.
...
==== Definition of the Model Structure [[model-structure]]

The structure of the model is described by the <<dependencies>> and <<dependenciesKind>> attributes of the model variables.
This structure is with respect to the underlying model equations, independently how these model equations are solved.
_[This information can, for example, be used to compute efficiently a sparse Jacobian for simulation, or to utilize the <<input>> / <<output>> dependency in order to detect that in some cases there are actually no algebraic loops when connecting FMUs together.]_
Dependencies for event indicators are not provided.
_[The calculation of derivatives of event indicators is not provided.]_
_[For example, when exporting a model in more than one FMI type; then the model structure is identical in all cases._
...
The required part defines an ordering of the <<output,`outputs`>>, the (exposed) <<derivative,`derivatives`>>, the <<eventIndicator, event indicators>>, and the unknowns that are available during Initialization.
_[Therefore, when linearizing an FMU, every tool will use the same ordering for the <<output,`outputs`>>, <<state,`states`>>, and <<derivative,`derivatives`>> for the linearized model._
_The ordering of the <<input,`inputs`>> should be performed in this case according to the ordering in `<ModelVariables>`.]_
A Model Exchange FMU must expose all <<derivative,`derivatives`>> of its continuous-time <<state,`states`>> and <<eventIndicator, event indicators>> as model variables.
...
The model structure is described by the variables detailed in <<table-model-structure-elements>>.
The symbols of the mathematical equations describing the dependency are defined in <<math-model-exchange>>.
...
|Outputs
|All variables with <<causality>> = <<output>>.
_[Note that all <<output>> variables are listed here, especially <<discrete>> and <<continuous>> <<output,`outputs`>>._
_The ordering of the variables is defined by the exporting tool._
_Usually, it is best to order according to the declaration order in the source model, since then the order does not change if the declaration order of <<output,`outputs`>> in the source model is not changed._
...
Attribute <<dependencies>> defines the dependencies of the <<output,`outputs`>> on the knowns at the current super-dense time instant in *Event Mode* and in *Continuous-Time Mode* (ME) and at the current communication point (CS and SE).
...
The functional dependency is defined as (dependencies of variables that are fixed in *Event Mode* and *Continuous-Time Mode* and at communication points are not shown): +
latexmath:[{(\mathbf{y}_c, \mathbf{y}_d) := \mathbf{f}_{\mathit{output}}(\mathbf{x}_c, \mathbf{u}_c, \mathbf{u}_d, t, \mathbf{p}_{\mathit{tune}})}]

|Derivatives
|All variables with attribute <<derivative>>.
...
_Usually, it is best to order according to the declaration order of the <<state,`states`>> in the source model, since then the order of the derivatives does not change if the declaration order of states in the source model is not changed._
...
For Co-Simulation, elements derivatives are ignored, if `providesDirectionalDerivatives = false`, in other words, it cannot be computed.
_[This is the default._
_If an FMU supports more than Model Exchange, then the derivatives might be present, since they are needed for Model Exchange._
_If the above flag is set to `false` for the Co-Simulation cases, then the derivatives are ignored for Co-Simulation._
...
Attribute <<dependencies>> defines the dependencies of the state derivatives on the knowns at the current super-dense time instant in *Event Mode* and in *Continuous-Time Mode* (ME) and at the current communication point (CS and SE).
...
- all continuous-time <<state,`states`>> and all state derivatives with <<initial>> = <<approx>> or <<calculated>>.

Attribute <<dependencies>> defines the dependencies of the unknowns on the knowns in *Initialization Mode* at the initial time.
...
|Event Indicators
|
All variables with <<eventIndicator>> = `true`.
_[Note that only <<continuous>> floating point variables are listed here._
...
For Co-Simulation, variables with <<eventIndicator>> = `true` are ignored.
_[If an FMU supports both Model Exchange and Co-Simulation, then the event indicator variables might be present, since they are needed for Model Exchange.]_
...
_In such a case, all <<dependencies>> lists for <<output>> variables should be defined as empty lists defining that the setting of <<input,`inputs`>> and/or of <<start>> values does not influence the <<output,`outputs`>>._
