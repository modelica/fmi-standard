...
_This information can, for example, be used to increase numerical stability and accuracy by setting the corresponding bound for the relative error to zero (relative tolerance = 0.0), if the corresponding variable or an is a continuous <<state>> variable.]_
...
Variable aliases of numeric variables can optionally be defined with the attribute `negated = true` which indicates that the variable's value must be multiplied with `-1` to retrieve the value of the variable alias.
_[This allows for efficient storage of signals with the same absolute value but opposite sign.]_
