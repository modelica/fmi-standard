...
_[If an <<outputClock,`output clock`>>  depends on an <<inputClock,`input clock`>>, it means at clock ticks of the <<inputClock,`input clock`>> in *Event Mode*, an immediate clock tick may be programmed at <<outputClock,`output clock`>>. ]_
