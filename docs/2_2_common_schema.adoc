...
Instead, its value is initialized with <<fmi3SetupExperiment>> and after initialization set by <<fmi3SetTime>> for Model Exchange and by parameters `currentCommunicationPoint` and `communicationStepSize` of <<fmi3DoStep>> for Co-Simulation FMUs.
...
|_<<parameter,`Parameter`>> used in  `<Dimension>` element;  can be changed before initialization in *Configuration Mode* and in Reconfiguration Mode_
