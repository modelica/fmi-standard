...
FMI for Model Exchange provides a standardized interface for solving <<ordinary-differential-equation,ordinary differential equations>> with events (abbreviated as "hybrid <<ODE,ODE>>").
...
* explicit or implicit <<integration-algorithm,integration algorithms>> with constant or variable step size,
