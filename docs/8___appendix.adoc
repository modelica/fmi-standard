...
- Fixing ticket #47: +
In FMI 1.0 for Model Exchange the attribute `version` in `<fmiModelDescription>` was
...
Function `fmi2SetDebugLogging` has two new parameters to define the categories (from `<LogCategory>`) to be used in log messages.
...
Struct `fmi2CallbackFunctions` gets a new last parameter: +
`fmi2ComponentEnvironment instanceEnvironment` +
The first parameter of function <<logMessage>> is changed from type `fmi2Component`
...
- Fixing ticket #85: +
New parameter `noSetFMUStatePriorToCurrentPoint` to function `fmi2CompletedIntegratorStep`, similarly to `fmi2DoStep`, in order that the FMU can flush a result buffer if necessary.
...
TODO: FMU also goes by "slave". Should we unify this?
...
|_master algorithm_
|TODO (similar to co-simulation environment? Also used as "co-simulation master algorithm".)
...
TODO: A parameter can have two meanings: it's also being used to denote the value of an argument passed in a function call. As far as I see, the meaning is clear from the context.
...
|_target simulator_
|_TODO (The terms "simulation environment", "calling environment", "target environment", "target platform", "environment" are also used. We should unify these. Maybe distinguish between the software and the platform executing it.)
