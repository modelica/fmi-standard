...
|The time interval from the previous to the current Clock tick, specified differently for the different Clock types.
|===
...
is a time-based periodic Clock which is activated with an arbitrary, but fixed interval latexmath:[T_{\mathit{interval}}] starting after an arbitrary latexmath:[T_{\mathit{shift}}], both interval and shift can be changed by the importer.
...
is a time-based periodic Clock which is activated with an arbitrary and changeable interval latexmath:[T_{\mathit{interval}}] starting after an arbitrary latexmath:[T_{\mathit{shift}}], both interval and shift can be changed by the importer.
...
|The time interval from the current to the next Clock tick.
Must be retrieved using <<fmi3GetInterval>> in <<EventMode>> or <<ClockActivationMode>> if and only if the corresponding Clock was activated.

|===

Countdown aperiodic Clock::
is a time-based Clock whose next interval is not yet known right after the Clock was just activated, forcing the importer to call <<fmi3GetInterval>> in <<InitializationMode>>, in every <<EventMode>> and <<ClockUpdateMode>>.
