...
If some resource is required by the FMU but is not available, the FMU must log what resource failed, respecting the current logging settings, and return with error.
