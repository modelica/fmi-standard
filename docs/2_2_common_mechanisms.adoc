...
[NOTE]
====
When the qualifier `fmi3IntervalUnchanged` is returned, the corresponding entries in the counters and resolutions arrays, or in the intervals array, are not modified by `fmi3GetIntervalFraction` or `fmi3GetIntervalDecimal`, respectively. The FMI-importer should consider the values from a previous call to `fmi3GetIntervalFraction` or `fmi3GetIntervalDecimal` and may adjust them to account for any time increment.
====
