...
.2+|`input`
|`changing`
|[[changing,`changing`]]<<fmi3SetClock>> in *Event Mode*,
...
++++
\begin{align*}
t_0 &:= t_{\mathit{start}} + \mathbf{T}_{\mathit{shift}} \\
t_i &:= t_{i-1} + \mathbf{T}_{interval, i} \qquad i = 1,2,3,{...}
\end{align*}
++++
where:
...
//TODO indent sub-types of clock
...
Aperiodic clocks::
[[aperiodic-clocks,aperiodic clocks]]
are time-based clocks which have a nonconstant interval.
<<fmi3GetInterval>> must be called to retrieve the first interval in *Initialization Mode*.
Calling <<fmi3GetShift>> is not allowed.

Changing aperiodic clocks::
[[changing-aperiodic-clocks,changing aperiodic clocks]]
are time-based clocks where the next interval is unchangeably known right after the clock just ticked.
<<fmi3GetInterval>> must be called to retrieve the next interval in every *Event Mode* if and only if the corresponding clock ticked.

Countdown aperiodic clocks::
[[countdown-aperiodic-clocks,countdown aperiodic clocks]]
are time-based clocks where the next interval is not yet known right after the clock just ticked, forcing the importer to call <<fmi3GetInterval>> in every *Event Mode*.
...
The precise definitions are given in the mathematical descriptions of Model Exchange (<<table-math-model-exchange>>) and Co-Simulation (<<table-math-co-simulation>>).
