 ...
 * `fmi3IntervalNotYetKnown` useful only for `countdown` clocks that must be queried for the next interval at each Event Mode, regardless if that `countdown` clock was active,
 * `fmi3IntervalUnchanged` used if a previous call to <<fmi3GetIntervalDecimal>> already returned a value which has not changed since,
 * `fmi3IntervalChanged` is returned to indicate that the values for the interval have changed for this <<Clock>>.
