...
Clock variables synchronize events between importer and across FMUs, by
 - communicating exactly which specific event happens
 - and exactly at which time instant, independent from continuous time specified by the arguments of `fmi3SetTime` or `fmi3DoStep`.

FMI specifies clearly the behavior of a clocked FMU.
FMI does not specify how the importer uses this functionality.
This will allow different use cases to be implemented with different semantics.
