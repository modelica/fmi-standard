...
[cols="1,5"]
|===
|latexmath:[\mathbf{t}_{\mathit{start}}]
|The time instant at which the simulation starts.
...
Let latexmath:[\mathbf{k}] denote a given <<Clock>>.
If we neglect the type (input, output, time based, etc.) of <<Clock>>,
assume that there are no other events on the FMU,
and just focus on the times latexmath:[\mathbf{t}_0, \mathbf{t}_1, \ldots, ] at which latexmath:[\mathbf{k}] ticks,
then <<Clock>> latexmath:[\mathbf{k}] induces a discrete system, whose state advances at each tick of latexmath:[\mathbf{k}] at times latexmath:[\mathbf{t}_{i}] with latexmath:[i = 0,1,2,\ldots].
Such a system, also called the <<model-partition>> of latexmath:[\mathbf{k}], is commonly written as:

latexmath:[\begin{align*}
\mathbf{x}_k(\mathbf{t}_{i}) &= \mathbf{f}_k(\mathbf{x}_k(\mathbf{t}_{i-1}), \mathbf{u}(\mathbf{t}_i), \mathbf{t}_i) \\
\mathbf{y}_k(\mathbf{t}_{i}) &= \mathbf{g}_k(\mathbf{x}_k(\mathbf{t}_{i-1}), \mathbf{u}(\mathbf{t}_i), \mathbf{t}_i)
\end{align*}]

where:

. latexmath:[\mathbf{t}_i] denotes the simulation time of <<Clock>> latexmath:[\mathbf{k}] at time latexmath:[\mathbf{t}_{i}] and
. latexmath:[\mathbf{x}_k(\mathbf{t}_i)] denotes the state vector at the current <<Clock>> tick at time latexmath:[\mathbf{t}_{i}] and
. latexmath:[\mathbf{x}_k(\mathbf{t}_{i-1})] denotes the state vector at the previous <<Clock>> tick of latexmath:[\mathbf{k}] at latexmath:[\mathbf{t}_{i-1}],
...
. latexmath:[\mathbf{u}(\mathbf{t}_i)] denotes the input vector at time latexmath:[\mathbf{t}_{i}], and
...
_The name "model partition" refers to the fact that an model inside an FMU may declare multiple <<Clock,Clocks>>, each with its own partition of the overall model.]_

When latexmath:[\mathbf{k}] is active (i.e., ticking) invoking the FMI functions `fmi3Get{VariableType}` on variables in the output vector latexmath:[\mathbf{y}_k] will trigger the execution of the output latexmath:[\mathbf{g}_k] function to compute such variables.
Note that latexmath:[\mathbf{g}_k] only depends on the previous state vector latexmath:[\mathbf{x}_k(\mathbf{t}_{i-1})].
However, it may depend on the current state by composing latexmath:[\mathbf{f}_k] into latexmath:[\mathbf{g}_k]:

latexmath:[\mathbf{y}_k(\mathbf{t}_{i}) = \mathbf{g}_k(\mathbf{f}_k(\mathbf{x}_k(\mathbf{t}_{i-1}), \mathbf{u}(\mathbf{t}_i), \mathbf{t}_i), \mathbf{u}(\mathbf{t}_i), \mathbf{t}_i)].
...
Uninitialized clocked variables must be initialized in the corresponding <<Clock,Clock's>> first tick.
...
<<clocked-variable,Clocked variables>> can depend on multiple <<Clock,Clocks>>.
_[For example, a global counter could be incremented by multiple tasks, each controlled by a different Clock.]_

<<Clock,Clocks>> may depend on <<inputClock,input Clocks>>.
