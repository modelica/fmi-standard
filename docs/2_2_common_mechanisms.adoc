...
_[Note that even though <<fmi3GetInterval>> may be called at any time for every time-based Clock, a new interval is not necessarily provided._
...
is a time-based Clock whose next interval is not yet known right after the Clock just ticked, forcing the importer to call <<fmi3GetInterval>> in <<InitializationMode>> and in every <<EventMode>> and <<ClockUpdateMode>>.
...
*#ToDo#* Check style of keywords

*#ToDo#* Check also the definition of GetShift

*#ToDo#* When it is possible to have a new interval for tunable Clocks, should it also be possible to have new shifts?
...
^|_(d)_
|_The combination fixed Clock and fmi3IntervalChanged is not valid in states other than Initialization Mode, since intervals of fixed Clocks cannot change in super state <<Initialized>>._
...
^|_(1)_
|_In Initialization Mode, fmi3IntervalChanged signals that a new interval is provided that has not been known by the importer in advance, or that the interval of a previous call of fmi3GetInterval is overwritten._
...
_In this case, the new interval until the next <<Clock>> tick is relative to the time of the current <<EventMode>> or <<ClockActivationMode>> in contrast to the general definition of a periodic <<Clock>> interval, where the interval is defined as the time between consecutive <<Clock>> ticks._
...
^|_(8)_
|_In <<EventMode>> and <<ClockUpdateMode>>, for a countdown Clock, fmi3IntervalChanged is returned if the last interval given to the importer is no longer valid (i.e., the Clock needs to be re-scheduled), or if the interval has not been known yet._
