...
Input clocks::
[[inputClock, `input clock`]]All clocks with <<causality>> <<input>> are collectively called <<inputClock>>.

OutputClocks::
[[outputClock, `output clock`]]There is only one <<clock>> type that is an <<outputClock>>: The <<triggered>> <<outputClock>>.
We still refer to the as <<outputClock,output clocks>> for simplicity.
