...
The time instant of the first Clock tick is defined by a decimal (<<shiftDecimal>>) and optionally by a fraction (<<shiftNumerator>>, <<shiftDenominator>>).
+
...
is a time-based periodic clock that defines its <<intervalDecimal>> in the <<modelDescription.xml>>. Optionally a fractional representation for the interval using <<intervalNumerator>> and <<intervalDenominator>>, a time shift <<shiftDecimal>> and also its fractional representation defined by <<shiftNumerator>> and <<shiftDenominator>> can optionally be provided in the <<modelDescription.xml>>.
...
If the function is not called the value given in <<intervalDecimal,`intervalDecimal`>>, <<intervalNumerator,`intervalNumerator`>> and <<intervalDenominator,`intervalDenominator`>> is used.
...
If the function is not called, the value given in <<intervalDecimal,`intervalDecimal`>>, <<intervalNumerator,`intervalNumerator`>> and <<intervalDenominator,`intervalDenominator`>> is used by the importer.
...
The values of the arguments <<intervals>>, <<shifts>>, <<intervalNumerators>> / <<intervalDenominators>> and <<shiftNumerators>> / <<shiftDenominators>> refer to the unit of the <<independent>> variable.
...
[[intervalDenominators,`intervalDenominators`]]
* <<intervalDenominators>> are arrays of size `nValueReferences` holding the Clock intervals to be set.
...
<<qualifiers>> describes how to treat the <<intervals>>, <<intervalNumerators>> and <<intervalDenominators>> arguments and is defined as:
...
* <<intervalNumerators>> and <<intervalDenominators>> are arrays of size `nValueReferences` to retrieve the Clock intervals as a fraction `intervalNumerator / intervalDenominator`.
...
* <<shifts>> is an array of size `nValueReferences` to retrieve the Clock intervals.
...
[[shiftDenominators,`shiftDenominators`]]
* <<shiftDenominators>> are arrays of length `nValueReferences` that define the time of the first Clock tick (see <<shiftNumerator>> and <<shiftDenominators>>).
...
* <<shifts>>, <<shiftNumerators>> and <<shiftDenominators>> are arrays of length `nValueReferences` that define the time of the first Clock tick (see <<shiftNumerators>> and <<shiftDenominators>>).
