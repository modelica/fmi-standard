...
are time-based clocks witch have a constant interval during the execution, except `interval = tunable` which indicates changes when <<tunable>> parameters change this clock interval depends on.
...
// TODO: add API for GetShift/Offset
...
[[shift, `shift`]]
In case the first clock activation is shifted away from the start of the execution, the <<shift>> amount can be
retrieved from the FMU using the following functions[[fmi3GetShift,`fmi3GetShift`]]:
