...
Output clocks may also depend on input Clocks and other variables. Such dependencies are declared in the <<ModelStructure>>. A clock latexmath:[\mathbf{k}] depends on a clock or variable latexmath:[\mathbf{v}] if a tick or change of value in latexmath:[\mathbf{v}] may trigger a tick of latexmath:[\mathbf{k}], during the same super-dense time instant.
