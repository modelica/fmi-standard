...
To support the source code representation of FMUs, macros are provided in `fmi3Functions.h` to build the actual function names by using a function prefix that depends on how the FMU is shipped (<<SO,shared object>> or static library).
...
In Model Exchange, time is under the sole control of the <<importer>> and its <<integration-algorithm,integration algorithm>>.
The model itself receives the current time to be used in its computation with <<fmi3SetTime>>.
Time is not necessarily always advancing as <<solver,solvers>> might need to jump back and forth in time, for example, to localize events using <<eventIndicator, event indicators>>.
...
Numerical algorithms, for example, <<solver,solvers>> and optimizers, cannot guarantee these limits.
If a variable is outside of the bounds, the <<solver,solver>> tries to bring it back into the bounds.
As a consequence, calling <<get-and-set-variable-values,`fmi3Set{VariableType}`>> during an iteration of such a <<solver,solver>> might provide values that are not within defined min/max region.
...
- Numerical <<integrator,integrators>> of stiff methods need matrix latexmath:[{\frac{\partial \mathbf{f}}{\partial \mathbf{x}}}].
