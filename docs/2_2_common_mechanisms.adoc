...
Clocked variables latexmath:[\mathbf{v}_k] can acquire a new values and can be queried only when their Clock latexmath:[k] is active, except during <<InitializationMode>>.
...
_Note that it is the partial derivative and not the total derivative that is considered here._
_When computing the partial derivative with respect to the independent variable_ latexmath:[t] _of the function_
...
_its partial derivative with respect to_ latexmath:[t] _will be_
...
[latexmath]
++++
\frac{\partial \mathbf{f}}{\partial t} + \frac{\partial \mathbf{f}}{\partial \mathbf{x}} \mathbf{\dot{x}}+\frac{\partial \mathbf{f}}{\partial \mathbf{\dot{x}}} \mathbf{\ddot{x}}
++++
_]_
