...
==== Getting Derivatives of Continuous Output [[getting-output-derivatives]]

Function <<fmi3GetOutputDerivatives>>::

--
The n-th <<derivative,`derivatives`>> with respect to time of continuous <<output,`outputs`>> can be retrieved with <<fmi3GetOutputDerivatives>> to allow interpolation/extrapolation of connected input variables between communication points by the importer using <<fmi3CallbackIntermediateUpdate>>. +
