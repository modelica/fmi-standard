...
latexmath:[\begin{align*}
t_0 &:= t_{\mathit{start}} + T_{interval, 0} \\
t_i &:= t_{event} + T_{interval, i} \qquad i = 1,2,3,{...}
\end{align*}]
...
|latexmath:[T_{interval, 0} \geq 0]
|The time interval from latexmath:[t_{start}] to the first Clock tick.
Must be retrieved using <<fmi3GetInterval>> in <<InitializationMode>>.
...
latexmath:[\begin{align*}
(\mathbf{x}_k) &:= \mathbf{f}_{disc}(^\bullet{\mathbf{x}_k}, \mathbf{u}, t) \\
(\mathbf{y}_k) &:= \mathbf{f}_{event}(\mathbf{x}_k, \mathbf{u}, t)
\end{align*}]

where:

* latexmath:[\mathbf{f}_{disc}] denotes the state transition function and
* latexmath:[\mathbf{f}_{event}] denotes the output function of the <<model-partition>> of latexmath:[k].

When latexmath:[k] is active (i.e., ticking) invoking the FMI functions `fmi3Get{VariableType}` on variables in the output vector latexmath:[\mathbf{y}_k] will trigger the execution of the output function latexmath:[\mathbf{f}_{event}] to compute such variables.
...
In all tables describing the mathematical model of a state (e.g. <<table-math-model-exchange,ContinuousTimeMode>>), for notational convenience one function (e.g. latexmath:[\mathbf{f}_{cont}]) is defined to compute all outputs from all input arguments.
