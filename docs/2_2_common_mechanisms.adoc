...
<<fmi3GetInterval>> in both <<EventMode>> and <<ClockActivationMode>> if this Clock ticked
...
|The delay for the first Clock tick relative to latexmath:[t_{\mathit{start}}]. latexmath:[T_{\mathit{shift}}] is provided differently for the different Clock types, and can be set with <<fmi3SetShiftDecimal>> or retrieved from the FMU with <<fmi3GetShiftDecimal>> as floating point value, or as rational number using <<fmi3SetShiftFraction>> or <<fmi3GetShiftFraction>>.

|latexmath:[T_{\mathit{interval, i}}]
a|The time interval until the next Clock tick, provided differently for the different Clock types.
...
is a time-based periodic Clock which ticks with an arbitrary, but fixed interval latexmath:[T_{\mathit{interval}}] starting after an arbitrary latexmath:[T_{\mathit{shift}}].
...
is a time-based periodic Clock which ticks with an arbitrary and changeable interval latexmath:[T_{\mathit{interval}}] starting after an arbitrary latexmath:[T_{\mathit{shift}}].
...
Must be retrieved using <<fmi3GetInterval>> in <<EventMode>> or <<ClockActivationMode>> if the corresponding Clock ticked.

|===

Countdown aperiodic Clock::
is a time-based Clock whose next interval is not yet known to the FMU right after the Clock just ticked, forcing the importer to call <<fmi3GetInterval>> in <<InitializationMode>> and in every <<EventMode>> and <<ClockUpdateMode>>.
...
A Clock latexmath:[k] depends on a Clock or variable latexmath:[v] if a tick or the value of latexmath:[v] may cause a activation of latexmath:[k] during the same super-dense time instant.
...
In this case, the new interval until the next Clock tick is relative to the time of the current <<EventMode>> or <<ClockActivationMode>> in contrast to the general definition for a periodic Clock, where the interval specifies the time between consecutive ticks.
Subsequent Clock ticks follow the general interval definition for periodic Clocks until the tunable interval changes again.

^|(7)
|In super state <<Initialized>>, for a changing Clock, <<fmi3IntervalChanged>> is returned if and only if the Clock has just ticked.
