...
The actual values of the variables that are defined in the <<modelDescription.xml>> can be inquired with the following functions.
Restrictions for certain variable types, variable causalities and variable variabilities are described in the state machine and state descriptions (for example <<common-state-machine>>).
