...
The importer may use the return argument <<earlyReturnRequested>> of the callback <<fmi3CallbackIntermediateUpdate>> to signal the FMU to return early from the current <<fmi3DoStep>>.
...
_For <<triggered>> <<outputClock,output Clocks>>, the importer calls <<fmi3GetClock>> during <<ClockUpdateMode>>.]_
...
|<<fmi3SetClock>> in <<EventMode>>,
<<fmi3ActivateModelPartition>> in <<ClockActivationMode>>,
<<fmi3GetInterval>> in <<InitializationMode>>, in every <<EventMode>>.
In Scheduled Execution the FMU has to inform the importer by calling <<fmi3CallbackClockUpdate>> that the Clock is about to tick
...
|The time interval from the event time or the point in time when <<fmi3CallbackIntermediateUpdate>> is called (see <<intermediateUpdateTime>>) at which the argument <<qualifiers,`qualifiers`>> of <<fmi3GetInterval>> returns `fmi3IntervalChanged`.
...
The FMU signals that a <<countdown>> Clock is about to tick by calling <<fmi3CallbackClockUpdate>>.
