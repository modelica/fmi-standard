...
The value of a variable must be accessed with the respective `fmi3Get/Set{VariableType}` for the <<definition-of-model-variables,type defined>> in the <<modelDescription.xml>>.
For variables of type <<figure-schema-Enumeration,`Enumeration`>>, `fmi3SetInt64` and `fmi3GetInt64` must be used.
...
The question is how `fmi3Set{VariableType}`, `fmi3Get{VariableType}` shall utilize this definition.
...
As a consequence, calling `fmi3Get{VariableType}` during an iteration of such a solver might return values that are not in the defined min/max region.
...
<<fmi3GetInterval>> in <<ClockActivationMode>> after every call to <<fmi3ActivateModelPartition>>
...
latexmath:[\mathbf{T}_{interval, i}] can be set with <<fmi3SetIntervalDecimal>>) or get with <<fmi3GetIntervalDecimal>> as floating point value, or as rational number using <<fmi3SetIntervalFraction>> or <<fmi3GetIntervalFraction>>: latexmath:[\mathit{intervalCounter} / \mathit{resolution}] (see <<resolution>>).
...
are time-based clocks where the next interval is not yet known right after the clock just ticked, forcing the importer to call <<fmi3GetInterval>> in every <<EventMode>> and after every call to <<fmi3ActivateModelPartition>> in <<ClockActivationMode>>.
...
* latexmath:[\mathbf{v}_{\mathit{unknonwn}}] is the vector of unknown variables computed in the actual state:
...
* latexmath:[\mathbf{v}_{\mathit{known}}] is the vector of known <<input>> variables of function *f* that changes its value in the actual state.
...
* latexmath:[{\mathbf{v}_{\mathit{rest}}}] is the set of <<input>> variables of function *f* that either changes its value in the actual state, or do not change their values in this state, but change their values in other states _[for example, discrete-time <<input,`inputs`>> in <<ContinuousTimeMode>>]_.
