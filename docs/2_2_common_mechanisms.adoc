...
_For example, during <<ContinuousTimeMode>>, the partial derivate of a continuous-time output *y* with respect to discrete-time <<input,`inputs`>> is zero, because discrete-time <<input, `inputs`>> are constant between events._
