...
|The delay for the first Clock tick relative to latexmath:[t_{\mathit{start}}]. latexmath:[\mathbf{T}_{\mathit{shift}}] is defined differently for the different Clock types, and can be set with <<fmi3SetShiftDecimal>> or retrieved from the FMU with <<fmi3GetShiftDecimal>> as floating point value, or as rational number using <<fmi3SetShiftFraction>> or <<fmi3GetShiftFraction>>.
...
The next Clock tick at time instant latexmath:[t_i] is defined as: +
latexmath:[\begin{align*}
t_0 &:= t_{\mathit{start}} + \mathbf{T}_{interval, 0} \\
t_i &:= t_{i-1} + \mathbf{T}_{interval, i} \qquad i = 1,2,3,{...}
\end{align*}]
...
latexmath:[\begin{align*}
t_0 &:= t_{\mathit{start}} + \mathbf{T}_{interval, 0} \\
t_i &:= t_{event} + \mathbf{T}_{interval, i} \qquad i = 1,2,3,{...}
\end{align*}]
