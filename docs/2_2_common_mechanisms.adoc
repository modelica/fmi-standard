...
*#ToDo#* Add sentence that interval is defined differently for tunable Clocks (sometimes - when the interval changes)
...
*#ToDo#* Explain the meaning of "with specified interval" and link to the respective section

*#ToDo#* State that getInterval can be called for all Clock types (incl. settable)
...
*#ToDo#* Add tunable CLocks and fixed Clocks set by the importer to the intervalNotYetKnown qualifier (if getInterval is called before setInterval)
...
[[fmi3IntervalChanged,`fmi3IntervalChanged`]]
  * `fmi3IntervalChanged` is returned to indicate that the value for the interval has changed for this <<Clock>>.
  ...
  In Initialization Mode, the new <<Clock>> interval is relative to the Clock shift latexmath:[T_{\mathit{shift}}] (periodic Clocks) or to latexmath:[t_{\mathit{start}}] (aperiodic Clocks).
...
<<table-qualifiers-for-time-based-clocks>> shows the qualifiers that are allowed for different time-based Clock types.
For some combinations the FMU state is relevant.
If a cell has two values, the left value refers to <<InitializationMode>>, whereas the right value refers to <<EventMode>> and <<ClockActivationMode>>/<<ClockUpdateMode>>.
...
*#ToDo#*
_How to treat periodic Clocks: [...]]_
