...
Therefore, the importer must take the activation of the <<clocks>> declared via the <<clocks>> attribute into account when forming algebraic loop candidates across connected FMUs: a declared dependency between two clocked variables forms a loop candidate only if their Clocks can be active at the same super-dense time instant.
