...
*#ToDo#* Allow getInterval calls also for changing Clocks that do not currently tick, but check that everywhere else is defined that intervalChanged can only be returned, if the CLock ticked

*#ToDo#* Distinguish between allowed API calls and sensible API calls. Maybe state somewhere that in general getInterval is always allowed
