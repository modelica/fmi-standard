...
<<fmi3GetInterval>> in both <<EventMode>> and <<ClockActivationMode>> when this Clock ticked
...
|The time interval until the next Clock tick, provided differently for the different Clock types.
...
_[Note that even though <<fmi3GetInterval>> may be called at any event time for every time-based Clock, a new interval is not necessarily provided._
_For instance, if <<fmi3GetInterval>> is called within a previously returned interval, an FMU returning <<fmi3IntervalUnchanged>> signals that the previously provided interval is still valid._
_In Scheduled Execution, this means that the model partition must not be re-scheduled.]_
...
Must be retrieved using <<fmi3GetInterval>> in <<EventMode>> or <<ClockActivationMode>> when the corresponding Clock ticked.
...
[NOTE]
====
The Clocks API function argument names <<intervals>>, <<shifts>>, <<counters>> and <<resolutions>> are chosen to match the naming used in link:https://specification.modelica.org/maint/3.6/synchronous-language-elements.html#clock-constructors[Modelica Clock Constructors].
...
====
=======
_[The Clocks API function argument names <<intervals-set>>, <<shifts>>, <<counters>> and <<resolutions>> are chosen to match the naming used in link:https://specification.modelica.org/maint/3.6/synchronous-language-elements.html#clock-constructors[Modelica Clock Constructors]._
...
>>>>>>> 94939fe (Add more clarification on intervals)
...
^|_(a)_
|_The combination constant Clock and <<fmi3IntervalChanged>> is not valid, since the importer knows the interval and it cannot be changed by the FMU nor by the importer._

^|_(b)_
|_The combination fixed/tunable Clock with <<intervalDecimal,`intervalDecimal`>> specified and <<fmi3IntervalChanged>> does not make sense, since the interval is set by the importer and cannot be changed by the FMU._

^|_(c)_
|_fmi3IntervalNotYetKnown is only defined for countdown Clocks._
_For all other Clocks an interval must be known or be retrievable by the importer at any time._

^|_(d)_
|_The combination fixed Clock and <<fmi3IntervalChanged>> is not valid in states other than <<InitializationMode>>, since intervals of fixed Clocks cannot change in super state <<Initialized>>._
...
|====
|
|_Explanation of the allowed combinations_

^|_(1)_
|_In <<InitializationMode>>, <<fmi3IntervalChanged>> signals that a new interval is provided that has not been known by the importer in advance, or that the interval of a previous call of <<fmi3GetInterval>> is overwritten._

^|_(2)_
|_For constant Clocks and for fixed Clocks in super state <<Initialized>>, <<fmi3IntervalUnchanged>> is the only qualifier that can be returned, since the interval is already known by the importer and cannot be changed by the FMU nor by the importer._

^|_(3)_
|_For fixed/tunable Clocks with <<intervalDecimal,`intervalDecimal`>> not specified and for aperiodic Clocks, <<fmi3IntervalUnchanged>> is returned when a previous call of <<fmi3GetInterval>> already returned an interval, which is still valid._

^|_(4)_
|_For fixed/tunable Clocks with <<intervalDecimal,`intervalDecimal`>> specified, <<fmi3IntervalUnchanged>> is the only qualifier that can be returned, since the interval is set by the importer and cannot be changed by the FMU._

^|_(5)_
|_See <<fmi3IntervalNotYetKnown>>_.

^|_(6)_
|_For tunable Clocks with <<intervalDecimal,`intervalDecimal`>> not specified, <<fmi3IntervalChanged>> is returned when the last interval known by the importer is no longer valid, because the interval has changed due to the change of tunable parameters._
_In this case, the new interval until the next Clock tick is relative to the time of the current <<EventMode>> or <<ClockActivationMode>> in contrast to the general definition for a periodic Clock, where the interval specifies the time between consecutive ticks._
...
^|_(8)_
|_In <<EventMode>> and <<ClockUpdateMode>>, for a countdown Clock, <<fmi3IntervalChanged>> is returned if the last interval given to the importer is no longer valid (i.e., the Clock needs to be re-scheduled), or if the interval was not yet known to the FMU._
