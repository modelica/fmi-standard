...
latexmath:[\Delta T_{i} > 0] can be set with <<fmi3SetIntervalDecimal>>) or get with <<fmi3GetIntervalDecimal>> as floating point value, or as rational number using <<fmi3SetIntervalFraction>> or <<fmi3GetIntervalFraction>>: latexmath:[\mathit{intervalCounter} / \mathit{resolution}] (see <<resolution>>).
|===
