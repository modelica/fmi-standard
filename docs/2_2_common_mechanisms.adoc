...
The FMU may use <<earlyReturn>> argument of the <<fmi3DoStep>> function to tell the import that the FMU needs to return earlier, and the importer may use the callback <<fmi3CallbackIntermediateUpdate>> to signal the FMU that the latter should return earlier.
...
_Artificial algebraic loops (see left diagram of <<figure-connected-fmus>>) can be solved in the modes <<InitializationMode>>, <<EventMode>>, and <<ContinuousTimeMode>> by an appropriate sequence of `fmi3Set{VariableType}` and `fmi3Get{VariableType}` calls:_
...
_In <<StepMode>>, <<fmi3SetFMUState>> is required to restore the FMU state before the next iteration with `fmi3Set{VariableType}`, <<fmi3DoStep>>, and `fmi3Get{VariableType}` is executed._
...
Input clocks::
[[inputClock, `input clock`]]All clocks with <<causality>> <<input>> are collectively called <<inputClock, `input clocks`>>.
The importer will activate <<inputClock, `input clocks`>> using <<fmi3SetClock>>.
Contrary to the importer being the source of the clock activation, the definition of the clock timing comes from the FMU, either through the <<modelDescription.xml>> or calling <<fmi3GetInterval>>.

Output clocks::
[[outputClock, `output clock`]]There is only one <<clock>> type that is an <<outputClock>>: The <<triggered>> <<outputClock>>.
...
<<fmi3GetInterval>> in <<InitializationMode>> and in <<EventMode>> if and only if this clock ticked
...
<<fmi3GetInterval>> in <<InitializationMode>> and in every <<EventMode>>
...
|Delay for the first clock activation relative to latexmath:[\mathbf{t}_{\mathit{start}}], defined differently for the different clock types.
latexmath:[\mathbf{T}_{\mathit{shift}}] can be retrieved from the FMU with <<fmi3GetShiftDecimal>> as floating point value, or with <<fmi3GetShiftFraction>> as rational number: latexmath:[\mathit{shiftCounter} / \mathit{resolution}] (see <<resolution>>).
...
latexmath:[\mathbf{T}_{interval, i} > 0] can be set with <<fmi3SetIntervalDecimal>>) or get with <<fmi3GetIntervalDecimal>> as floating point value, or as rational number using <<fmi3SetIntervalFraction>> or <<fmi3GetIntervalFraction>>: latexmath:[\mathit{intervalCounter} / \mathit{resolution}] (see <<resolution>>).
|===

[[periodic-clock-ticks]]Periodic clocks::
are time-based clocks which have a constant interval, except `interval = tunable` which indicates changes when <<tunable>> parameters change this clock interval depends on.
...
The importer must use <<fmi3GetInterval>> to retrieve the clock interval in <<EventMode>>, if any of the <<tunable>> <<parameter, parameters>> it depends on was changed.
...
<<fmi3GetInterval>> must be called to retrieve the next interval in every <<EventMode>> if and only if the corresponding clock ticked.

[[countdown-aperiodic-clocks,countdown aperiodic clocks]]Countdown aperiodic clocks::
are time-based clocks where the next interval is not yet known right after the clock just ticked, forcing the importer to call <<fmi3GetInterval>> in every <<EventMode>>.
...
are activated with <<fmi3SetClock>> by the importer in <<EventMode>>.

Triggered output clocks::
are activated within the FMU and the importer must call <<fmi3GetClock>> in <<EventMode>>.
There are two mechanisms for an FMU to communicate to the importer that a triggered output clock is about to tick:
 * <<early-return,early return>> from a <<fmi3DoStep>> with <<clocksAboutToTick, `clocksAboutToTick == fmi3True`>>, and
...
A <<clock>> activates its specific model partition.
...
However, `fmi3Get{VariableType}` will return the value computed at the last clock activation time.
Before the first clock tick, clocked variables have their initial value.

The association between <<clocked-variable,clocked variables>> and their <<clock,clocks>> is defined by the <<model-dependencies,model dependencies>>.
<<clocked-variable,Clocked variables>> can only depend on one clock.

===== Scheduled Execution

An external scheduler needs to know when a <<triggered>> <<inputClock>> depends on a <<triggered>> <<outputClock>> of the same FMU.
...
This ordering is defined by the <<priority>> attributes of the <<inputClock>>.
...
The dependencies are encoded in the <<modelDescription.xml>> with the element <<ModelStructure,`<ModelStructure>`>> up to <<valueReference>> resolution.
...
** <<InitializationMode>>: The exposed unknowns listed as elements `<ModelStructure><InitialUnknown>` that have a floating point type.

** <<ContinuousTimeMode>> (Model Exchange): The continuous-time outputs and state derivatives (= the variables listed as elements `<ModelStructure><Output>` with a floating point type and <<variability>> = <<continuous>> and the variables listed as elements `<ModelStructure><Derivative>`).

** <<EventMode>> (Model Exchange): The same variables as in the <<ContinuousTimeMode>> and additionally variables listed as elements `<ModelStructure><Output>` with a floating point type and <<variability>> = <<discrete>>.

** <<StepMode>> (Co-Simulation): The variables listed as elements `<ModelStructure><Output>` with a floating point type and <<variability>> = <<continuous>> or <<discrete>>.
