...
|latexmath:[T_{interval, i}]
|The time interval until the next Clock tick, defined differently for the different Clock types.
latexmath:[T_{interval, i}] can be set with <<fmi3SetIntervalDecimal>> or retrieved with <<fmi3GetIntervalDecimal>> as floating point value, or as rational number using <<fmi3SetIntervalFraction>> or <<fmi3GetIntervalFraction>>.
...
t_i &:= t_{i-1} + T_{interval, i} \qquad i = 1,2,3,{...}
\end{align*}]
...
The next Clock tick at time instant latexmath:[t_i] is defined as: +
latexmath:[\begin{align*}
t_0 &:= t_{\mathit{start}} + T_{interval, 0} \\
t_i &:= t_{i-1} + T_{interval, i} \qquad i = 1,2,3,{...}
\end{align*}]
...
|latexmath:[T_{interval, i} \geq 0, \qquad i = 1,2,3,{...}]
...
latexmath:[\begin{align*}
t_0 &:= t_{\mathit{start}} + T_{interval, 0} \\
t_i &:= t_{\mathit{event}} + T_{interval, i} \qquad i = 1,2,3,{...}
\end{align*}]
...
|latexmath:[T_{interval, 0} \geq 0]
|The time interval from latexmath:[t_{\mathit{event}}] to the first Clock tick.
Must be retrieved using <<fmi3GetInterval>> in <<InitializationMode>>.
...
latexmath:[\begin{align*}
(\mathbf{x}_k) &:= \mathbf{f}_{\mathit{event}}(^\bullet{\mathbf{x}_k}, \mathbf{u}, t) \\
(\mathbf{y}_k) &:= \mathbf{f}_{\mathit{event}}(\mathbf{x}_k, \mathbf{u}, t)
\end{align*}]

where:

* latexmath:[\mathbf{f}_{\mathit{event}}] denotes the state transition function and
...
If <<providesEvaluateDiscreteStates>> is `false`, then latexmath:[\mathbf{f}_{\mathit{event}}] cannot be called explicitly using <<fmi3EvaluateDiscreteStates>>, but will be implicitly executed during latexmath:[\mathbf{f}_{\mathit{event}}].
When <<fmi3EvaluateDiscreteStates>> is called, the state vector latexmath:[\mathbf{x}_k] is updated according to state transition function latexmath:[\mathbf{f}_{\mathit{event}}].

_[Separating_ latexmath:[\mathbf{f}_{\mathit{event}}] _from_ latexmath:[\mathbf{f}_{\mathit{event}}] _allows manipulation of the current (discrete) state and compute the corresponding outputs, for instance for model-based control applications.]_

The discrete-time variables latexmath:[\mathbf{x}_k] and latexmath:[\mathbf{y}_k] are called clocked variables.
A clocked variable latexmath:[\mathbf{v}_k] can acquire a new value and can be queried only when its Clock latexmath:[k] is active, except during <<InitializationMode>>.
...
A Clock latexmath:[k] depends on a Clock or variable latexmath:[\mathbf{v}] if a tick or the value of latexmath:[\mathbf{v}] may trigger a tick of latexmath:[k] during the same super-dense time instant.
...
In all tables describing the mathematical model of a state (e.g. <<table-math-model-exchange,ContinuousTimeMode>>), for notational convenience one function (e.g. latexmath:[\mathbf{f}_{\mathit{event}}]) is defined to compute all outputs from all input arguments.
...
where latexmath:[\mathbf{v}_{\mathit{known}}] are the latexmath:[n] knowns, and latexmath:[h] are the latexmath:[m] functions to calculate the latexmath:[m] unknwon variables latexmath:[\mathbf{v}_{\mathit{unknwon}}]  from the knowns.
