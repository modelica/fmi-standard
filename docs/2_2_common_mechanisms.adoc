...
_[Output clocks are intended to enable an FMU to provide a trigger to the outside world. Note, that if intended to activate model partitions or a clock of the same FMU calling <<fmi3GetInterval>> and evaluating the <<qualifier>> is recommended. An example is provided in <<example-scheduled-execution>>]_
...
During <<ClockActivationMode>> an FMU may inform the external scheduler that a model partition of the same FMU has to be scheduled. 
To retrieve this information the importer calls <<fmi3GetInterval>> on all <<table-overview-clocks, `supporting clocks`>>.
<<qualifier,`qualifier == fmi3NewInterval`>> indicates that the corresponding <<clock>> has to be scheduled.
<<countdown, `countdown clocks`>> allow to define a delay for the trigger indicated by the returned <<interval>>.
The FMU may request to schedule another model partition even while currently a model partition is being executed.
In this case the FMU calls <<fmi3CallbackIntermediateUpdate>> and allows the importer to call <<fmi3GetInterval>> and initiate the scheduling of a new task.
