...
The value of a variable must be accessed with the respective <<get-and-set-variable-values,`fmi3Get/Set{VariableType}`>> (see also <<VariableType,`{VariableType}`>>) for the <<definition-of-model-variables,type defined>> in the <<modelDescription.xml>>.
...
An FMU may inform the external scheduler that a <<triggered>> <<inputClock>> depends on a <<triggered>> <<outputClock>> of the same FMU.
