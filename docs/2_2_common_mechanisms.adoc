...
A dependency declared in <<dependencies>> between <<clocked-variable,clocked variables>> only contributes to an algebraic loop while the variables involved can be accessed at the same super-dense time instant, that is, while their respective <<clocks>> are simultaneously active.
