...
The time instant of the first Clock tick is defined by a decimal (<<shiftDecimal>>) and optionally by a fraction (<<shiftNumerator>>, <<intervalAndShiftDenominator>>).
+
...
is a time-based periodic clock that defines its <<intervalDecimal>> in the <<modelDescription.xml>>. Optionally a fractional representation for the interval using <<intervalNumerator>> and <<intervalAndShiftDenominator>>, a time shift <<shiftDecimal>> and also its fractional representation defined by <<shiftNumerator>> and <<intervalAndShiftDenominator>> can optionally be provided in the <<modelDescription.xml>>.
...
If the function is not called the value given in <<intervalDecimal,`intervalDecimal`>>, <<intervalNumerator,`intervalNumerator`>> and <<intervalAndShiftDenominator,`intervalAndShiftDenominator`>> is used.
...
If the function is not called, the value given in <<intervalDecimal,`intervalDecimal`>>, <<intervalNumerator,`intervalNumerator`>> and <<intervalAndShiftDenominator,`intervalAndShiftDenominator`>> is used by the importer.
...
The values of the arguments <<intervals>>, <<shifts>>, and <<intervalNumerators>> / <<intervalAndShiftDenominators>> refer to the unit of the <<independent>> variable.
...
[[intervalNumerators,`intervalNumerators`]]
* <<intervalNumerators>> and

[[intervalAndShiftDenominators,`intervalAndShiftDenominators`]]
* <<intervalAndShiftDenominators>> are arrays of size `nValueReferences` holding the Clock <<intervalNumerator>> and <<intervalAndShiftDenominators>> to be set.
...
<<qualifiers>> describes how to treat the <<intervals>>, <<intervalNumerators>> and <<intervalAndShiftDenominators>> arguments and is defined as:
...
* <<intervalNumerators>> and <<intervalAndShiftDenominators>> are arrays of size `nValueReferences` to retrieve the Clock intervals as a fraction `intervalNumerator / intervalAndShiftDenominator`.
...
[[shiftNumerators,`shiftNumerators`]]
* <<shifts>>, <<shiftNumerators>> and <<intervalAndShiftDenominators>> are arrays of length `nValueReferences` that define the time of the first Clock tick (see <<shiftNumerator>> and <<intervalAndShiftDenominators>>).
...
* <<shifts>>, <<shiftNumerators>> and <<intervalAndShiftDenominators>> are arrays of length `nValueReferences` that define the time of the first Clock tick (see <<shiftNumerators>> and <<intervalAndShiftDenominators>>).
