...
----
 CALL(FMI3InstantiateCoSimulation(S,
    ...
    // record clocks and clocked variables before event update
    CALL(recordClocks(S, time, outputFile));
    CALL(recordClockedVariables(S, time, outputFile));
    ...
    do {
        ...
        // record clocks and clocked variables 
        CALL(recordClocks(S, time, outputFile));
        CALL(recordClockedVariables(S, time, outputFile));
