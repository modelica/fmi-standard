...
// a|previous time instant, either
//
// * left limit or
// * previous super-dense time instant or
// * for clocked variables the previous clock activation time.
...
|latexmath:[
^{\bullet}v(t) \Leftrightarrow\ ^{\bullet}v(\left( \mathbf{t}_R,\mathbf{t}_I \right)) \Leftrightarrow
\begin{cases}
 v(^-t) & \text{ if } v \text{ is continuous-time} \\
 v(\left( \mathbf{t}_R,\mathbf{t}_I - 1 \right)) & \text{ if } v \text{ is discrete-time} \\
 v \text{ at previous tick of C } & \text{ if } v \text{ is a clocked var. of clock C}
\end{cases}
]
a|previous value (depends on the type of variable)
|====
