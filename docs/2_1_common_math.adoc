...
The <<independent>> variable latexmath:[t \in \mathbb{T}] _[typically: time]_ is a tuple latexmath:[t = (\mathbf{t}_R,\mathbf{t}_I)], where latexmath:[\mathbf{t}_R \in \mathbb{R},\ \mathbf{t}_{I} \in \mathbb{N} = \{0, 1, 2, \ldots\}].
...
|`d`
|A discrete-time variable changes its value only at an event instant latexmath:[t = (\mathbf{t}_i,n)].
...
The communication step size is denoted as latexmath:[h_i = \mathbf{t}_{i+1} - \mathbf{t}_i] +
...
|At initialization or at an event instant, an FMU can define the next time instant latexmath:[\mathbf{T}_{\mathit{next}}], at which the next time event occurs (see also the definition of <<EventMode,events>>).
