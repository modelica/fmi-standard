...
The <<independent>> variable latexmath:[t \in \mathbb{T}] is a tuple latexmath:[t = (t_R,t_I)], where latexmath:[t_R \in \mathbb{R},\ t_{I} \in \mathbb{N} = \{0, 1, 2, \ldots\}].

NOTE: The independent variable is typically time.
...
\begin{cases}
 ...
 v \textsf{ at previous activation of k } & \textsf{ if } v \textsf{ is a clocked variable of clock k}
\end{cases}
]
a|previous value
|====
...
Events can be known beforehand (<<time-event,time events>>), or are defined implicitly (<<state-event,state events>> and <<step-event,step events>>), see below.
...
image::PieceWiseContinuousVariables.svg[width=400]
