...
With the early return feature, an FMU shall be able to signal output clock events or internal state changes, i.e., discontinuity events to the co-simulation master at any time (not only at the end of `{DOSTEP}` function calls). 
