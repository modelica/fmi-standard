...
NOTE: For example, FMUs containing ODEs and implementing FMI for Co-Simulation require to include an ODE solver inside the FMU to internally advance time between the communication points.
...
Which of the features above are supported by a specific FMU is defined by <<co-simulation-schema,capability flags>>.
