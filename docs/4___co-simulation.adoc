...
With the early return feature, an FMU shall be able to signal <<outputClock>> events or internal <<state>> changes, i.e., discontinuity events to the Co-Simulation master at any time (not only at the end of <<fmi3DoStep>> function calls).
...
In this way multiple event types and also <<outputClock>> ticks or interrupts can be supported based on the early return functionality and additional functionalities provided in the mode `fmi3ModeHybridCoSimulation`.

==== Clocked Co-Simulation

[[clocked-co-simulation]]
The mode `fmi3ModeHybridCoSimulation` provides functionalities to control and observe the ticking of <<clock,`clocks`>>.
For FMI for Co-Simulation, the ticking of a <<clock>> is interpreted as an activity of the associated model partition.
During simulation, the Co-Simulation master updates and manages values of <<input,`inputs`>> and <<output,`outputs`>> of FMUs and further models at communication points (CP) for each model partition.
...
|only if `canNotUseBasicCoSimulation = false`
