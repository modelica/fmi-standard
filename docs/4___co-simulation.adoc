...
With <<api-scheduled-execution-co-simulation,Scheduled Execution Simulation>> a simulation master can directly control the time of computation (i.e. scheduling) for such model partitions.
+
The Scheduled Execution Simulation interface addresses simulation use cases that require:
+
...
+
An FMU that supports <<api-scheduled-execution-co-simulation,Scheduled Execution Simulation>> will in most cases also support <<api-hybrid-co-simulation,Hybrid Co-Simulation>>.
+
