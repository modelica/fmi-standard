...
The master algorithm is responsible to advance the overall simulation time and trigger execution of synchronous and asynchronous external events for an FMU and to handle events (e.g. clock ticks) for the whole simulation system if signaled by the FMU to the master algorithm.
