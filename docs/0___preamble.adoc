...
_FMI for Co-Simulation (CS) (<<fmi-for-co-simulation>>)_
...
The master algorithm is responsible to advance the overall simulation time and trigger execution of synchronous and asynchronous external events for a set of FMUs and to handle events (e.g. clock ticks) for the whole simulation system if signaled by the FMUs to the master algorithm.
...
_FMI for Scheduled Execution Simulation (SES)_

The Scheduled Execution Simulation interface exposes individual parts of a model or tasks of a control algorithm to be called from an external scheduler of an importing tool's master algorithm.
That master algorithm scheduler is responsible for all exposed model parts of an FMU and trigger execution of synchronous and asynchronous model parts (e.g. tasks of a controller model) and to handle events (e.g. clock ticks) for the whole simulation system if signaled by the FMU to the master algorithm.
...
In the XML file, information about exposed model variables (e.g. parameters, inputs and outputs) and about the capabilities of the FMU are present, for example to characterize the ability of a Co-Simulation slave to support advanced master algorithms such as the usage of variable communication step sizes, higher order signal extrapolation, or others.
