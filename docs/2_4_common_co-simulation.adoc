...
In different contexts, the communication points latexmath:[tc_i], the communication steps latexmath:[tc_i \rightarrow tc_{\mathit{i+1}}] and the communication step sizes latexmath:[hc_i := tc_{\mathit{i+1}} - tc_i] are also known as sampling points (synchronization points), macro steps and sampling rates, respectively.
...
. functions to synchronize the simulation of all subsystems and to proceed in communication steps latexmath:[tc_i \rightarrow tc_{\mathit{i+1}}] from initial time latexmath:[tc_0 := t_{\mathit{start}}] to end time latexmath:[tc_N := t_{\mathit{stop}}]
...
The most common master algorithm stops at each communication point latexmath:[tc_i] the simulation (time integration) of all slaves, collects the outputs latexmath:[y(tc_i)] from all subsystems, evaluates the subsystem inputs latexmath:[u(tc_i)], distributes these subsystem inputs to the slaves and continues the (co-)simulation with the next communication step latexmath:[tc_i \rightarrow tc_{\mathit{i+1}} = tc_i + hc] with fixed communication step size latexmath:[hc].
In each slave, an appropriate solver is used to integrate one of the subsystems for a given communication step latexmath:[tc_i \rightarrow tc_{\mathit{i+1}}].
The most simple co-simulation algorithms approximate the (unknown) subsystem inputs latexmath:[u(t), (t > tc_i))] by frozen data latexmath:[u(tc_i)] for latexmath:[tc_i \leq t < tc_{\mathit{i+1}}].
...
- the ability to repeat a rejected communication step latexmath:[tc_i \rightarrow tc_{\mathit{i+1}}] with reduced communication step size,
...
. Whenever the simulation in a slave is interrupted, a new time value latexmath:[tc_{\mathit{i+1}}, tc_i \leq tc_{\mathit{i+1}} \leq t_{\mathit{stop}}], can be given to simulate the time subinterval latexmath:[tc_i < t \leq tc_{\mathit{i+1}}]

. The subinterval length latexmath:[hc_i] is the communication step size of the latexmath:[i^{th}] communication step, latexmath:[hc_i = tc_{\mathit{i+1}} - tc_i].
...
Each time an internal discontinuity or an event happens inside an FMU with capability flag <<canReturnEarlyAfterIntermediateUpdate,`canReturnEarlyAfterIntermediateUpdate == fmi3True`>>, the callback function <<fmi3CallbackIntermediateUpdate>> is called by the FMU.
...
If the FMU returns with `earlyReturn == fmi3True` from <<fmi3DoStep>> then <<intermediateUpdateTime>> is the internal simulation time of the FMU of the last <<fmi3CallbackIntermediateUpdate>> call that signaled <<canReturnEarly,`canReturnEarly == fmi3True`>>.
