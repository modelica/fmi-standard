...
* If <<intermediateStepFinished, `intermediateStepFinished == fmi3False`>>, the Co-Simulation master shall only use intermediate output variables to compute intermediate <<input>> variables at the current state. This must only be used on input variables with a communication delay, which must be greater than the internal solver step in the FMU.
