...
A Co-Simulation FMU can provide valid or temporary <<output>> variables at intermediate points between two consecutive communication points, or is able to accept new values for <<input>> variables at these intermediate points.

- Intermediate <<output>> variables are typically the result of an internal solver taking multiple integration steps between two consecutive communication points of a <<fmi3DoStep>>.
...
- Intermediate <<input>> variables for an FMU are available from the Co-Simulation master.
...
Especially this enables the same input approximation that was possible in FMI 2.0 with `fmi2SetInputDerivatives`, now evaluating the approximation polynomial and not within the FMU as in FMI 2.0,
...
* If <<intermediateVariableSetAllowed,`intermediateVariableSetAllowed == fmi3True`>>, the Co-Simulation master may provide intermediate <<input>> variables by calling `fmi3Set{VariableType}` for continuous variables with <<intermediateAccess,`intermediateAccess = true`>>.
...
* If <<intermediateStepFinished, `intermediateStepFinished == fmi3False`>>, intermediate outputs of the FMU that the master inquires with `fmi3Get{VariableTypes}` correspond to not yet finished steps of the internal solver of the FMU, which may still change in the future, because the solver might revoke these intermediate steps. +
_[So these outputs shall for example not be communicated to other connected FMUs.]_ +
_[The use case for this is to let the Master do some calculations for the FMU even for unfinished solver steps._
