...
* If <<intermediateVariableSetRequested,`intermediateVariableSetRequested == fmi3True`>>, the Co-Simulation master should provide intermediate <<input>> variables by calling `fmi3Set{VariableType}` for variables with <<intermediateAccess,`intermediateAccess = true`>>.
