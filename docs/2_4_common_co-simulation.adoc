...
* If <<intermediateStepFinished, `intermediateStepFinished == fmi3False`>>, the Co-Simulation master shall only use intermediate output variables to compute intermediate <<input>> variables at the current state without triggering any recalculation in connected FMUs. The intended use-case is that two FMUs, A and B, interchanges variables with a communication delay enforced by the master. Output variables from FMU A are used by the master to interpolate input variables for FMU B with a specified time delay. The master may also perform calculations using intermediate output variables from FMU B to compute its intermediate inputs. 
