...
The term "communication point" in FMI for Co-Simulation refer to the communication between subsystems in a co-simulation environment and should not be mixed with the output points for saving simulation results to file.
...
For all Co-Simulation interface types intermediate variable access can be activated, if the FMU and the master supports this.
