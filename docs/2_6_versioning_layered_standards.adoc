...
The FMI standard uses semantic version numbers, as defined in <<PW13>>, where the standard version consists of a triple of version numbers, consisting of major version, minor version, and patch version numbers, so that for a version number of `1.2.3`, `1` is the major version, `2` the minor version, and  `3` the patch version.
