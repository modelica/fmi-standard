...
The FMU signals its support for direct model partition scheduling in the <<modelDescription.xml>> via the element `<fmiModelDescription><ScheduledCoSimulation>`.
...
In Scheduled Co-Simulation the handling of <<fmi3CallbackIntermediateUpdate>> callbacks is the same as in <<fmi-for-hybrid-co-simulation>>.
...
==== State Machine for Scheduled Co-Simulation [[state-machine-scheduled-co-simulation]]
...
In Scheduled Co-Simulation the following functions must not be called: <<fmi3DoStep>>, <<fmi3NewDiscreteStates>>, <<fmi3SetClock>>, <<fmi3SetIntervalDecimal>>, <<fmi3SetIntervalFraction>>, <<fmi3EnterEventMode>>, <<fmi3EnterStepMode>> including all functions that are specific to the Model Exchange interface.
...
Can be called if no other task related to the computation of model partitions of this FMU is currently active or preempted and the prerequisites defined for these functions in Basic Co-Simulation are fulfilled.
...
Additionally to the generally forbidden functions in Scheduled Co-Simulation as listed above, the following functions must not be called in this state: <<fmi3ExitConfigurationMode>>, <<fmi3Instantiate>>, <<fmi3EnterInitializationMode>>, <<fmi3ExitInitializationMode>>.
...
.Scheduled Co-Simulation Example ThreeInputClocks
[[figure-scs_example]]
image::images/scs_example.png[width=90%, align="center"]
