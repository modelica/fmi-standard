...
#TODO: general question: do we need this info in the docu, it is visiable in the state diagram anyways. the spec is already quite long, such redundant info could be dropped#
...
#TODO: no rational given for the recommendation#
...
#TODO: should we really not allow it or just like for co-sim mention that values retrieved via get calls will only change after activating partition?
this would be more consistent with co-sim. Isn't a ActivateModelPartition call comparable to "fine-grained" DoStep call for a model partition?#
