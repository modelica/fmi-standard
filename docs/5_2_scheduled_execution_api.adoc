...
The values returned by a call <<get-and-set-variable-values,`fmi3Get`>> that is placed after <<fmi3ActivateModelPartition>> may return the values of the other model partition if that model partition has higher priority and preempted the task execution meanwhile. 
