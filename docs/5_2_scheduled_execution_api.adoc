...
+
_[If required, the FMU can internally derive the <<Clock>> interval_ latexmath:[T_{\mathit{interval, i}}] _based on the last <<Clock>> tick time_ latexmath:[t_{i-1}] _i.e. last <<activationTime>> for this <<clockReference>> (based on last <<fmi3ActivateModelPartition>> call).]_
