...
A model partition of a Scheduled Execution FMU calls <<fmi3CallbackClockUpdate>> to signal that a <<triggered, `triggered output`>> <<Clock>> ticked or a new interval for a <<countdown>> <<Clock>> is provided with <<clockHandlingRequested, `clockHandlingRequested == fmi3True`>>._
