...
Both, set and get operations must be atomic: the importer must guarantee that these functions are not interrupted by preemption.
This guarantees that the values returned are consistent across all <<valueReference,valueReferences>>.
