...
In <<ClockActivationMode>> the scheduler of the importer activates model partitions according to their respective Clock and priority defintions.
...
_To avoid data inconsistencies and safeguard predictable behavior with <<get-and-set-variable-values,`fmi3Get{VariableType}`>>, <<get-and-set-variable-values,`fmi3Set{VariableType}`>> a unique assignment of the respective variables to model partitions via its associated <<Clock>> is strongly recommended, see <<textExample, example>>.]_
