...
Each <<fmi3ActivateModelPartition>> call is now associated to the computation of a (publicly disclosed, externally controlled) model partition of the model and therefore to a single defined <<clocks, `clock`>> of <<causality, `causality == input`>>.
+
[source, C]
----
...
* [[clockReference,`clockReference`]] `clockReference`: <<valueReference>> of a <<clocks, `clock`>> of <<causality, `causality == input`>> which shall be activated
...
Calls are based on activations of <<clocks, `clocks`>> of <<causality, `causality == input`>>.
