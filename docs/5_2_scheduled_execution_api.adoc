...
_It is recommended, to call <<get-and-set-variable-values,`fmi3Set{VariableTypeExclClock}`>> and <<get-and-set-variable-values,`fmi3Get{VariableTypeExclClock}`>> in the same task as <<fmi3ActivateModelPartition>>.
