...
These <<inputClock,`input clock`>> activations can be based on <<clock>> activations from FMU external sources (e.g. <<outputClock,`output clocks`>> of other FMUs).
The <<inputClock>> activations can also be based on <<outputClock,`output clock`>> ticks of the same FMU.
Refer to <<math-scheduled-execution>> and <<clocks>>.
...
During the execution of the model partition of <<inputClock,`input clock`>> 10msClock the <<outputClock,`output clock`>> OutClock may tick and invoke the execution of model partition of aperiodic <<inputClock>> AperiodicClock.
