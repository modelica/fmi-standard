...
In this example <<inputClock,`input clock`>> AperiodicClock is triggered by <<outputClock,`output clock`>> outClock thus the master ensures the task associated to AperiodicClock is scheduled when <<fmi3CallbackIntermediateUpdate>> is called by the FMU and outClock has ticked.
...
In the context of the task beeing executed every 10 ms the FMU lets <<outputClock,`output clock`>> outClock tick and calls <<fmi3CallbackIntermediateUpdate>>.
