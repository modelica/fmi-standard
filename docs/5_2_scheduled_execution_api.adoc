...
By scheduling the exposed model partitions of an FMU and executing them at dedicated points in time it is the scheduler that defines how time progresses, see <<advancing-time>>.
...
The callback may be called from several model partitions.
