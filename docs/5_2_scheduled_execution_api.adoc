...
+
_[If required, the FMU can internally derive the <<Clock>> interval latexmath:[\mathbf{T}_\mathit{interval, i}] based on the last <<Clock>> tick time latexmath:[\mathbf{t}_\mathit{i-1}] i.e. last <<activationTime>> for this <<clockReference>> (based on last <<fmi3ActivateModelPartition>> call).]_
