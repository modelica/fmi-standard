...
Forbidden Function Calls::
The same functions which are not allowed to be called in *Clock Activation Mode* must not be called in this state.
Additionally the following functions must not be called <<fmi3ActivateModelPartition>>, <<fmi3EnterConfigurationMode>>, <<fmi3Terminate>>.
