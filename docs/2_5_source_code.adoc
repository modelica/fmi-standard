=== Definition of Source Code (fmiBuildConfiguration)

A source code FMU contains the sources of the model in the `sources` directory together with a `buidConfiguration.xml` that contains at least one `<BuildConfiguration>` element for the supported platforms.
The `<BuildConfiguration>` provides the necessary information to compile and link the sources of the model into a dynamic library or as part of an executable.
...
[source, xml]
----
include::examples/simple_build_configuration.xml
----

.Multiple complex build configurations
[source, xml]
----
include::examples/complex_build_configuration.xml
----
