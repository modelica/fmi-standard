...
|_argument_
|Refers to a function parameter. Not to be confused with _parameter_.
...
|_exporter_
|A program that creates and FMU.
...
Also called target simulator, simulation environment, environment, calling environment, (co-)simulation algorithm, integrator (in ME)
