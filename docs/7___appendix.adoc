...
For an _FMU_, the <<independent>> variable time latexmath:[t \in \mathbb{T}] is a tuple latexmath:[t = (t_R, n)] where latexmath:[t_R \in \mathbb{R}, n \in \mathbb{N} = \{0,1,2,\ldots\}].
...
|A moment in time, either a continuous-time instant latexmath:[t = t_R], or a super-dense time instant latexmath:[t = (t_R, n)], see also <<super-dense-time>>.
