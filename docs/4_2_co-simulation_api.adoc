...
* _The FMU~2~ unfortunately could not be interrupted before it reached_ latexmath:[t_{i+1}] _, which reauires a complete rollback and a new co-simulation from_ latexmath:[t_{i}] _to_ latexmath:[t^{'}_{i+1}] _._
* _FMU~3~ triggered the callback function to the co-simulation algorithm after passing_ latexmath:[t^{'}{i+1}] _, which led to a request by the co-simulation algorithm to immediately do an early return. The FMU interrupts its computation and a partial rollback and a new co-simulation from_ latexmath:[t_{i}] _to_ latexmath:[t^{'}_{i+1}] _is necessary._
* _In the case of FMU~4~, the FMU triggered the callback function before reaching_ latexmath:[t^{'}_{i+1}] _. The co-simulation algorithm requests FMU~4~ to do an early return at_ latexmath:[t^{'}{i+1}] _and the FMU will interrupt its current integration step at_ latexmath:[t^{'}{i+1}] _. No rollback is necessary for FMU~4~._
