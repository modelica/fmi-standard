...
Both arguments <<currentCommunicationPoint>> and <<communicationStepSize>> allow computing of_ latexmath:[t_{i+1}] _by one addition, avoiding accumulation of numerical errors.
...
+
NOTE: (e.g. integrator encountered division by zero).
+
