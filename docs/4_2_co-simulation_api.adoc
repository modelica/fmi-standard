...
#[TODO]: @Masoud: Should this be moved to section "==== Mode fmi3ModeHybridCoSimulation"???#
...
If the `earlyReturnTime` is not equal to the last signaled `intermediateUpdateTime`, the FMU must call `{CBIU}` with `canReturnEarly` equals `fmi3true` to transport the new internal simulation time via `intermediateUpdateTime` to the master before it is returning from `{DOSTEP}`.

The co-simulation master can now decide about a rollback of the FMU to reach the `earlyReturnTime` time if this is required. Note that `Event Mode` is not supported in `fmi3ModeCoSimulation`.
...
If the FMU signals a `earlyReturn` equals `fmi3True` after returning from `{DOSTEP}` then `{IUTIME}` is the internal simulation time of the FMU of the last `{CBIU}` call that signaled `canReturnEarly` equals `fmi3true`.
