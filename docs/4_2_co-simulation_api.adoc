...
These functions must be called only at the first activation of periodic output clocks.
...
These functions can be called for <<inputClock,`input clocks`>> after the first <<clock>> activation.
These functions can be called multiple times for <<periodic,`aperiodic`>> <<inputClock,`input clocks`>>.
