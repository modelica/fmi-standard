...
The co-simulation master can request the FMU to end the `{DOSTEP}` at a Newtonian time instant and do an early return by calling the  `{DER}` function inside the callback function `{CBIU}`.
...
===== Co-Simulation with Clock Support [[clocked-co-simulation-api]] 
...
===== Scheduled Execution Simulation [[scheduled-execution-simulation-api]] 
:stem: latexmath
...
===== Intermediate Variable Access [[intermediate-variable-access-api]] 
:DOSTEP: fmi3DoStep()
