...
In order to enable the slave to interpolate the <<continuous>> real <<input,`inputs`>> between communication steps, the derivatives of the <<input,`inputs`>> with respect to time can be provided.
Also, higher derivatives can be set to allow higher order interpolation.
Whether the slave is able to handle the derivatives of <<input,`inputs`>> is given by the unsigned integer capability flag MaxInputDerivativeOrder. It delivers the maximum order of the input derivative.
...
<<input,`Inputs`>> and their derivatives are set with respect to the beginning of a communication time step.

To allow interpolation/approximation of the real output variables between communication steps (if they are used as inputs for other slaves), the derivatives of the <<output,`outputs`>> with respect to time can be read.
Whether the slave is able to provide the derivatives of <<output,`output`>> is given by the unsigned integer capability flag `MaxOutputDerivativeOrder`.
...
For variables with `variability` unequal to <<constant>> and for which `initial` = <<exact>> or <<approx>>.
...
`fmi3Set{VariableType}`::
For variables with:
* `variability` unequal <<constant>> that have `initial` = <<exact>>, or
* `causality` = <<input>>, or
...
The `{CBIU}` callback allows internal events (e.g. associated to <<outputClock>> ticks) to be signaled from an FMU to the Co-Simulation master.
...
The `{STIUI}`  argument in the `{CBIU}` function is a structure which is used to signal <<outputClock>> ticks and internal events to the master.
...
If an event happens or a <<outputClock>> ticks, `{IUTIME}` is the time of event or <<outputClock>> tick.
If the FMU signals an `earlyReturn` = `fmi3True` and `fmi3OK`after returning from <<fmi3DoStep>> then `{IUTIME}` is the internal simulation time of the FMU of the last `{CBIU}` call that signaled `canReturnEarly` = `fmi3true`.
...
*	when `clocksTicked`  is `fmi3True`, it means that <<fmi3GetClock>> function should be called for gathering all <<clock>> related information about ticking <<outputClock,`output clocks`>> at `{IUTIME}`.
...
-	when _newDiscreteStatesNeeded_ = `true` it means that the master should stay in Event Mode and another call to <<fmi3NewDiscreteStates>> is required.

-	when _nextEventTimeDefined_ = `true` it means that an event time is available and the value is given by the `nextEventTime` value.
This is the case when the model can report in advance the accurate time of the next predictable time event.

-	when _terminateSimulation_ = `true` it means that the model wants to stop integration and the Co-Simulation master should call <<fmi3Terminate>>.
...
If an FMU for Co-Simulation declares <<clock,`clocks`>> and clocked variables in the _modelDescription.xml_ file, it supports <<clock,`clocks`>>.
...
If the flag `providesScheduledExecutionSimulation` = `false`, it is not allowed to set the Co-Simulation mode to `fmi3ModeScheduledExecutionSimulation`.
