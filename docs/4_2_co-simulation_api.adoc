...
Allowed Function Calls::
`fmi3Terminate()`, #[TODO] @Masoud some more?#
...
#[TODO] @Masoud: Shall pointer or dot dereferencing be used at `eventInfo->/.newDiscreteStatesNeeded`? -> Must be adapted in some more sections.#
...
`fmi3GetClock()`::
#[TODO] @Masoud: Recheck with section <<fmi-api-setting-getting-clock-activation-state>>.#
The status of triggered clocks should be retrieved by this function.
[line-through]#It is not allowed to call this function for inferred clocks.# 

`fmi3SetClock()`::
#[TODO] @Masoud: Recheck with section <<fmi-api-setting-getting-clock-activation-state>>.#
[line-through]#The status of inferred clocks is set by this function.#
[line-through]#It is not allowed to call this function for triggered clocks.#
For inferred clocks, `fmi3SetClock()` is called after entering event mode to set the activation status of clocks.
Repeated calls are allowed if recomputations of clock state are needed during event mode.
...
`fmi3GetClock()`::
#[TODO] @Masoud: Review#
When `intermediateUpdateInfo->clocksTicked` is `fmi3True`, the master should call this function for gathering all clock related information about ticking triggered clocks at `intermediateUpdateInfo->intermediateUpdateTime`.

#[TODO] @Masoud: In section <<state-intermediate-update-mode-co-simulation>> the allowed function call `fmi3DoEarlyReturn()` is not further described. Does it have the same purpose here? Should it be mentioned here?#
