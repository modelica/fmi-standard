...
The state machine mode <<StepMode>> is used to compute the values of all variables between communication points and events and to advance time in the FMU.
...
The FMU enters <<IntermediateUpdateMode>> by calling <<fmi3IntermediateUpdateCallback>> within <<StepMode>> and leaves the state machine mode towards <<StepMode>> when the function returns.
