...
In order to enable the slave to interpolate the <<continuous>> float <<input,`inputs`>> between communication steps, the <<derivative,`derivatives`>> of the <<input,`inputs`>> with respect to time can be provided.
...
Sets the n-th time <<derivative>> of float <<input>> variables.
...
To allow interpolation/approximation of the float output variables between communication steps (if they are used as inputs for other slaves), the <<derivative,`derivatives`>> of the <<output,`outputs`>> with respect to time can be read.
...
We consider two slaves, where both have one <<continuous>> float <<input>> and one <<continuous>> float <<output>> which are connected in the following way:
...
It is not allowed to call <<fmi3EnterEventMode>> or <<fmi3EnterStepMode>> for an FMU with interface `fmi3CoSimulation` or `fmi3ScheduledExecutionSimulation`.
...
.Calling sequence of clocked Co-Simulation interface.
...
==== Scheduled Execution Simulation [[api-scheduled-execution-co-simulation]]

This section contains the interface description for supporting the Scheduled Execution Simulation interface from a C program.
...
The Co-Simulation master signals to the FMU that it supports and has recognized the <<clock>> and model partition scheduling capabilities of the FMU via setting the Co-Simulation interface in `fmi3CoSimulationMode` to `fmi3ScheduledExecutionSimulation` in the struct `fmi3CoSimulationConfiguration` function <<fmi3Instantiate>>.

If the flag `providesScheduledExecutionSimulation = false`, it is not allowed to set the Co-Simulation interface to `fmi3ScheduledExecutionSimulation`.
...
===== Transfer of Input / Output Values and Parameters in fmi3ScheduledExecutionSimulation
...
In `fmi3ScheduledExecutionSimulation` the handling of <<fmi3CallbackIntermediateUpdate>> callbacks is the same as in section <<clocked-co-simulation>>.
...
===== Computation in fmi3ScheduledExecutionSimulation
...
===== State Machine for fmi3ScheduledExecutionSimulation
...
.Calling sequence of Scheduled Execution Simulation.
[caption="Figure 10: "]
image::images/state-machine-scheduled-execution-simulation.svg[width=80%, align="center"]

In Scheduled Execution Simulation the following functions must not be called: <<fmi3DoStep>>, <<fmi3NewDiscreteStates>>, <<fmi3SetClock>>, <<fmi3SetIntervalDecimal>>, <<fmi3SetIntervalFraction>>, <<fmi3DoEarlyReturn>>, <<fmi3EnterEventMode>>, <<fmi3EnterStepMode>> including all functions that are specific to the Model Exchange interface.

The states *Configuration Mode*, *Instantiated*, *Initialization Mode*, and *Terminated* are handled in the same way as in `fmi3CoSimulation`, with the exception that the generally not allowed functions of `fmi3ScheduledExecutionSimulation` must not be called.
...
Forbidden Function Calls::
Additionally to the generally forbidden functions in Scheduled Execution Simulation as listed above, the following functions must not be called in this state: <<fmi3ExitConfigurationMode>>, <<fmi3Instantiate>>, <<fmi3SetupExperiment>>, <<fmi3EnterInitializationMode>>, <<fmi3ExitInitializationMode>>, <<fmi3GetDoStepDiscardedStatus>>.
...
_This is especially the case for `fmi3ScheduledExecutionSimulation` in which multiple model partitions can be active at the same time._
...
Forbidden Function Calls::
Additionally to the generally forbidden functions in Scheduled Execution Simulation listed above the following functions must not be called in this state: <<fmi3EnterConfigurationMode>>, <<fmi3Instantiate>>, <<fmi3SetupExperiment>>, <<fmi3EnterInitializationMode>>, <<fmi3ExitInitializationMode>>, <<fmi3GetDoStepDiscardedStatus>>, <<fmi3ActivateModelPartition>>,`fmi3SetInputDerivatives`, <<fmi3GetDirectionalDerivative>>, `fmi3GetOutputDerivatives`, `fmi3Get{VariableTypes}`, <<fmi3DoEarlyReturn>>
...
Thus in Scheduled Execution Simulation a support for a correct handling of the preemption of model partition computations is required.
...
===== Pseudo-code Example for fmi3ScheduledExecutionSimulation
