...
* <<eventOccurred,`eventOccurred == fmi3True`>> indicates that an event occurred at <<lastSuccessfulTime>> and the co-simulation algorithm has to enter *Event Mode* and call <<fmi3NewDiscreteStates>> to gather related information about the event.

[[clocksTicked,`clocksTicked`]]
* <<clocksTicked,`clocksTicked == fmi3True`>> indicates that one or more clocks ticked at <<lastSuccessfulTime>> and <<fmi3GetClock>> function should be called to gather all information related to <<outputClock,`output clocks`>> that have the value `fmi3ClockActive`.
...
The FMU stops computation at the first encountered internal event (if any) and the event time is provided through the output argument <<lastSuccessfulTime>>, along with the reason <<eventOccurred, `eventOccurred == fmi3True`>>.
...
If an <<outputClock>> ticks, the FMU returns from <<fmi3DoStep>> with <<clocksTicked,`clocksTicked == fmi3True`>> and the co-simulation algorithm pushes the FMU into *Event Mode* by calling <<fmi3EnterEventMode>>.
...
<<fmi3GetClock>>::
The status of <<outputClock,`output clocks`>> and <<localClock,`local clocks`>> can be inquired by this function.
...
For <<outputClock,`output clocks`>> and <<localClock,`local clocks`>> it is allowed to call these functions during *Event Mode*.
These functions must be called only at the first activation of <<perodic>> <<outputClock,`output clocks`>>s.
For <<perodic,`aperiodic`>> <<outputClock,`output clocks`>>, these functions must be called at every activation.

<<fmi3SetIntervalDecimal>> & <<fmi3SetIntervalFraction>>::
These functions must be called only at the first activation of <<perodic>> <<inputClock,`input clocks`>>s.
These functions must be called for every activation of <<periodic,`aperiodic`>> <<inputClock,`input clocks`>>.
