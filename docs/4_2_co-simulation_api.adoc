...
_Especially aperiodic <<inputClock,`input clocks`>> can not always be sufficiently emulated in modes that do not directly support clocks._
...
If the Co-Simulation master supports clocks, all <<inputClock,`input clocks`>> of the model should be handled and <<inputClock>> events should be scheduled by the master.
...
This might not be possible if an aperiodic <<inputClock>> of an FMU depends on the ticking of an aperiodic output <<clock>> of another FMU or other aperiodic tick sources.]_
...
<<fmi3SetClock>>::
For <<inputClock,`input clocks`>>, <<fmi3SetClock>> is called after entering event mode to set the activation status of clocks.
...
For both output and <<inputClock,`input clocks`>> it is allowed to call these functions during event mode.
...
Set/get calls for each task are only allowed for variables that are associated to the <<inputClock>> associated to that task or – here preemption issues become important – to variables that are associated to no clocks (global variables), based on the XML information (see 4.4.3).
...
We consider one slave with three model partitions. Two model partitions associated to two periodic <<inputClock,`input clocks`>> (clock periods 1 ms and 2 ms) and one aperiodic <<input>> clock.
