...
If <<fmi3InstantiateCoSimulation>> signaled <<eventModeUsed,`eventModeUsed = fmi3True`>> (which in turn requires the FMU to declare `hasEventMode == true`), then <<fmi3EnterEventModeCS>> must be called before the first call to <<fmi3DoStep>>.
