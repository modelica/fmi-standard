...
If the master does not call <<fmi3DoEarlyReturn>> the FMU continues the <<fmi3DoStep>> computation until the next predefined communication instant (i.e., currentCommunicationPoint + communicationStepSize) or until the next call of `{CBIU}`.
