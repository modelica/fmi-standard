...
In this co-simulation mode the following functions must not be called: `fmi3ActivateModelPartition()`, `fmi3NewDiscreteStates()`, `fmi3EnterEventMode()`, {TODO ModelExchangeFunctions}
...
`fmi3Instantiate()`, `fmi3Terminate()`, `fmi3Get{VariableType}`, `fmi3GetDirectionalDerivatives()`, `fmi3GetRealOutputDerivatives()`, `fmi3DoStep()`, `fmi3SetClock()`, `fmi3GetClock()`, `fmi3DoEarlyReturn()`, `fmi3ExitInitializationMode()`, `fmi3GetDoStepDiscardedStatus()`, `fmi3ExitConfigurationMode()`
...
In between the FMU has to evaluate all relevant equations.
...
Forbidden Function Calls::
`fmi3Instantiate()`, `fmi3Terminate()`, `fmi3DoStep()`, `fmi3SetClock()`, `fmi3GetClock()`, `fmi3DoEarlyReturn()`, `fmi3EnterInitializationMode()`, `fmi3GetDoStepDiscardedStatus()`, `fmi3EnterConfigurationMode()`, `fmi3ExitConfigurationMode()`, `fmi3SetupExperiment()` 
...
Forbidden Function Calls::
`fmi3ExitConfigurationMode()`, `fmi3Instantiate()`, `fmi3SetupExperiment()`, `fmi3EnterInitializationMode()`, `fmi3ExitInitializationMode()`, `fmi3GetDoStepDiscardedStatus()`, `fmi3DoEarlyReturn()`
...
Allowed Function Calls::
`fmi3Set{VariableTypes}`, `fmi3Get{VariableTypes}` for variables that are marked with attribute "intermediateAccess" equals true in the modelDescription.xml.
`fmi3DoEarlyReturn()`
...
Forbidden Function Calls::
 `fmi3EnterConfigurationMode()`, `fmi3Instantiate()`, `fmi3SetupExperiment()`, `fmi3EnterInitializationMode()`, `fmi3ExitInitializationMode()`, `fmi3GetDoStepDiscardedStatus()`, `fmi3DoStep()`

Allowed Function Calls::
`fmi3Get{VariableTypes}`, `fmi3ExitConfigurationMode()`
...
The states `(Re-)Configuration Mode`, `instantiated`, `Initialization Mode`, `terminated`, `fatal` are handled in the same way as in mode `fmi3ModeCoSimulation` with the exception that the the following function calls functions must not be called in any state in this co-simulation mode: `fmi3DoStep()`, `fmi3SetClock()`, `fmi3SetIntervalDecimal()`, `fmi3SetIntervalFraction()`, `fmi3DoEarlyReturn()`, `fmi3EnterEventMode()`, `fmi3EnterStepMode()`.

`fmi3GetVersion()`, `fmi3SetDebugLogging()`::
Can be called at any time in `fmi3ModeScheduledExecutionSimulation`.
...
<<fmi3ActivateModelPartition>>::
If the function returns with `fmi3OK` or `fmi3Warning` the FMU stays in this state.
If the function returns with `fmi3Discard` or `fmi3Error` the FMU goes into state `terminated`.
If the function returns with `fmi3Fatal` the FMU goes into state `fatal`.
...
+
It is recommended to call `fmi3Set(Float32, Float64, Int8 ...)` and `fmi3Get(Float32, Float64, Int8 ...)` in the same task as `fmi3ActivateModelPartition()` for setting and retrieving variable values associated to a clock activation.

<<get-and-set-variable-values,fmi3Set{VariableType}>>, <<TODO,fmi3SetRealInputDerivatives>>::
...
+
The restrictions related to variable causality and variability defined for <<TODO,Step Mode>> in mode `fmi3ModeCoSimulation` apply.
+
If the function returns with `fmi3OK` or `fmi3Warning` the FMU stays in this state.
If the function returns with `fmi3Discard` or `fmi3Error` the FMU goes into state `terminated`.
If the function returns with `fmi3Fatal` the FMU goes into state `fatal`.
In case of `fmi3Fatal` the master can prematurely end all tasks related to the computation of model partitions of this FMU.
In case of `fmi3Discard` or `fmi3Error` the master must wait until all other tasks related to the computation of the model partitions of this FMU have ended, but no new tasks can be started (e.g. related to triggered clock tick) and all other function calls for this FMU must also return `fmi3Discard` or `fmi3Error` until the state `terminated` is reached.

<<get-and-set-variable-values,fmi3Get{VariableType}>>, <<TODO,fmi3GetRealOutputDerivatives()>>, <<TODO,fmi3GetDirectionalDerivative()>>::
...
If the function returns with `fmi3Fatal` the FMU goes into state `fatal`.
In case of `fmi3Fatal` the master can prematurely end all tasks related to the computation of model partitions of this FMU.
In case of `fmi3Discard` or `fmi3Error` the master must wait until all other tasks related to the computation of model partitions of this FMU have ended, but no new tasks can be started (e.g. related to triggered clock tick) and all other function calls for this FMU have to also return `fmi3Discard` or `fmi3Error` until the state `terminated` is reached.
...
The callback can be called from concurrent tasks.

`fmi3EnterConfigurationMode()`::
This function can only be called if the FMU contains at least one structural parameter and no other task related to the computation of model partitions of this FMU is currently active or preempted.
...
====== State: Intermediate Update Mode
The FMU enters this state by calling `fmi3CallbackIntermediateUpdate()` in state `Clock Activation Mode` and leaves the state towards state `Clock Activation Mode` if the function returns `fmi3OK` or `fmi3Warning`.  

Forbidden Function Calls::
The same functions which are not allowed to be called in `Clock Activation Mode` must not be called in this state. Additionally the following functions must not be called `fmi3ActivateModelPartition()`, `fmi3EnterConfigurationMode()`.   

Allowed Function Calls::
For a triggered clock only the first call of fmi3GetClock() for a specific activation of this clock signals `fmi3True`.
The FMU sets the reported activation state immediately back to `fmi3False` for the following fmi3GetClock() calls for that clock (in the same or other tasks) until this triggered clock is internally activated again.
...
Additionally to the generally forbidden functions in this co-simulation mode listed above the following functions must not be called in this state: `fmi3EnterConfigurationMode()`, `fmi3Instantiate()`, `fmi3SetupExperiment()`, `fmi3EnterInitializationMode()`, `fmi3ExitInitializationMode()`, `fmi3GetDoStepDiscardedStatus()`, `fmi3ActivateModelPartition()`,
//TODO-API: set/get!? other functions!?

Allowed Function Calls::

`fmi3ExitConfigurationMode()`::
Can be called at any time in this state.
