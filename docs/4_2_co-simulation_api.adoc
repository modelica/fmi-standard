...
The states *Configuration Mode*, *Instantiated*, *Initialization Mode*, *Terminated*, and *Fatal* are handled in the same way as in `fmi3CoSimulation`, with the exception that the generally not allowed functions of `fmi3ScheduledExecutionSimulation` must not be called.
