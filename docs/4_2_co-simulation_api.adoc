...
a|
* latexmath:[(\mathbf{y}_{c+d-k}, \mathbf{y}_c^{(j)}, \mathbf{x}_{d-k}, \mathbf{w}_{c+d-k}, \mathbf{b}, t_{i+1}) := \mathbf{f}_{\mathit{doStep}}({}^{\bullet}\mathbf{x}_d, \mathbf{u}_{c+d}, {}^{\bullet}\mathbf{u}_{c,u}, \mathbf{p}, {}^{\bullet}\mathbf{b}, t_i, h_i)] +
...
<<get-and-set-variable-values,`fmi3Get{VariableType}`>> does not trigger an evaluation of latexmath:[\mathbf{f}_{doStep}].
...
|Get time derivatives of <<continuous>> <<output,`outputs`>> latexmath:[\mathbf{y}_{c,u}^{(j)}(\mathbf{t_{u}})].
|<<fmi3GetOutputDerivatives>>
