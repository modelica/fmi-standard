...
If the co-simulation algorithm signals the support for intermediate update and an FMU has at least one variable with <<intermediateAccessAttr,`intermediateUpdate = true`>>, the FMU can use the callback function <<fmi3CallbackIntermediateUpdate>> to communicate information back to the co-simulation algorithm:
