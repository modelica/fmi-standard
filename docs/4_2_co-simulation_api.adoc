...
The states *Configuration Mode*, *Instantiated*, *Initialization Mode*, and *Terminated* are handled in the same way as in mode `fmi3ModeCoSimulation` with the exception that the generally not allowed functions of the Co-Simulation mode `fmi3ModeScheduledExecutionSimulation` must not be called.
