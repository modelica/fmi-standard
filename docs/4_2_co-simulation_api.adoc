...
`fmi3Set{VariableTypes}()`, `fmi3Get{VariableTypes}()`, `fmi3SetInputDerivatives()`, `fmi3GetOutputDerivatives()`::
...
====== State: Step Failed [[state-step-failed-co-simulation]]
...
Allowed Function Calls::
`fmi3Terminate()`, `fmi3EnterStepMode()`, `fmi3Get{VariableType}`, `fmi3GetDirectionalDerivatives()`, `fmi3GetOutputDerivatives()`
...
The co-simulation master is only allowed to call this function, if the boolean variable `fmi3IntermediateUpdateInfo->canReturnEarlyAfterIntermediateUpdate` is `fmi3True`.
...
If the FMU signals a `earlyReturn` equals `fmi3True` the internal simulation time of the FMU equals the value of `intermediateUpdateTime` of the last `{CBIU}` call that signaled `canReturnEarlyAfterIntermediateUpdate` equals `fmi3true`.

If the `earlyReturnTime` is greater than or equal to the last signaled `intermediateUpdateTime`, the FMU may integrate up to the time instant  `earlyReturnTime`  and call `{CBIU}` with `canReturnEarlyAfterIntermediateUpdate` equals `fmi3true` to transport the new internal simulation time via `intermediateUpdateTime` to the master before it is returning from `{DOSTEP}`.
...
If the FMU signals an `earlyReturn` equals `fmi3True` after returning from `{DOSTEP}` then `{IUTIME}` is the internal simulation time of the FMU of the last `{CBIU}` call that signaled `canReturnEarlyAfterIntermediateUpdate` equals `fmi3true`.
...
*   when `canReturnEarlyAfterIntermediateUpdate` is `fmi3True` the master can request the FMU to return early at the current `{IUTIME}` time instant via calling `{DER}` within the callback function `{CBIU}`.
If `canReturnEarlyAfterIntermediateUpdate` is `fmi3False` the FMU will not do the early return, regardless of the master request.
...
#[TODO]: Recheck if this is the correct position. --> If this is mandatory, should it be placed in section <<api-intermediate-variable-access>>?#
...
`fmi3EnterStepMode()`::
Once all events are handled and `fmi3NewDiscreteStates()` returns with `eventInfo.newDiscreteStatesNeeded` equals to false, the FMU should return back to the Step Mode.
Unless the FMU requests to terminate the co-simulation by setting  `fmi3EventInfo->terminateSimulation` to true.
...
`fmi3GetClock()`::
# Masoud: To be discussed. Not sure if this function is needed inside the callback.#
When `intermediateUpdateInfo->clocksTicked` is `fmi3True`, the master should push the FMU into the event mode. 
In the Event mode, the master should call this function for gathering all clock related information about ticking triggered clocks at `intermediateUpdateInfo->intermediateUpdateTime`.

`fmi3DoEarlyReturn()`::
`intermediateUpdateInfo->canReturnEarlyAfterIntermediateUpdate` is `fmi3True`, the co-simulation master can request the FMU to end the `fmi3DoStep()` and to do an early return by calling this function.
...
`fmi3GetXXX()`::
if `intermediateUpdateInfo->intermediateVariableGetAllowed` is equal to `fmi3True`, the value of intermediate variables can be inquired.
...
====== State: Step Failed
This state in hybrid co-simulation does not differ from co-simulation described in section <<state-step-failed-co-simulation>>.
