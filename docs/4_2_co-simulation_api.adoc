...
Compared to FMI for Model Exchange, <<fmi3ExitInitializationMode>> enters directly into *Step Mode*.
This was designed in case the Co-Simulation FMU does not support *Event Mode*.
Importers can directly switch a Co-Simulation FMU into *Event Mode* without calling <<fmi3DoStep>> to handle events immediately after initialization.
...
Therefore, <<algebraic-loops,algebraic loops>> at communication points cannot be handled by an appropriate sequence of `fmi3Get{VariableType}` and `fmi3Set{VariableType}` calls _[contrary to <<InitializationMode>>, <<EventMode>> and <<ContinuousTimeMode>>]_.
