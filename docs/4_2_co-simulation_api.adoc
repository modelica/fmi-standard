...
Only after `fmi3DoStep()` returns `earlyReturn = fmi3True` a `fmi3NewDiscreteStates()` call returns `fmi3EventInfo` structure. When `eventInfo.newDiscreteStatesNeeded = fmi3True` it means that the master should stay in Event Mode and another call to `fmi3NewDiscreteStates()` is required. 
#[TODO]: @Masoud: Revise. Taken from <<api-hybrid-co-simulation>>#
