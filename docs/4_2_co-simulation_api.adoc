...
a|
* latexmath:[(\mathbf{y}_{c+d-k}, \mathbf{y}_c^{(j)}, \mathbf{x}_{d-k}, \mathbf{w}_{c+d-k}, \mathbf{b}, t_\mathit{i+1}) := \mathbf{f}_{\mathit{doStep}}({}^{\bullet}\mathbf{x}_d, \mathbf{u}_{c+d}, {}^{\bullet}\mathbf{u}_{c,u}, \mathbf{p}, {}^{\bullet}\mathbf{b}, t_i, \mathbf{h}_i)] +
...
|latexmath:[(\mathbf{v}_{u-k}, t_u) := \mathbf{f}_{\mathit{inter}}({}^{\bullet}\mathbf{x}_d, \mathbf{u}_{c+d}, {}^{\bullet}\mathbf{u}_{c,u}, \mathbf{p}, {}^{\bullet}\mathbf{b}, t_i, \mathbf{h}_i)]
...
* [[communicationStepSize,`communicationStepSize`]] `communicationStepSize` is the communication step size (latexmath:[\mathbf{h}_\mathit{i}]).
<<communicationStepSize>> must be latexmath:[> 0.0].
The FMU is expected to compute until time latexmath:[t_{i+1} = t_\mathit{i} + \mathbf{h}_\mathit{i}].
