...
<<input,`Input`>> and <<output>> variables and other variables are accessed via the `fmi3Get{VariableTypes}` and `fmi3Set{VariableTypes}` functions, defined in <<get-and-set-variable-values>>.

In order to enable the slave to interpolate the <<continuous>> real <<input,`inputs`>> between communication steps, the <<derivative,`derivatives`>> of the <<input,`inputs`>> with respect to time can be provided.
...
To allow interpolation/approximation of the real output variables between communication steps (if they are used as inputs for other slaves), the <<derivative,`derivatives`>> of the <<output,`outputs`>> with respect to time can be read.
...
Allowed Function Calls::
<<fmi3Terminate>>, `fmi3Get{VariableType}`, <<fmi3GetDirectionalDerivative>>, <<fmi3GetOutputDerivatives>>
...
[[fmi3IntermediateUpdateInfo,`fmi3IntermediateUpdateInfo`]]
[source, c]
----
...
==== fmi3HybridCoSimulation [[api-hybrid-co-simulation]]
...
The <<fmi3CallbackIntermediateUpdate>> callback allows internal events (e.g. associated to <<output-clock>> ticks) to be signaled from an FMU to the Co-Simulation master.
...
The <<fmi3IntermediateUpdateInfo>>  argument in the <<fmi3CallbackIntermediateUpdate>> function is a structure which is used to signal <<output-clock>> ticks and internal events to the master.
...
If an event happens or a <<output-clock>> ticks, `intermediateUpdateTime` is the time of event or <<output-clock>> tick.
...
* when `clocksTicked`  is `fmi3True`, it means that <<fmi3GetClock>> function should be called for gathering all <<clock>> related information about ticking <<output-clock,`output clocks`>> at `intermediateUpdateTime`.
...
The Co-Simulation master signals to the FMU that it supports and has recognized the early return Co-Simulation capabilities of the FMU by setting the CoSimulation mode to `fmi3HybridCoSimulation` in the <<fmi3Instantiate>> function.
...
It is not allowed to call <<fmi3EnterEventMode>> or <<fmi3EnterStepMode>> for an FMU with interface `fmi3CoSimulation` or `fmi3ScheduledExecutionCoSimulation`.

===== State Machine of Calling Sequence from Master to Slave in fmi3HybridCoSimulation
...
The Co-Simulation master should indicate the FMU that the master has recognized the <<clock>> capabilities of the FMU and supports the <<clock>> handling by setting the Co-Simulation mode to `fmi3HybridCoSimulation` in <<fmi3Instantiate>>.
...
_Especially aperiodic <<input-clock,`input clocks`>> can not always be sufficiently emulated in modes that do not directly support <<clock,`clocks`>>._
...
If the Co-Simulation master supports <<clock,`clocks`>>, all <<input-clock,`input clocks`>> of the model should be handled and <<input-clock>> events should be scheduled by the master.
If an <<output-clock>> ticks, the FMU invokes the <<fmi3CallbackIntermediateUpdate>> callback and sets `intermediateUpdateInfo.clocksTicked` to `fmi3True`.
...
Once the FMU is in *Event Mode*, the activation status of <<output-clock,`output clocks`>> are retrieved by <<fmi3GetClock>> function.
Then <<fmi3SetClock>> (and <<fmi3SetIntervalDecimal>> or <<fmi3SetIntervalFraction>> if necessary) should be invoked to enable the ticked <<input-clock,`input clocks`>>.
Each <<clock>>, that ticks outside of the FMU (i.e. <<input-clock>>), is activated for an FMU based on its <<clockReference>> and an associated <<fmi3SetClock>> in *Event Mode*.
...
Similar to FMI for Model Exchange, in order to activate <<input-clock,`input clocks`>> of an FMU, it is required to push the FMU into *Event Mode* by calling <<fmi3EnterEventMode>>.
...
In order to retrieve the status of <<output-clock,`output clocks`>>, <<fmi3GetClock>> and <<fmi3GetIntervalDecimal>> or <<fmi3GetIntervalFraction>> need to be called in the *Event Mode*.
...
Note that it is not allowed to call <<fmi3EnterEventMode>> or <<fmi3EnterStepMode>> in `fmi3CoSimulation` or `fmi3ScheduledExecutionSimulation`.

_[Usually the Co-Simulation master should be able to derive (but is not forced to do so) the correct communication point times for <<input-clock,`input clocks`>> in advance and thus it should be able to set the proper `communicationStepSize` for <<fmi3DoStep>>._
_This might not be possible if an aperiodic <<input-clock>> of an FMU depends on the ticking of an aperiodic <<output-clock>> of another FMU or other aperiodic tick sources.]_
...
<<fmi3SetClock>>::
For <<input-clock,`input clocks`>>, <<fmi3SetClock>> is called after entering *Event Mode* to set the activation status of <<clock,`clocks`>>.
...
For both <<output-clock,`output clocks`>> and <<input-clock,`input clocks`>> it is allowed to call these functions during *Event Mode*.
...
==== Scheduled Execution Co-Simulation [[api-scheduled-execution-co-simulation]]
...
The FMU signals its support for direct model partition scheduling in the `modelDescription.xml` via the flag `providesScheduledExecutionSimulation = true`.
The Co-Simulation master signals to the FMU that it supports and has recognized the <<clock>> and model partition scheduling capabilities of the FMU via setting the Co-Simulation mode in `fmi3CoSimulationMode` to `fmi3ScheduledExecutionCoSimulation` in the struct `fmi3CoSimulationConfiguration` function <<fmi3Instantiate>>.

If the flag `providesScheduledExecutionSimulation = false`, it is not allowed to set the Co-Simulation mode to `fmi3ScheduledExecutionCoSimulation`.

If no <<input-clock,`input clocks`>> are defined by the FMU it is not allowed to set `providesScheduledExecutionSimulation` to `true` in the `modelDescription.xml`.
...
===== Transfer of Input / Output Values and Parameters in fmi3ScheduledExecutionCoSimulation
...
In `fmi3ScheduledExecutionCoSimulation` the handling of <<fmi3CallbackIntermediateUpdate>> callbacks is the same as in section <<clocked-co-simulation>>.
...
===== Computation in fmi3ScheduledExecutionCoSimulation

If this mode is set, the master has to directly control the time of computation of model partitions associated to <<input-clock,`input clocks`>>.
The activation states of <<output-clock,`output clocks`>> are transported via <<fmi3CallbackIntermediateUpdate>> and <<fmi3GetClock>>.
...
Calls are based on ticks of periodic or aperiodic <<input-clock,`input clocks`>>.
These <<input-clock>> ticks can be based on <<clock>> ticks from FMU external sources (e.g. <<output-clock,`output clocks`>> of other FMUs) or other external <<clock,`clocks`>>/events/interrupts assigned via the simulation master configuration (such external events can be based on a potentially unpredictable process or can be just simulation time based events e.g. the planned communication point).
The <<fmi3ActivateModelPartition>> function is not called for <<output-clock,`output clocks`>> of an FMU.
...
===== State Machine for fmi3ScheduledExecutionCoSimulation
...
The states *Configuration Mode*, *Instantiated*, *Initialization Mode*, *Terminated*, and *Fatal* are handled in the same way as in `fmi3CoSimulation`, with the exception that the generally not allowed functions of `fmi3ScheduledExecutionCoSimulation` must not be called.
...
In this state the master can create multiple concurrent tasks related to an FMU and in each task the master can activate one or multiple <<input-clock,`input clocks`>> of an FMU based on the defined <<clock>> properties via a <<fmi3ActivateModelPartition>> call for each <<clock>>.
...
_This is especially the case for `fmi3ScheduledExecutionCoSimulation` in which multiple model partitions can be active at the same time._
...
Set/get calls for each task are only allowed for variables that are associated to the <<input-clock>> associated to that task or - here preemption issues become important - to variables that are associated to no <<clock,`clocks`>> (global variables), based on the XML information (see 4.4.3).
...
The simulation master schedules the <<fmi3ActivateModelPartition>> (as well as related `fmi3Get{VariableType}` and `fmi3Set{VariableType}`) calls based on given priorities for <<input-clock,`input clocks`>> defined in the `modelDescription.xml`.
...
If `providesScheduledExecutionSimulation = true` the default setting is that preemption is permitted.
...
Even if the Co-Simulation master does not support preemption and the mode `fmi3ScheduledExecutionCoSimulation`, at least an empty implementation of these callback functions is required.
...
===== Pseudo-code Example for fmi3ScheduledExecutionCoSimulation

In the following example, the usage of the FMI functions is sketched in order to clarify the typical calling sequence of the functions in a simulation environment.
The example is given in a mix of pseudo-code and C, in order to keep it small and understandable.
We consider one slave with three model partitions. Two model partitions associated to two periodic <<input-clock,`input clocks`>> (<<clock>> periods 1 ms and 2 ms) and one aperiodic <<input-clock>>.
