...
_This might not be possible if an aperiodic <<inputClock>> of an FMU depends on the ticking of an aperiodic <<output-clock>> of another FMU or other aperiodic tick sources.]_
...
For both <<output-clock,`output clocks`>> and <<inputClock,`input clocks`>> it is allowed to call these functions during *Event Mode*.
