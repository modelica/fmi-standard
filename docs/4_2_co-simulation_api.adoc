...
If the FMU cannot return exactly at the required `earlyReturnTime`, the FMU must call `{CBIU}` with `canReturnEarly` equals `fmi3true` to transport the new internal simulation time via `intermediateUpdateTime` to the master before it is returning from `{DOSTEP}`. 
...
The states `(Re-)Configuration Mode`, `instantiated`, `Initialization Mode`, `terminated`, `fatal` are handled in the same way as mode `fmi3ModeCoSimulation` with the exception that the the following function calls functions must not be called in any state in this co-simulation mode: `fmi3DoStep()`, `fmi3SetClock()`, `fmi3SetIntervalDecimal()`, `fmi3SetIntervalFraction()`, `fmi3DoEarlyReturn()`, `fmi3EnterEventMode()`, `fmi3EnterStepMode()`.

====== State: Clock Activation Mode
The master and the FMU enter this state when the master calls `fmi3ExitInitializationMode()` in state `Initialization Mode` or `fmi3ExitConfigurationMode` in state `Reconfiguration Mode`.
...
Is called to start the computation of a model partition associated to an inferred clock. 
+
...
In case of `fmi3Discard` or `fmi3Error` the master must wait until all other tasks related to the computation of model partitions of this FMU have ended, but no new tasks can be started (e.g. related to triggered clock tick) and all other function calls for this FMU must also return `fmi3Discard` or `fmi3Error`.
+
It is recommended to call `fmi3SetXXX` and `fmi3GetXXX` in the same task as `fmi3ActivateModelPartition()` for setting and retrieving variable values associated to the clock.

`fmi3Set(Float32, Float64, Int8 ...)`::
Is called before the start of the computation of a model partition (i.e. before call of `fmi3ActivateModelPartition()`) and only for variables associated to the inferred clock assigned to the model partition.
+
...
`fmi3Get(Float32, Float64, Int8 ...)`::
Is called after the end of the computation of a model partition (i.e. after return of `fmi3ActivateModelPartition()`) and only for variables associated to the inferred clock assigned to the model partition. 
+
The restrictions related to variable causality and variability defined for `Step Mode` in mode `fmi3ModeCoSimulation` apply.
+
If the function returns with `fmi3OK` or `fmi3Warning` the FMU stays in this state. 
...
If the function returns with `fmi3Fatal` the FMU goes into state `fatal`.
In case of `fmi3Fatal` the master can prematurely end all tasks related to the computation of model partitions of this FMU.
In case of `fmi3Discard` or `fmi3Error` the master must wait until all other tasks related to the computation of model partitions of this FMU have ended, but no new tasks can be started (e.g. related to triggered clock tick) and all other function calls for this FMU have to also return `fmi3Discard` or `fmi3Error`.
...
The master can call fmi3SetXXX and fmi3GetXXX during the callback for variables associated to a triggered clock that is active during this callback.
...
`fmi3Terminate(), fmi3Reset(), fmi3SetDebugLogging()`::
Can be called if no other task related to the computation of model partitions of this FMU is currently active or preempted. 
...
In case of `fmi3Discard` or `fmi3Error` no repetition of the step is possible, the only possible ways to go forward are to end or to reset the simulation.
