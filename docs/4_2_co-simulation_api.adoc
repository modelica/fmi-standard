...
* [[needsEventHandling,`needsEventHandling`]] <<needsEventHandling,`needsEventHandling == fmi3True`>> indicates that an event was encountered at <<lastSuccessfulTime>> and the importer has to enter <<EventMode>> by calling <<fmi3EnterEventModeCS>>.
<<needsEventHandling,`needsEventHandling == fmi3True`>> must not be signaled, if the importer indicates to not support <<EventMode>> by <<eventModeUsed,`eventModeUsed == fmi3False`>>.
...
** expressed by the return arguments <<needsEventHandling>> or <<terminateSimulationDS>> of <<fmi3DoStep>>, or
