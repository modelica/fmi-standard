...
These functions can be called only at the first activation of <<perodic>> <<outputClock,`output clocks`>>.
For <<perodic,`aperiodic`>> <<outputClock,`output clocks`>>, these functions must be called at every activation _[to inquire when triggered <<inputClock,`input clocks`>> must tick]_.

<<fmi3SetIntervalDecimal>> & <<fmi3SetIntervalFraction>>::
These functions can be called only at the first activation of <<perodic>> <<inputClock,`input clocks`>>.
