...
If the `earlyReturnTime` is greater than or equal to the last signaled `intermediateUpdateTime`, the FMU may integrate up to the time instant `earlyReturnTime`.
...
If the requested `earlyReturnTime` is less than the last signaled `intermediateUpdateTime` or the FMU can not reach the requested `earlyReturnTime` for other reasons, the FMU must set the parameter `earlyReturn` to `fmi3True` in `fmi3DoStep()` and return `fmi3Discard` and enter `Step Discarded`.  
