...
The Co-Simulation master signals to the FMU that it supports and has recognized the <<clock>> and model partition scheduling capabilities of the FMU via setting the Co-Simulation mode in `fmi3CoSimulationMode` to `fmi3ScheduledExecutionSimulation` in the struct `fmi3CoSimulationConfiguration` function <<fmi3Instantiate>>.

If the flag `providesScheduledExecutionSimulation = false`, it is not allowed to set the Co-Simulation mode to `fmi3ScheduledExecutionSimulation`.
