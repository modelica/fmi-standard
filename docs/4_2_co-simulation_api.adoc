...
If `fmu3Type` is set to `fmi3CoSimulation` and the Co-simulation mode is set to `fmi3ModeHybridCoSimulation`, the <<fmi3CallbackIntermediateUpdate>> callback must be defined in <<fmi3CallbackFunctions>> in <<fmi3Instantiate>>.
...
The `{STIUI}`  argument in the <<fmi3CallbackIntermediateUpdate>> function is a structure which is used to signal <<outputClock>> ticks and internal events to the master.
Whenever the FMU invokes the <<fmi3CallbackIntermediateUpdate>> callback function, it fills the `{STIUI}` structure to pass information concerning the call to the Co-Simulation master.
...
If the FMU signals an `earlyReturn = fmi3True` and `fmi3OK` after returning from <<fmi3DoStep>> then `{IUTIME}` is the internal simulation time of the FMU of the last <<fmi3CallbackIntermediateUpdate>> call that signaled `canReturnEarly = fmi3true`.
...
* when `eventOccurred` is `fmi3True`, the master must call <<fmi3DoEarlyReturn>> to do an early return and handle the event via entering event mode. In this case, `earlyReturnTime` argument of <<fmi3DoEarlyReturn>> is ignored. In `Event Mode` the master shall call the <<fmi3NewDiscreteStates>> function for gathering <<fmi3EventInfo>> related information about the event occurred at `{IUTIME}`.
...
* when `canReturnEarly` is `fmi3True` the master can request the FMU to return early at the current `{IUTIME}` time instant via calling `{DER}` within the callback function <<fmi3CallbackIntermediateUpdate>>.
