...
_Assuming an unexpected internal event is detected at_ latexmath:[t^{'}_{i+1}< t_{i+1}] _during FMU~1~ integration, the co-simulation algorithm is informed of this early return._
_Now the co-simulation algorithm would like to avoid other FMUs exceed the event time, since all FMUs should be resynchronized at the event time which will be the next new communication point._
...
The intermediate variable access functions allows the FMU to receive inputs, and provide outputs, directly to the co-simulation algorithm, in those intermediate time points.
...
Intermediate variable access can be activated in all Co-Simulation interface types, if the FMU and the co-simulation algorithm, support it.
...
If the co-simulation algorithm signals the support for intermediate variable access and an FMU has at least one variable with <<intermediateAccess,`intermediateAccess = true`>>, the FMU can use the callback function <<fmi3CallbackIntermediateUpdate>> to communicate information back to the co-simulation algorithm:
...
In case of Co-Simulation, the following two `fmi3Boolean` function arguments give reasons for the <<fmi3CallbackIntermediateUpdate>> call.

[[eventOccurred,`eventOccurred`]]
* When <<eventOccurred,`eventOccurred == fmi3True`>>, the co-simulation algorithm returns <<earlyReturnRequested,`earlyReturnRequested == fmi3True`>> from <<fmi3CallbackIntermediateUpdate>> and request the FMU to return early from the current <<fmi3DoStep>> and handle the event via entering *Event Mode*.
In *Event Mode* the co-simulation algorithm shall call <<fmi3NewDiscreteStates>> to gather related information about the event that occurred at <<intermediateUpdateTime>>.
...
If this flag is `fmi3True` the co-simulation algorithm must return with <<earlyReturnRequested,`earlyReturnRequested == fmi3True`>> from <<fmi3CallbackIntermediateUpdate>> to request an early return and handle the <<clock>> event in *Event Mode*.

[[intermediateVariableSetRequested,`intermediateVariableSetRequested`]]
* If <<intermediateVariableSetRequested,`intermediateVariableSetRequested == fmi3True`>>, the co-simulation algorithm should provide intermediate <<input>> variables by calling `fmi3Set{VariableType}` for continuous variables with <<intermediateAccess,`intermediateAccess = true`>>.
...
* If <<intermediateVariableGetAllowed,`intermediateVariableGetAllowed == fmi3True`>>, the co-simulation algorithm may collect intermediate output variables by calling `fmi3Get{VariableType}` for variables with <<intermediateAccess,`intermediateAccess = true`>>.
...
_[The co-simulation algorithm is then able to extrapolate inputs using this information using <<intermediate-variable-access>>. This enables the same functionality as with `fmi2SetInputDerivatives` in FMI1.0 and FMI2.0]_
...
_If the internal polynomial is of order 1 and the co-simulation algorithm inquires the second <<derivative>> of an <<output>>, the slave will return zero.]_
...
_It is present in order to handle a mismatch between the co-simulation algorithm and the FMU state of the slave: The <<currentCommunicationPoint>> and the FMU state of the slaves defined by former_ <<fmi3DoStep>> _or_ `fmi3SetFMUState` _calls have to be consistent with respect to each other._
...
The <<fmi3CallbackIntermediateUpdate>> callback described in <<intermediate-variable-access>> is also used in order to communicate the <<input>> and <<output>> and, in particular, the event and <<clock>> time from the FMU to the co-simulation algorithm.
...
==== Handling Early Return and Events in Co-Simulation
...
The co-simulation algorithm can signal back to the FMU if and when to return early with the output arguments <<earlyReturnRequested>> and <<earlyReturnTime>> of <<fmi3CallbackIntermediateUpdate>>.
If the FMU returns with <<earlyReturn,`earlyReturn == fmi3True`>> from <<fmi3DoStep>>, the co-simulation algorithm will start event handling by calling <<fmi3EnterEventMode>> for that FMU to push the FMU into *Event Mode*.
...
==== Co-Simulation with Clock Support in Co-Simulation [[api-clocked-co-simulation]]
...
If the FMU provides the Co-Simulation interface type and is instantiated with it, the FMU must internally handle all events during <<fmi3DoStep>>.

_[Remark: Wrapping towards other Co-Simulation interfaces can influence the simulation results._
_Depending on the model especially wrapping towards Co-Simulation may result in divergent simulation results._
...
===== Transfer of Input/Output Values and Parameters in Co-Simulation [[transfer-of-input-output-and-parameters-clocked-co-simulation]]
...
Then the co-simulation algorithm returns with `earlyReturnReqested == fmi3True` from <<fmi3CallbackIntermediateUpdate>> to request the FMU for an early return from <<fmi3DoStep>> and push the FMU into *Event Mode* by calling <<fmi3EnterEventMode>>.
...
.Calling sequence of Co-Simulation C functions in form of a UML 2.0 state machine.
[#figure-co-simulation-state-machine]
image::images/state-machine-co-simulation.svg[width=80%, align="center"]

In this Co-Simulation interface the following functions must not be called: <<fmi3SetTime>>, <<fmi3ActivateModelPartition>>, <<fmi3NewDiscreteStates>>, <<fmi3SetContinuousStates>>, <<fmi3EnterEventMode>>, <<fmi3SetIntervalFraction>>, <<fmi3GetIntervalFraction>>, <<fmi3SetIntervalDecimal>>, <<fmi3GetIntervalDecimal>> including all functions that are specific to Model Exchange.
...
This mode is used by the co-simulation algorithm to compute consistent initial conditions for overall system.
...
This function must not be called, if the co-simulation algorithm signaled `eventModeRequired = fmi3False` during instantiation.
...
If the co-simulation algorithm signals `eventModeRequired == fmi3False` during instantiation, the co-simulation algorithm is not allowed to call <<fmi3EnterEventMode>>.
...
In this state the co-simulation algorithm can retrieve information from the FMU between communication points.
...
Intermediate variables are variables that are marked with attribute <<intermediateAccess,`intermediateAccess = true`>> in the <<modelDescription.xml>> and have been included in the <<requiredIntermediateVariables>> argument to <<fmi3Instantiate>>.
...
Additionally the following functions must not be called <<fmi3EnterConfigurationMode>>, <<fmi3Terminate>>, <<fmi3DoStep>>, <<fmi3GetOutputDerivatives>>, <<fmi3GetDirectionalDerivative>>.
...
==== Code Example for Co-Simulation [[code-example-clocked-co-simulation]]
