...
* enables advanced Co-Simulation with interpolation/extrapolation techniques of inputs and outputs (such as polynomial extrapolation, <<transmission-line-modeling,transmission line modeling>> (<<TLM>>) co-simulation, anti-alias filtering, smoothing of input among others).
