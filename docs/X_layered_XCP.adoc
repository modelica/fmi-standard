...
This standard defines how to describe XCP support of 3.0 FMUs.
...
The XCP service must start during `fmi3EnterInitializationMode` (or earlier) to allow XCP access right from the start of the simulation.
The XCP service must be properly shut down during `fmi3Terminate`, or, in case of fatal errors, in `fmi3FreeInstance`.
