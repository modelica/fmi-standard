 ...
 * Optionally, there can be an additional `<TerminalMemberVariable>` element referencing a variable of type `fmi3Binary` that contains the binary representation of the CAN message.
   If one output CAN message has such a binary representation of the message, all output CAN messages must have such a binary representation.
   Any of the input CAN messages may have such a binary representation of the message.
