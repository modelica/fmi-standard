 ...
 * Furthermore, there can be a variable of type `fmi3Binary` that represents the entire frame, including `Start-of-Frame` and `End-of-Frame`.
...
Both <<TerminalMemberVariables>> must have the same causality of either input or output, all <<PDUTerminal,PDU Terminals>> included here must reference <<TerminalMemberVariable>> with the same causality.
