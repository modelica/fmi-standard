...
*   when `canDoEarlyReturn` is `fmi3True` the master can request the FMU to return early at the current `{IUTIME}` time instant via calling `fmi3DoEarlyReturn()` within the callback function `{CBIU}`. If `canDoEarlyReturn` is `fmi3False` the FMU will not do the early return, regardless of the master request.
