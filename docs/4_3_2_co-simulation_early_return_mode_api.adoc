...
The co-simulation master signals to the FMU that it supports and has recognized the early return co-simulation capabilities of the FMU by setting [TODO] in the `fmi3Instantiate()` function.  
If the master does not support the early-return, the FMU is expected to not set the event flag `fmi3IntermediateStepInfo.eventOccurred` and in this case the FMU is imperatively ending `{DOSTEP}` computation at the next predefined communication instant (i.e., _currentCommunicationPoint_+ _communicationStepSize_).
...
The FMU stops computation at the first encountered triggered event (if any) and the event time is provided through `fmi3IntermediateStepInfo.intermediateStepTime`.
If `{DOSTEP}` returns with `earlyReturn = fmi3True` and an event has happened, i.e., `fmi3IntermediateStepInfo.eventOccurred` is `fmi3True`, then an event handling has to be done by the co-simulation master. 
...
Note that `{DOSTEP}` does not always do an early return if `{IOVCALLBACK}` is called, only if either master requests an early return if `fmi3IntermediateStepInfo.canDoEarlyReturn` is `fmi3True` or the FMU signals an upcoming early return by setting `fmi3IntermediateStepInfo.willDoEarlyReturn` to `fmi3True`. 
