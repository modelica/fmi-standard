...
:DOSTEP: fmi3DoStep()
:IOVCALLBACK: fmi3IntermediateUpdate()
...
Each time an internal discontinuity or an event happens inside an FMU with capability flag `canHandleEarlyReturn=true`, a triggered clock ticks and the callback function `{IOVCALLBACK}` is invoked by the FMU then the associated structure `fmi3IntermediateInfo` is sent back to the co-simulation master.
...
The `fmi3IntermediateInfo`  argument in the `{IOVCALLBACK}` function is a structure which is used to signal triggered clock ticks and internal events to the master.
Whenever the FMU invokes the `{IOVCALLBACK}` callback function, it fills the `fmi3IntermediateInfo` structure to pass information concerning the call to the co-simulation master.
...
*   when `canDoEarlyReturn` is `fmi3True` the master can request the FMU to return early at the current `intermediateTime` time instant via calling `fmi3DoEarlyReturn()` within the callback function `{IOVCALLBACK}`. If `canDoEarlyReturn` is `fmi3False` the FMU will not do the early return, regardless of the master request.

*   when `willDoEarlyReturn` is `fmi3True` the FMU informs the master that the FMU will return early at the current `intermediateTime` time instant.
...
Based on the information delivered in `fmi3IntermediateInfo`, additional information about the discontinuity at that time instant can be obtained from the `fmi3EventInfo` structure after calling  `fmi3NewDiscreteStates()`  and/or `fmi3GetClock()`.
...
The co-simulation master is only allowed to call this function, if the boolean variable `fmi3IntermediateInfo.canDoEarlyReturn` is `fmi3True`.
...
// Irrelevant to this section, to be removed or rewritten?: `[` If an `{DOSTEP}` call of another FMU has already exceeded the time defined by `intermediateTime` from `fmi3IntermediateInfo` the master has to take care for a rollback of such `{DOSTEP}` to meet the intermediateTime for reaching a suitable Communication Point for all model partitions.`]`
...
If the master does not support the early-return, the FMU is expected to not set the event flag `fmi3IntermediateInfo.eventOccurred` and in this case the FMU is imperatively ending `{DOSTEP}` computation at the next predefined communication instant (i.e., _currentCommunicationPoint_+ _communicationStepSize_).
...
The FMU stops computation at the first encountered triggered event (if any) and the event time is provided through `fmi3IntermediateInfo.intermediateTime`.
If `{DOSTEP}` returns with `earlyReturn = fmi3True` and an event has happened, i.e., `fmi3IntermediateInfo.eventOccurred` is `fmi3True`, then an event handling has to be done by the co-simulation master.
...
Note that `{DOSTEP}` does not always do an early return if `{IOVCALLBACK}` is called, only if either master requests an early return if `fmi3IntermediateInfo.canDoEarlyReturn` is `fmi3True` or the FMU signals an upcoming early return by setting `fmi3IntermediateInfo.willDoEarlyReturn` to `fmi3True`.
