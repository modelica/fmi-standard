...
// TODO this section should go to 2.4 common co-sim concepts
...
If the FMU returns <<fmi3EarlyReturn>> from <<fmi3DoStep>> then <<intermediateUpdateTime>> is the internal simulation time of the FMU of the last <<fmi3CallbackIntermediateUpdate>> call that signaled <<canReturnEarly,`canReturnEarly == fmi3True`>>.
<<intermediateUpdateTime>> is also the time of intermediate steps of the internal FMU solver.

The following two `fmi3Boolean` variables define the reasons for the <<fmi3CallbackIntermediateUpdate>> call.
Several variables can be set by the FMU.
Default value of variables is `fmi3False`.

* When <<eventOccurred,`eventOccurred == fmi3True`>>, the master returns <<fmi3EarlyReturn>> from <<fmi3CallbackIntermediateUpdate>> request the FMU for an early return of the current <<fmi3DoStep>> and handle the event via entering *Event Mode*.
...
If this flag is `fmi3True` the master must return with <<fmi3EarlyReturn>> from <<fmi3CallbackIntermediateUpdate>> to request the FMU for an early return and handle the <<clock>> event in *Event Mode*.
...
* When <<canReturnEarly,`canReturnEarly == fmi3True`>> the master can request the FMU to return early at the current <<intermediateUpdateTime>> time instant returning with <<fmi3EarlyReturn>> from the callback function <<fmi3CallbackIntermediateUpdate>>.
If <<canReturnEarly,`canReturnEarly == fmi3False`>> the FMU will not do the early return, regardless of the master request.

* `earlyReturnTime` is used to tell the FMU at what time to return early from the current <<fmi3DoStep>> (at the earliest), if the return value of <<fmi3CallbackIntermediateUpdate>> is <<fmi3EarlyReturn>>.
...
If the FMU is successful in conducting an early return, the return code of <<fmi3DoStep>> is <<fmi3EarlyReturn>>.
If the FMU returns from <<fmi3DoStep>> with <<fmi3EarlyReturn>>, the Co-Simulation master has to call <<fmi3EnterEventMode>> for that FMU.
...
If <<fmi3DoStep>> returns with <<fmi3EarlyReturn>> and an event has happened, i.e. <<eventOccurred,`eventOccurred == fmi3True`>>, then an event handling has to be done by the Co-Simulation master.
...
If the early-return request of the Co-Simulation master is ignored by the FMU, then <<fmi3DoStep>> returns with a code other than <<fmi3EarlyReturn>>.
...
In *Event Mode* and only after <<fmi3DoStep>> returned with <<fmi3EarlyReturn>> the function <<fmi3NewDiscreteStates>> may be called.
...
Then the master returns with <<fmi3EarlyReturn>> from <<fmi3CallbackIntermediateUpdate>> to request the FMU for an early return from <<fmi3DoStep>> and push the FMU into *Event Mode* by calling <<fmi3EnterEventMode>>.
...
If <<fmi3DoStep>> returns with <<fmi3EarlyReturn>> and <<eventOccurred,`eventOccurred == fmi3True`>> or <<clocksTicked,`clocksTicked == fmi3True`>>, the FMU must be pushed into the *Event Mode* by calling <<fmi3EnterEventMode>>.
