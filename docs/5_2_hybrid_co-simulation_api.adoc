...
If <<fmi3DoStep>> returns with `earlyReturn = fmi3True` and `eventOccurred = fmi3True` or <<clocksTicked,`clocksTicked = fmi3True`>>, the FMU must be pushed into the *Event Mode* by calling <<fmi3EnterEventMode>>.
