...
* `earlyReturnTime` is used to tell the FMU at what time to return early from the current <<fmi3DoStep>> (at the earliest), if the return value of <<fmi3CallbackIntermediateUpdate>> is <<fmi3ReturnEarly>>.
