...
The order of the states and derivatives is the same order of the variables in the `<ModelVariables>`.
Array variables are serialized in "row major" order.

The event indicators are returned as a vector with `nEventIndicators` elements.
The order of event indicators in `eventIndicators` vector is the same as the ordered list of the respective variables in the `<ModelVariables>`.
Array variables are serialized in "row major" order.
...
Note that during *Initialization*, *Event Mode*, and *Continuous-Time Mode*, <<input>> variables can be set with `fmi3Set{VariableType}` and output variables can be retrieved with `fmi3Get{VariableType}` interchangeably according to the <<model-structure,model structure>>.
...
Output Clocks::
For an <<outputClock>>, the environment integrates until a <<time event,`continuous-time event`>> occurs and calls <<fmi3EnterEventMode>>. Afterwards with function call <<fmi3GetClock>> the <<outputClock,`output clocks`>> that became active at this event instant can be inquired by the environment.
