...
This is unproblematic for time and states, but is more involved for <<parameter,`parameters`>> and inputs, since the latter may have different data types.
...
Argument nx is the length of vector x and is provided for checking purposes (variables that depend solely on constants, <<parameter,`parameters`>>, time, and inputs do not need to be newly computed in the sequel, but the previously computed values can be reused).
...
Set new values for (<<independent>>) <<parameter,`parameters`>>, start values and inputs and re-initialize caching of variables that depend on these variables.
...
The clock activation status can be inquired with the function <<fmi3GetClock>>. The environment calls the function <<fmi3SetClock>> for periodic clocks and input clocks.
