...
* Argument `nStates` is the length of
...
The <<derivative,`derivatives`>> are returned as a vector with `nStates` elements.
...
The event indicators are returned as a vector with `nEventIndicator` elements.
...
Return the new (continuous) state vector `states`.
