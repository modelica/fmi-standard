...
* _Handle DAE projection by projecting the states onto the constraint manifold._
_Note that this occurrence is signalled to the importer by flagging that a state event has occurred._
...
`stepEvent` must be `fmi3True` if <<fmi3CompletedIntegratorStep>> returned with `enterEventMode = fmi3True`.
...
+
The function arguments `stateEvent`, `rootsFound`, and `timeEvent` inform the FMU about the reason for entering <<EventMode>> to enable efficient and robust event handling.
If an importer is not able to provide this information, the importer must set `stateEvent = fmi3True` and `nEventIndicators = 0` and `timeEvent = fmi3True`.
In this case, the FMU must determine the event type itself. +
