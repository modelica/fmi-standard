...
The objective of the start chart is to define the allowed calling sequences for functions of the FMI: Calling sequences not accepted by the state machine are not supported by the FMI.
The behavior of an FMU is undefined for such a calling sequence.
For example, the state machine indicates that when an FMU for Model Exchange is in state *Continuous-Time Mode*, a call to `fmi3Set{VariableType}` for a <<discrete>> <<input>> is not supported.
