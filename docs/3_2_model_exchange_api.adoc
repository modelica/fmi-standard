...
`rootsFound`:: array of length `nEventIndicators`. For `i = 1, ..., nEventIndicators, rootsFound[i] == fmi3True` if the event indicator has a root, and `rootsFound[i] == fmi3False` if not.
