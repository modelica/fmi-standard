...
+
_[An <<input-event>> can be detected by the FMU by keeping track of the calls of `fmi3Set{VariableType}` in <<EventMode>>.]_
...
Note that during <<InitializationMode>>, <<EventMode>>, and <<ContinuousTimeMode>>, <<input>> variables can be set with `fmi3Set{VariableType}` and output variables can be retrieved with `fmi3Get{VariableType}` interchangeably according to the model structure defined under element `<ModelStructure>` in the XML file.
