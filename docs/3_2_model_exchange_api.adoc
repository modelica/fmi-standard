...
Output Clocks::
For an <<outputClock>>, the environment integrates until a <<time event,`discrete-time event`>> occurs and calls <<fmi3EnterEventMode>>. Afterwards with function call <<fmi3GetClock>> the <<outputClock,`output clocks`>> that became active at this event instant can be inquired by the environment.

_[TODO: can an output clock only at time events? Sentence doesn't make sense]_

Input Clocks::
Interval is set with the function <<fmi3SetIntervalDecimal>> or <<fmi3SetIntervalFraction>>. It is activated by the environment with the function <<fmi3SetClock>>.

_[TODO: why is this here at all?]_
...
_[TODO: "clock event" is introduced here first, not mentioned in mathematical description]_
...
_[TODO: what is the difference between clock tick and clock activation? If it is the same we shouldn't use 2 different words.]_
...
_[TODO: What does the mean? As soon as one Get of a clocked variable has been called? Or can get be only called once per variable? How ist the relation to fmi3NewDiscreteStates (see below)?]_
...
_[This handling of discrete <<state,`states`>> and <<time event,`time events>> is forward compatible with FMI 2.0 for any model that could be treated with FMI 2.0 and is exported again using the new features._
