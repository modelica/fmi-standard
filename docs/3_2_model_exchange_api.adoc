...
_[Note that there is an inconsistency in the state graph and the table regarding if `fmi2GetEventIndicators` is allowed to be called in `initializationMode`.
