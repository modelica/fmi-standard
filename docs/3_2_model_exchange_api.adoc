...
Before calling this function in *Initialization Mode*, all variables with attribute `initial` equal `exact` or `approx` can be set with the `fmi3Set{VariableType}` functions (the `Variable` attributes are defined in the Model Description File, see <<definition-of-model-variables>>).
...
_[These arguments are not mutually exclusive and not may all be `fmi3False` if the caller cannot provide this information.]_
