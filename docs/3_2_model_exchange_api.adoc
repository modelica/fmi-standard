...
** for dynamic state selection: The FMU checks whether the dynamically selected states are still numerically appropriate.
...
** for projection: FMUs that represent a <<DAE,DAE>> can project their continuous-time states onto the constraint manifold in <<fmi3UpdateDiscreteStates>>.
