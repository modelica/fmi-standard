...
The variables that can be retrieved by `fmi3Get{VariableType}` calls are (1) defined in the XML file under `<ModelStructure><InitialUnknowns>` and (2) variables with <<causality>> = <<output>>.
