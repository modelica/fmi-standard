...
If an FMU is run through one of its binaries all items in that binary folder must be unpacked at the same location as the binary `<modelIdentifier>.*`.
...
For the FMU to access these resource files, the `resource` directory shall be available in unzipped form and the absolute path to this directory must be reported via argument `resourceLocation` of <<fmi3Instantiate>>.
