...
_[A perfectly acceptable implementation for such stub functions is to return `fmi3Error` after logging the error reason, as specified in <<status-returned-by-functions>>.
Calling environments are well-advised to not rely on the presence of these functions if not required, and to avoid calling them.]_
