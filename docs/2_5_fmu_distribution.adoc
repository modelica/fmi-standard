...
The use of subdirectories beginning with `org.modelica` and `org.fmi-standard` is explicitly 
reserved for use by <<VersioningLayered,layered standards>>, defined by Modelica Association or its own projects (<<MAP>>s), i.e. other uses must not use subdirectory names beginning with these prefixes.
