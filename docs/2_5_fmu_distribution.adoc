...
In order to avoid ambiguities and conflicts, the names of these subdirectories should use the <<reverse-DNS>> of a domain that is controlled by the entity defining the semantics and content of the additional entries _[(for example `extra/com.example/SimTool/meta.xml` or `extra/org.example.stdname/data.asd`)]_.
