...
For the FMU to access these resource files, the `resource` directory shall be available in unzipped form and the absolute path to this directory must be reported via argument `resourceLocation` of <<fmi3InstantiateXXX>>.
