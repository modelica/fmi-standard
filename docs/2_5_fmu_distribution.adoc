...
The use of subdirectories beginning with `org.modelica` and `org.fmi-standard` is explicitly reserved for use by <<MAP>> FMI-defined <<VersioningLayered,layered standards>>, i.e. other uses must not use subdirectory names beginning with these prefixes.
