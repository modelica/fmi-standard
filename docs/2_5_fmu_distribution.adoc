...
See <<fmiSourceCode>> for a definition of the directory `sources`.
...
The described multi-mode support is based on wrapping functionality into the <<fmi3DoStep>> function by emulating missing features of the Model Exchange and Scheduled Execution interface types.
