...
The API is used by the importer (e.g., a simulator or an optimizer) to create one or more instances of an FMU and to execute them, typically together with other models.
...
image::model-exchange-data-flow.svg[width=450]

==== FMI for Co-Simulation (CS)

The Co-Simulation interface is designed for the coupling of subsystem models, exported by a modeling environment including any necessary simulation code (e.g., solvers), or using an external simulator.
...
image::co-simulation-data-flow.svg[width=450]
...
NOTE: See also https://modelica.github.io/fmi-guides/main/fmi-guide/#_use_cases_for_scheduled_execution[Use-cases for Scheduled Execution] in the FMI 3.0 Implementers' Guide.
...
image::scheduled-execution-data-flow.svg[width=600]
...
image::fmi-types-overview.svg[width=550]
...
|<<fmi3GetEventIndicators,Event Indicators>>
|icon:check[]
|icon:times[]
|icon:times[]
...
|Includes similar or better mechanism
|icon:check[]
|Signal output <<Clock>> activations: icon:check[] +
...
The FMI standard allows problems which are numerically critical (for example, <<time-event,time events>> and <<state-event,state events>>, multiple sample rates, stiff problems) to be treated in a robust way.
