...
With FMI 3.0, virtual <<electronic-control-unit,electronic control units>> (<<vECU,vECUs>>) can be exported as FMUs in a more natural way.
...
The Functional Mock-up Interface (<<FMI>>) defines a <<ZIP>> archive and an <<API,application programming interface>> (<<API>>) to exchange dynamic models using a combination of <<XML>> files, binaries and C code: the Functional Mock-up Unit (<<FMU>>).
The API is used by the <<importer>> (e.g., a <<simulator,simulator>> or an optimizer) to create one or more instances of an FMU and to execute them, typically together with other models.
...
The Model Exchange interface exposes an <<ODE,ODE>> to an external <<solver,solver>> of an <<importer,importer>>.
Models are described by differential, algebraic and discrete equations with time-, state- and step-events.
That <<integration-algorithm,integration algorithm>> of the <<importer,importer>>, usually a <<ODE,ODE>>/<<DAE,DAE>> <<solver,solver>>, is responsible for advancing time, setting states, handling events, etc.
...
The Co-Simulation interface is designed for the coupling of subsystem models, exported by a modeling environment including any necessary simulation code (e.g., solvers), or using an external <<simulator,simulator>>.
...
The Scheduled Execution interface exposes individual model-partition.
A scheduler provided by the <<importer>> can control the execution of each model partition separately.
In some ways the Scheduled Execution interface has similarities to the Model Exchange interface: the first externalizes a scheduling algorithm usually found in a controller algorithm (see <<fmi-for-scheduled-execution>>) and the second interface externalizes the <<ODE,ODE>>/<<DAE,DAE>> <<solver,solver>>.
...
|Direct <<feedthrough,Feedthrough>>
|icon:check[]
...
Expressivity::
The FMI provides the necessary features to package models of different domains, such as multi-body and virtual <<ECU,ECUs>>, into an FMU.

Stability::
The FMI is expected to be supported by many importers worldwide.
...
+
Reason: The standard would be much less attractive otherwise, unnecessarily restricting the later use of an FMU at compile time and forcing users to maintain <<simulator,simulator>> specific variants of an FMU.
To be <<simulator,simulator>> independent, the FMU must export its implementation in self-contained binary form.
This requires the processor and target operating system (if dependencies exist) to be known.
Once exported with binaries, the FMU can be executed by any <<simulator,simulator>> running on the target platform (provided the necessary licenses are available, if required from the model or from the used runtime libraries).
...
+
Reason: the FMI standard shall not unnecessarily restrict or prescribe a certain implementation of FMUs or <<simulator,simulators>> (whichever contains the model data) to ease implementation by different tool vendors.

Support many and nested FMUs::
A <<simulator,simulator>> may run many FMUs in a single simulation run and/or multiple instances of one FMU.
The inputs and outputs of these FMUs can be connected with direct <<feedthrough,feedthrough>>.
...
* The FMI for Model Exchange is for <<ordinary-differential-equation,ordinary differential equations>> (<<ODE,ODEs>>) in state space form.
It is not for a general <<differential-algebraic-equation,differential-algebraic equation>> (<<DAE>>) system.
