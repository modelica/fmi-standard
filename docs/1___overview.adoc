...
This is a major release with <<fmi-whats-new, new features>> compared to FMI 2.0.
...
|3.0
|
|This, third version of FMI for Model Exchange and Co-Simulation, for new features see the next section
|====
...
(FMI for Basic Co-Simulation, described in <<fmi-for-basic-co-simulation>> or FMI for Hybrid Co-Simulation, described in <<fmi-for-hybrid-co-simulation>>),
...
or require the simulation environment to trigger model partition execution (FMI for Scheduled Co-Simulation , described in <<fmi-for-scheduled-co-simulation>>).
...
(which have been exported by their simulators together with its solvers as runnable code).
...
(See <<fmi-for-basic-co-simulation>>.)
