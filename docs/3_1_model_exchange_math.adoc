...
|Enter *Event Mode* at latexmath:[t = t_{i}] with latexmath:[{t_{i}\ : = (t}_{R},t_{I} + 1)] *if*  externalEvent *or* nextMode latexmath:[\equiv] EventMode *or* latexmath:[t_i=(\mathbf{T}_{\mathit{next}}(t_{i-1}), 0)] *or*  latexmath:[\min_{t>t_{i-1}} t:\left\lbrack z_{j}\left( t \right) > 0\  \neq \ z_{j}\left( t_{i-1} \right) > 0 \right\rbrack] +
(activate discrete-time equations)
|`fmi3EnterEventMode`(only from *Continuous-Time Mode*)


2+|Equations during *Continuous-Time Mode*

|[lime]#Enter *Continuous-Time Mode*:# +
[lime]#latexmath:[\qquad \textrm{// deactivate discrete-time equations}]# +
...
|Set <<independent>> variable _[typically: time]_ latexmath:[t(>t_{\mathit{enter  mode}}): t:=(\min(t_{Ri} + h, \mathbf{T}_{\mathit{next}}), 0)]
|<<fmi3SetTime>>
