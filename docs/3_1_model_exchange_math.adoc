...
`[lime]#updateDiscreteStatesNeeded#` [lime]#whether a new event iteration is required.# +
[blue]#latexmath:[\qquad]*if not*# `[blue]#updateDiscreteStatesNeeded#`[blue]#*then* +
...
_However, just like in FMI 2.0, it could in principle also be implemented by activating the model equations only at the first event iteration and returning always `updateDiscreteStatesNeeded == fmi3False` from <<fmi3UpdateDiscreteStates>>._
