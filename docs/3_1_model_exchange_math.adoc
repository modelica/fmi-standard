...
<<integration-algorithm,Integration algorithms>> for differential equations assume continuous behavior of variables.
In hybrid ODEs, there are points during the simulation, when this assumption is violated.
A hybrid <<ODE,ODE>> <<solver,solver>> must therefore:
...
* restart the <<integration-algorithm,integration algorithm>> when leaving <<EventMode>>.
...
NOTE: This definition is slightly different from the usual standard definition of state events: latexmath:[\mathbf{z}(t) \cdot \mathbf{z}(t_{i-1}) \leq 0] which has the severe drawback that the value of the event indicator at the previous event instant, latexmath:[\mathbf{z}(t_{i-1}) \neq 0], must be non-zero and this condition cannot be guaranteed.
...
Even when an event indicator switches, its corresponding relation must stay "frozen" to assure the continuity assumption of the <<integration-algorithm,integration algorithm>>.
This allows an <<integration-algorithm,integration algorithm>> to properly finish the current integration step.
