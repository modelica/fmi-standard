...
The FMI for Model Exchange was designed to allow selective retrieval of FMU variables with specific functions (e.g. `fmi3Get{VariableType}`, <<fmi3GetEventIndicators>>, <<fmi3GetDerivatives>>).
...
==== Model Evaluations, Dependencies, Algebraic Loops and Call Sequence [[algebraic-loops]]
