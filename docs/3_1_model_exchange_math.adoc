...
_[In FMI 2.0, there is no mechanism that the FMU can provide the information whether a discrete-time variable is active or is not active (is not computed) at an event instant.
With clocked variables, FMI 3.0 helps the environment to determine, which variables are newly computed by the FMU.]_

_[TODO: something seems to be missing here]_
...
_[TODO: these clock ticks were mentioned in FMI2.0 already, is it consistent with FMI3.0 clocks?]
...
_[TODO: no clock events??? should there even be input clock event, output clock events?]_
...
_[TODO: what does fire mean???]

, but new <<input,`inputs`>> shall be taken into account). +
