...
|Set continuous-time states latexmath:[\mathbf{x}_c(t_i)]|`fmi2SetXXX`,
...
before `fmi2EnterEventMode` is called.]_
...
After calling `fmi2ExitInitializationMode` the FMU is implicitely in Event Mode and all discrete-time and continuous-time variables at the initial time instant latexmath:[(t_R, 0)] can be calculated,
