...
These are <<independent>> <<parameter,`parameters`>> and startvalues of other variables, such as initial values for states, state derivatives or <<output,`outputs`>>.
...
Basically, in this phase the <<derivative,`derivatives`>> of the continuous states are computed.
