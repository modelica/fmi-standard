...
|Set <<independent>> variable time latexmath:[T_{R0}] and define latexmath:[t_0 := (t_{R0},0)]
|<<fmi3SetupExperiment>>
...
Once finalized, <<fmi3NewDiscreteStates>> must be called, and depending on the value of the return parameter, the FMU either continues the event iteration at the initial time instant or switches to *Continuous-Time Mode*. +
