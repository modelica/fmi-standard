...
An <<fmi3GetEventIndicators,event indicator>>, and only an event indicator, signals a <<state-event>> with the domain change from latexmath:[\mathbf{z}_j > 0] to latexmath:[\mathbf{z}_j \leq 0] or from latexmath:[\mathbf{z}_j \leq 0] to latexmath:[\mathbf{z}_j > 0].
