...
#[TODO]: Mathematical concept and background is needed for clocks in co-simulation.#

==== Scheduled Execution Simulation Support [[math-scheduled-execution-simulation]]
:stem: latexmath

The Scheduled Execution Simulation mode has a different timing concept compared to `fmi3DoStep()` calls in the other co-simulation modes. 
