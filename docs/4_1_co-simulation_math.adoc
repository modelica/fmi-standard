...
|latexmath:[\mathbf{u}_{i, IVA}(t)]
|Intermediate input variables corresponding to the input variables for intermediate variable access ("IVA").
The values of these variables are defined outside of the model.
latexmath:[\mathbf{u}_{i, IVA}] are functions defined on the interval latexmath:[ [tc_i, tc_{i+1}] ] with latexmath:[\mathbf{u}_{i, IVA}(tc_i)=\mathbf{u}(tc_i)]. +
With the optional flag <<recommendedIntermediateInputSmoothness>> FMU can signal to the master that best convergence rates can be achieved if the these functions are of the smoothness latexmath:[C^{k}_\left(\left[tc_i, tc_{i+1}\right]\right)], that is k-time continuously differentiable, with latexmath:[C^{0}] meaning continuous.
