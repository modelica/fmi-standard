...
It is also recommended that the function defined by the continuation of latexmath:[\mathbf{u}_{i, IU}] with latexmath:[\mathbf{u}_{i+1, IU}] be of smoothness latexmath:[C^{k}([t_i, t_{i+2}\])] with the optional flag <<recommendedIntermediateInputSmoothness>> of value latexmath:[k]. +
