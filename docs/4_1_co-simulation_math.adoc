...
- A real sampled-data system (so a sampled discrete controller; the <<input,`inputs`>> and <<output,`outputs`>> can be of numeric type, String, or enumeration.
...
It is assumed here that all <<input,`inputs`>> and all <<output,`outputs`>> of this hybrid ODE are float signals (defined with <<variability>> = <<continuous>>),
...
latexmath:[\mathbb{R}]: float variable, latexmath:[\mathbb{R}]: float *or* boolean *or* integer *or* enumeration *or* string variable +
