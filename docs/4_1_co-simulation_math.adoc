...
Such a sampled-data system consists typicalla of a hybrid ODE that is integrated between communication points (known as "sampled access to time continuous systems") where internal events may occur and be handled, but events are not visible from the outside of the FMU.
...
_[ In FMI 2.0, Co-Simulation FMUs were also used for real sampled-data systems (so a sampled discrete controller; the <<input,`inputs`>> and <<output,`outputs`>> could be of type Real, Integer, Boolean, String, or enumeration with <<variability>> = <<discrete>>._
_However, in FMI 3.0, Hybrid Co-Simulation (HCS) and Scheduled Execution Simulation (SES) are intended for this use-case.]_
...
==== Scheduled Execution Simulation interface [[math-scheduled-execution-simulation]]

The Scheduled Execution Simulation interface has a different timing concept compared to both Co-Simulation interfaces.
