...
On massive multi-FMU architectures, significant co-simulation speed-up may be obtained if the master can avoid waiting until the end of the slowest step integration when many FMUs are integrating in parallel.
...
Step integrations are executed in parallel, so that each FMU starts integration from communication point latexmath:[t_{i}] to reach the next communication point latexmath:[t_{i+1}].
