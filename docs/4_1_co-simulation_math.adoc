...
#[TODO: The description should not use api calls but describe the idea. api functions are introduced in the api section later]#
...
//TODO: (review the below text descriptions of the four cases) text form (like below) instead of only png (i.e. no text search and no tracking of changes possible in png form)
...
In the case of FMU3, a partial rollback is necessary.
In the case of FMU4, the current `{DOSTEP}` has been interrupted at latexmath:[t^{'}_{i+1}] and no rollback is necessary.
...
So now the master would like avoid other FMUs exceed the event time, since all FMUs should be resynchronized at the event time which will be the next new communication point.
...
//TODO-API: Why next internal intermediate step and not at the current intermediate step!?

Afterwards, the FMU goes into the `Step Mode` state and  `{DOSTEP}`  returns and signals the premature stop (early-return) to the master by setting `earlyReturn` parameter of `{DOSTEP}` to `fmi3True`.
If the FMU signals a `earlyReturn` equals `fmi3True` the internal simulation time of the FMU equals the value of `intermediateUpdateTime` of the last `{CBIU}` call that has signaled `canReturnEarly` equals `fmi3true`.
//TODO-API: Is my assumption correct? Is this intermediateUpdateTime the simulation time of the FMU the master can assume after an successful early return!? If not, what is the early return simulation time?
...
#[//TODO: The description should not use api calls but describe the idea. api functions are introduce in the api section later]#
