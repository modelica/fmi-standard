...
Inferred Clocks::
Intervall is set with the function `fmi3SetInterval`. It is activated by the environment with the function `fmi3SetClock`.

==== Handling of clock events (fmi3SetClock, fmi3GetClock, fmi3SetInterval, fmi3GetInterval)
...
1.	Synchronize clock activation and intervals with the environment::
...
Moreover the current clock intervals may be inquired with the function `fmi3GetInterval` and set with the function `fmi3SetInterval`.
...
2.	Solve algebraic loops among discrete-time equations of multiple FMUs or repeated evaluation in model-based control applications (optional)::
With the `fmi3SetXXX` functions of section 2.1.6, the input variables of the clocked partition can be set. 
If algebraic loops are present among clocked partitions of multiple FMUs, then with the `fmi3GetXXX` functions of section 2.1.6, all variables on the corresponding clock can be inquired. 
...
The environment may ignore the new functions fmi3SetClock, fmi3GetClock, fmi3SetInterval and fmi3GetInterval. 
