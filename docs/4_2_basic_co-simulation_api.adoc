...
This is especially interesting, if <<fmi3DoStep>> returns with <<fmi3EarlyReturn>>.
...
It is even possible that the `lastSuccessfulTime` is still equal to `currentCommunicationPoint` when <<fmi3EarlyReturn>> is returned (contrary to the possibly expected <<fmi3Discard>>) to indicate a changed internal state of the FMU, e.g. steps in super dense time.
...
<<fmi3EarlyReturn>>::
if the FMU computed successfully only a subinterval of the communication step.

<<fmi3Discard>>::
if the FMU could not compute any new (more advanced) state successfully, but was able to return its internal state to exactly the state when <<fmi3DoStep>> was called.
This return code indicates that the call to <<fmi3DoStep>> was essentially a NOOP.
The importer may now try alternative ways to simulate, or, simply, treat this equal to <<fmi3Error>>.

`fmi3Error`::
the communication step could not be carried out at all, the internal state of the FMU is undefined.
The master can try to repeat the step with other input values and/or a different communication step size, but has to re-initialize the state by either <<fmi3SetFMUState>> or terminating and reinitializing the instance.
...
<<fmi3EnterInitializationMode>>, <<fmi3SetInputDerivatives>>, <<fmi3EnterConfigurationMode>>
...
<<fmi3Instantiate>>, <<fmi3Terminate>>, `fmi3Get{VariableType}`, <<fmi3GetDirectionalDerivative>>, <<fmi3GetOutputDerivatives>>, <<fmi3DoStep>>, <<fmi3ExitInitializationMode>>, <<fmi3GetDoStepDiscardedStatus>>, <<fmi3ExitConfigurationMode>>, <<fmi3EnterStepMode>>
...
Forbidden Function Calls::
<<fmi3Instantiate>>, <<fmi3Terminate>>, <<fmi3DoStep>>, <<fmi3EnterInitializationMode>>, <<fmi3GetDoStepDiscardedStatus>>, <<fmi3EnterConfigurationMode>>, <<fmi3ExitConfigurationMode>>, <<fmi3EnterStepMode>>
...
Forbidden Function Calls::
<<fmi3ExitConfigurationMode>>, <<fmi3Instantiate>>, <<fmi3EnterInitializationMode>>, <<fmi3ExitInitializationMode>>, <<fmi3GetDoStepDiscardedStatus>>, <<fmi3EnterStepMode>>
...
Forbidden Function Calls::
<<fmi3EnterConfigurationMode>>, <<fmi3Instantiate>>, <<fmi3EnterInitializationMode>>, <<fmi3ExitInitializationMode>>, <<fmi3GetDoStepDiscardedStatus>>, <<fmi3DoStep>>, <<fmi3EnterStepMode>>, <<fmi3SetInputDerivatives>>, <<fmi3GetDirectionalDerivative>>, <<fmi3GetOutputDerivatives>>, `fmi3Get{VariableType}`
...
Forbidden Function Calls::
<<fmi3EnterConfigurationMode>>, <<fmi3ExitConfigurationMode>>, <<fmi3Instantiate>>, <<fmi3EnterInitializationMode>>, <<fmi3ExitInitializationMode>>, <<fmi3DoStep>>, `fmi3Set{VariableType}`, <<fmi3SetInputDerivatives>>
...
// TODO: merge this with the section from hybrid-co-sim (and/or remove) 2_4 common co-sim concepts 

The Co-Simulation master can request the FMU to end the <<fmi3DoStep>> at a Newtonian time instant and to do an early return returning with the status <<fmi3EarlyReturn>> from the callback function <<fmi3CallbackIntermediateUpdate>>.
The Co-Simulation master is only allowed to return this status code, if <<canReturnEarly,`canReturnEarly == fmi3True`>>.
