...
The Co-Simulation master can request the FMU to end the <<fmi3DoStep>> at a Newtonian time instant and to do an early return by returning with `earlyReturnRequested == fmi3True` from the callback function <<fmi3CallbackIntermediateUpdate>>.
...
If the early return is conducted successfully by the FMU it must return with `earlyReturn == fmi3True` from <<fmi3DoStep>>.
