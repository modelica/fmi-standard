...
The slave is expected to compute until time latexmath:[tc_{\mathit{i+1}} = tc_i + hc_i], or <<lastSuccessfulTime>> `=` <<currentCommunicationPoint>> `+` <<communicationStepSize>>.

This is especially interesting, if <<fmi3DoStep>> returns with `earlyReturn == fmi3True`.
In this case, the step did not compute until latexmath:[tc_{\mathit{i+1}}], but stopped computation error free until <<lastSuccessfulTime>>.
...
_It is even possible that the <<lastSuccessfulTime>> is still equal to <<currentCommunicationPoint>> when `earlyReturn == fmi3True` is returned (contrary to the possibly expected <<fmi3Discard>>) to indicate a changed internal state of the FMU, e.g. steps in super-dense time.]_
...
_For example, if the slave does not use the update formula for the <<independent>> variable as required above,_ latexmath:[tc_{\mathit{i+1}} = tc_i + hc_i] _(using argument_ latexmath:[tc_i] = <<currentCommunicationPoint>> _of_ <<fmi3DoStep>>) _but uses internally an own update formula, such as_ latexmath:[tc_{\mathit{s,i+1}} = tc_{\mathit{s},\mathit{i}} + hc_{\mathit{s},\mathit{i}}] _then the slave could use as time increment_ latexmath:[\text{hc}_{\mathit{s},\mathit{i}} := (tc_i - tc_{\mathit{s},\mathit{i}}) + hc_i] _(instead of_ latexmath:[\text{hc}_{\mathit{s},\mathit{i}} := hc_i] _) to avoid a mismatch between the master time_ latexmath:[tc_{\mathit{i+1}}] _and the slave internal time_ latexmath:[tc_{\mathit{s,i+1}}] _for large i.]_
...
* If the function returns with `earlyReturn == fmi3True` the FMU will change to states depending on the flags of <<fmi3CallbackIntermediateUpdate>>.
