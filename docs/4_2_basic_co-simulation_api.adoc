...
_It is even possible that the `lastSuccessfulTime` is still equal to `currentCommunicationPoint` when <<fmi3EarlyReturn>> is returned (contrary to the possibly expected <<fmi3Discard>>) to indicate a changed internal state of the FMU, e.g. steps in super-dense time.]_
...
This return code indicates that the call to <<fmi3DoStep>> was essentially without effect (did not change the FMU state).
...
* If the function returns with <<fmi3EarlyReturn>> the FMU will change to states depending on the flags of <<fmi3CallbackIntermediateUpdate>>.
...
Forbidden Function Calls::
<<fmi3EnterConfigurationMode>>, <<fmi3Instantiate>>, <<fmi3EnterInitializationMode>>, <<fmi3ExitInitializationMode>>>>, <<fmi3DoStep>>, <<fmi3EnterStepMode>>, <<fmi3SetInputDerivatives>>, <<fmi3GetDirectionalDerivative>>, <<fmi3GetOutputDerivatives>>, `fmi3Get{VariableType}`
...
|<<fmi3GetVersion>>               |x |x |x |x |x |x |x |x
|<<fmi3SetDebugLogging>>          |  |x |x |x |x |x |x |x
|<<fmi3Instantiate>>              |x |  |  |  |  |  |  |
...
|`fmi3GetReal`                    |  |  |2 |x |  |7 |x |7
|`fmi3GetInteger`                 |  |  |2 |x |  |7 |x |7
|`fmi3GetBoolean`                 |  |  |2 |x |  |7 |x |7
|`fmi3GetString`                  |  |  |2 |x |  |7 |x |7
|`fmi3SetReal`                    |  |1 |3 |6 |  |  |  |
|`fmi3SetInteger`                 |  |1 |3 |6 |  |  |  |
|`fmi3SetBoolean`                 |  |1 |3 |6 |  |  |  |
|`fmi3SetString`                  |  |1 |3 |6 |  |  |  |
|<<fmi3GetFMUState>>              |  |x |x |x |  |7 |x |7
...
|<<fmi3GetDirectionalDerivative>> |  |  |x |x |  |7 |x |7
|<<fmi3SetInputDerivatives>>  {set:cellbgcolor:yellow}     |  {set:cellbgcolor!} |x |x |x |  |  |  |
|<<fmi3GetOutputDerivatives>> {set:cellbgcolor:yellow}     |  {set:cellbgcolor!} |  |  |x |  |x |x |7
...
If the early return is conducted successfully by the FMU it must return with <<fmi3EarlyReturn>> from <<fmi3DoStep>>.
