...
It depends on the capabilities of the slave which parameter constellations and calling sequences are allowed (see <<basic-co-simulation>>)
...
==== Scheduled Co-Simulation [[api-scheduled-co-simulation]]
...
The Co-Simulation master signals to the FMU that it supports and has recognized the <<clock>> and model partition scheduling capabilities of the FMU via setting the Co-Simulation interface in `fmi3CoSimulationMode` to `fmi3ScheduledCoSimulation` in the struct `fmi3CoSimulationConfiguration` function <<fmi3Instantiate>>.

If the flag `providesScheduledCoSimulation = false`, it is not allowed to set the Co-Simulation interface to `fmi3ScheduledCoSimulation`.
...
===== Transfer of Input / Output Values and Parameters in fmi3ScheduledCoSimulation
...
In `fmi3ScheduledCoSimulation` the handling of <<fmi3CallbackIntermediateUpdate>> callbacks is the same as in section <<clocked-co-simulation>>.
...
===== Computation in fmi3ScheduledCoSimulation
...
===== State Machine for fmi3ScheduledCoSimulation
...
The states *Configuration Mode*, *Instantiated*, *Initialization Mode*, and *Terminated* are handled in the same way as in `fmi3CoSimulation`, with the exception that the generally not allowed functions of `fmi3ScheduledCoSimulation` must not be called.
...
_This is especially the case for `fmi3ScheduledCoSimulation` in which multiple model partitions can be active at the same time._
...
Even if the Co-Simulation master does not support preemption and the mode `fmi3ScheduledCoSimulation`, at least an empty implementation of these callback functions is required.
...
===== Pseudo-code Example for fmi3ScheduledCoSimulation
