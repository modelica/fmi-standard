...
_For example, if the slave does not use the update formula for the <<independent>> variable as required above,_ latexmath:[tc_{\mathit{i+1}} = tc_i + hc_i] _(using argument_ latexmath:[tc_i] = <<currentCommunicationPoint>> _of_ <<fmi3DoStep>>) _but uses internally an own update formula, such as_ latexmath:[tc_{\mathit{s,i+1}} = tc_{\mathit{s,i}} + hc_{\mathit{s,i}}] _then the slave could use as time increment_ latexmath:[\text{hc}_{\mathit{s,i}} := (tc_i - tc_{\mathit{s,i}}) + hc_i] _(instead of_ latexmath:[\text{hc}_{\mathit{s,i}} := hc_i] _) to avoid a mismatch between the master time_ latexmath:[tc_{\mathit{i+1}}] _and the slave internal time_ latexmath:[tc_{\mathit{s,i+1}}] _for large i.]_
