...
--
* Parameter `valueReferences[]` is a vector of value references that define the variables whose <<derivative,`derivatives`>> shall be set or get.

* Parameter `nValueReferences` is the dimension of the parameters `valueReferences[]` and `orders[]`.

* Parameter `orders[]` contains the orders of the respective <<derivative>> (1 means the first <<derivative>>, 0 is not allowed).

* Parameter `values[]` is a vector with the values of the <<derivative,`derivatives`>>.

* Parameter `nValues` is the size of the parameter `values[]`.
--
...
* Parameter `currentCommunicationPoint` is the current communication point of the master (latexmath:[tc_i]) and

* parameter `communicationStepSize` is the communication step size (latexmath:[hc_i]). `comunicationStepSize` must be latexmath:[> 0.0].

* Argument `noSetFMUStatePriorToCurrentPoint = fmi3True` if `fmi3SetFMUState` will no longer be called for time instants prior to `currentCommunicationPoint` in this simulation run. _[The slave can use this flag to flush a result buffer]_

* Parameter `earlyReturn` signals to the Co-Simulation master if the FMU returns early from the `doStep` call (`earlyReturn = fmi3True`) or if the `doStep` was completed successfully (`earlyReturn = fmi3False`).
...
_[Formally, parameter `currentCommunicationPoint` is not needed._
...
* Parameter `lastSuccessfulTime` is the time instant at which the slave stopped the <<fmi3DoStep>> call.
