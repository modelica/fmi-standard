...
Every build configuration must contain at least one `<SourceFileSet>`. An importer of the FMU has to regard every SourceFileSet of the matching `<BuildConfiguration>`.
