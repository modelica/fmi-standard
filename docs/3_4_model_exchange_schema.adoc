...
|If `true` (the default), the function <<fmi3CompletedIntegratorStep>> must be called after every completed integrator step.
If `true` and the function is called anyway, it has no effect.
