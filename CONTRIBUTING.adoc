...
To re-generate the images, follow the steps:

. Download the latest XSDDiagram binaries from https://github.com/clagms/xsddiagram/releases.
. Unzip and place the binaries in the folder `<fmi-standard>/bin/xsdiagram` so that, e.g., the path of the binary `XSDDiagramConsole.exe` is `<fmi-standard>/bin/xsdiagram/XSDDiagramConsole.exe`
. On a powershell terminal placed at directory `<fmi-standard>`, run `.\bin\generate_figures.ps1`.
