...
We use a branching scheme inspired by https://nvie.com/posts/a-successful-git-branching-model/[GitFlow] with _support_ branches that allows us to maintain multiple major and minor releases concurrently.

Branch `main`::
Holds the latest (pre-)release.

Branch `develop`::
Holds the latest development version.
This is where the _next_ version of the standard is developed.
All pull requests (PRs) should be made against this branch.

Branch `support/v<major>.<minor>.x`::
Upon every release the latest version of `main` is tagged `v<major>.<minor>`.
The maintenance of this release is performed on a support branch `support/v<major>.<minor>.x` starting at this tag.
+
Example: `support/v3.0.x`

Branch `release/v<version>`::
Temporary branch for the release engineering.
+
Example: `release/v3.0-rc.1`
...
* create a release branch (e.g. `release/v3.0-rc.1`) to prepare the changes
...
* create a tag on `main` (e.g. `v3.0-rc.1`)
* create a new https://github.com/modelica/fmi-standard/releases[release] on GitHub
